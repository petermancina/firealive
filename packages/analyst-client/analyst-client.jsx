...
export default function AnalystClientApp() {
  ...
  const tabs=[
    {id:"home",label:"Home"},{id:"signals",label:"My Signals"},{id:"delegate",label:"Delegate"},
  ...
  return(
    <div style={{minHeight:"100vh",background:"#060A10",color:C.t,fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{padding:24,maxWidth:820,animation:"fadeIn 0.3s ease"}}>
        ...
        {tab==="certs_merged"&&(<div>
          <L>Certifications</L>
          <M style={{color:C.tm,display:"block",marginBottom:16,lineHeight:1.6}}>Register your professional certifications and training completions. Your Team Lead can verify them and they contribute to your skill profile. This creates a formal documented record of your professional development.</M>

          {/* INDUSTRY CERTIFICATIONS */}
          <Card style={{marginBottom:16}}>
            <div style={{fontSize:13,fontWeight:500,color:"#E8EDF5",marginBottom:10}}>Register Industry Certification</div>
            <Sel label="Certification">
              <option value="">Select certification...</option>
              <optgroup label="CompTIA">
                <option>CompTIA A+</option><option>CompTIA Network+</option><option>CompTIA Security+</option>
                <option>CompTIA CySA+</option><option>CompTIA PenTest+</option><option>CompTIA CASP+</option>
                <option>CompTIA Cloud+</option><option>CompTIA Linux+</option><option>CompTIA Server+</option>
                <option>CompTIA DataSys+</option>
              </optgroup>
              <optgroup label="ISC²">
                <option>CISSP</option><option>SSCP</option><option>CCSP</option><option>CSSLP</option>
                <option>CC (Certified in Cybersecurity)</option>
              </optgroup>
              <optgroup label="ISACA">
                <option>CISA</option><option>CISM</option><option>CRISC</option><option>CGEIT</option>
                <option>CDPSE</option><option>ITCA</option><option>CSX-P</option>
              </optgroup>
              <optgroup label="GIAC / SANS">
                <option>GSEC</option><option>GCIH</option><option>GCIA</option><option>GCFE</option>
                <option>GNFA</option><option>GPEN</option><option>GWAPT</option><option>GXPN</option>
                <option>GCFA</option><option>GREM</option><option>GDSA</option><option>GMON</option>
              </optgroup>
              <optgroup label="EC-Council">
                <option>CEH (Certified Ethical Hacker)</option><option>CHFI</option><option>ECSA</option>
                <option>LPT</option><option>CND</option>
              </optgroup>
              <optgroup label="Offensive Security">
                <option>OSCP</option><option>OSWE</option><option>OSEP</option><option>OSED</option>
                <option>OSWP</option><option>OSCE3</option>
              </optgroup>
              <optgroup label="Cloud Security">
                <option>AWS Security Specialty</option><option>AWS Solutions Architect</option>
                <option>Azure Security Engineer (AZ-500)</option><option>Azure Administrator (AZ-104)</option>
                <option>GCP Professional Cloud Security Engineer</option>
                <option>CCSK (CSA)</option><option>CCSP (ISC²)</option>
              </optgroup>
              <optgroup label="Cisco">
                <option>CCNA</option><option>CCNP Security</option><option>CyberOps Associate</option>
                <option>CyberOps Professional</option>
              </optgroup>
              <optgroup label="IAPP">
                <option>CIPP/US</option><option>CIPP/E</option><option>CIPM</option><option>CIPT</option>
                <option>AIGP</option>
              </optgroup>
              <optgroup label="Other">
                <option>OSINT (McAfee Institute)</option><option>BTL1 (Blue Team Level 1)</option>
                <option>BTL2 (Blue Team Level 2)</option><option>eCDFP</option><option>eCTHP</option>
                <option>Other (specify below)</option>
              </optgroup>
            </Sel>
            <Input label="Certification ID / Verification Number" placeholder="e.g., COMP001234567890"/>
            <Input label="Verification URL (if applicable)" placeholder="https://verify.comptia.org/..."/>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              <Input label="Date earned" type="date"/>
              <Input label="Expiration date (if applicable)" type="date"/>
            </div>
            <M style={{color:C.td,display:"block",marginBottom:10}}>Upload certificate file (PDF/image, max 5MB). Encrypted at rest.</M>
            <Btn primary onClick={()=>logC("INDUSTRY_CERT_SUBMITTED","Industry certification submitted for verification")}>Submit Certification</Btn>
          </Card>

          {/* TRAINING COMPLETIONS */}
          <Card style={{marginBottom:16}}>
            <div style={{fontSize:13,fontWeight:500,color:"#E8EDF5",marginBottom:10}}>Submit Training Completion</div>
            <Sel label="Platform">
              <option value="">Select...</option><option value="letsdefend">LetsDefend</option><option value="htb">Hack The Box</option><option value="thm">TryHackMe</option><option value="cyberdefenders">CyberDefenders</option><option value="sans">SANS Institute</option><option value="immersive">Immersive Labs</option><option value="udemy">Udemy</option><option value="coursera">Coursera</option><option value="pluralsight">Pluralsight</option><option value="internal">Internal Training</option>
            </Sel>
            <Input label="Course Name" placeholder="e.g., SOC Analyst Learning Path"/>
            <Input label="Verification Code" placeholder="Certificate verification code"/>
            <Btn primary onClick={()=>logC("TRAINING_CERT_SUBMITTED","Training completion submitted")}>Submit Completion</Btn>
          </Card>

          <Card>
            <div style={{fontSize:13,fontWeight:500,color:"#E8EDF5",marginBottom:10}}>My Certifications & Completions</div>
            <M style={{color:C.td}}>Submissions are sent to your Team Lead for verification. Verified certs appear in your skill profile and contribute to team certification coverage metrics.</M>
          </Card>
        </div>)}
        ...
        {tab==="privacy"&&(<div>
          ...
          <Card style={{marginBottom:16}}>
            ...
            <Card style={{padding:14,borderColor:C.i+"30",marginTop:12}}><div style={{fontSize:12,fontWeight:500,color:C.i,marginBottom:8}}>TOTP Setup</div><div style={{background:"rgba(0,0,0,0.3)",borderRadius:12,padding:16,textAlign:"center",marginBottom:10}}><div style={{width:120,height:120,margin:"0 auto",background:"#fff",borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center"}}><M style={{color:"#000",fontSize:10}}>TOTP QR</M></div></div><Input label="Key" value="JBSWY3DPEHPK3PXP" readOnly/><Input label="Code" placeholder="000000" maxLength={6}/><Btn primary onClick={()=>logC("MFA","Verified")}>Verify</Btn></Card>
