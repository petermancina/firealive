...
export default function AnalystClientApp() {
  ...
  // Seal the note and the post body to the independent reviewer's public key on
  // this device (main-process abuse:seal) before sending -- the server stores
  // only the opaque boxes and can never read them. Mirrors submitLeadFlag. The
  // accused is resolved server-side from the post; thread context is not sealed
  // here (the reviewer console does not display it).
  const flagBoardPost = async (postId, rootId, body) => {
    ...
    try {
      ...
      const r = await api.post("/api/peer/flags", { target_type: "board_post", boardPostId: postId, tier: boardFlagTier, sealedNote: sn.sealed, sealedContent: sc.sealed });
