...
// field (offending text, note). A single key is still accepted for back-compat.
ipcMain.handle('abuse:seal', async (_e, { recipientPublicKeys, recipientPublicKey, plaintext } = {}) => {
  const recipients = Array.isArray(recipientPublicKeys) && recipientPublicKeys.length > 0
    ? recipientPublicKeys
    : (typeof recipientPublicKey === 'string' && recipientPublicKey ? [recipientPublicKey] : []);
  if (recipients.length === 0) {
    throw new Error('recipientPublicKeys (non-empty array of base64) required');
  }
  if (!recipients.every((k) => typeof k === 'string' && k)) {
    throw new Error('each recipient public key must be base64');
  }
  if (typeof plaintext !== 'string') {
    throw new Error('plaintext (string) required');
  }
  return { sealed: sealToReviewers(recipients, plaintext) };
});
