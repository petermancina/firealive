// ══════════════════════════════════════════════════════════════════════════════
// FIREALIVE — Analyst Client GGUF Header Validator (defensive)
// Copyright (C) 2026 Peter Mancina
// SPDX-License-Identifier: AGPL-3.0-or-later
// ══════════════════════════════════════════════════════════════════════════════
//
// The format-validation layer (layer 3) of the Analyst Client's model-file
// integrity & safety gate. The local loader (node-llama-cpp / llama.cpp) parses
// the GGUF in-process on the analyst's machine, so a malformed header that
// triggers a parser overflow is in-process RCE. This validator defangs that
// attack surface by sanity-checking the GGUF header BEFORE the native loader
// reads the file: magic, version, the tensor/metadata counts (absolute caps,
// numeric precision, and a relative-to-file-size plausibility bound), and a
// bounded walk of the metadata key/value records (OOB lengths, unknown value
// types, nested arrays).
//
// Defensive validator, not a full parser: reads only a header prefix (default
// 16 MB), never the whole multi-GB file, and is biased AGAINST false positives
// (metadata running past the prefix is ACCEPTED). Twin of server gguf-validate.
//
