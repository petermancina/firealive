...
function createSignalE2EE(deps) {
  ...
  const KEY_NEXT_OTP = 'identity:nextOtpId';
  ...
  // Mint additional one-time pre-keys with FRESH, monotonic ids (never reusing
  // 1..N), advancing a persisted next-id counter so the server's published
  // publics always match the locally sealed privates. Returns the public material
  // to POST to /api/e2ee/prekeys.
  async function replenishOneTimePreKeys(count) {
    const n = (count && count > 0) ? count : 50;
    const stored = await bget(KEY_NEXT_OTP);
    let start = parseInt(stored, 10);
    if (!Number.isInteger(start) || start < 1) start = 1;
    const oneTimePreKeys = await generateOneTimePreKeys(start, n);
    await bset(KEY_NEXT_OTP, String(start + n));
    return { domain, oneTimePreKeys };
  }
  ...
  async function buildPublishableBundle(opts) {
    ...
    await bset(KEY_NEXT_OTP, String(oneTimeStartId + oneTimeCount));
