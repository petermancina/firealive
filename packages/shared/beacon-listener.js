...
// fingerprint = SHA-256 of the SPKI DER, key-type-aware verify). A beacon that does not
...
// Verify a signature with the public key's native algorithm: an Ed25519 key
// (a legacy software anchor) uses the null digest, while an EC P-256 key (the
// hardware anchor, D26) uses SHA-256 with the raw r||s / IEEE P1363 encoding.
// Any other key type fails closed. MUST match server/services/peer-beacon.js.
function verifyWithKey(publicKey, data, signature) {
...
function verifyBeacon(beacon) {
  try {
    ...
    if (!verifyWithKey(pub, beaconSigningBytes(beacon), sig)) {
