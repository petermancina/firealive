...
function Login({ onAuthed }) {
  ...
  // Apply the role gate. Any non-reviewer is rejected and NO token is kept.
  function finalize(r) {
    if (!r || !r.user || !r.accessToken) { setBusy(false); setError("Unexpected server response."); return; }
  ...
  async function submitCreds() {
    ...
    if (r && r.mfa_enrollment_required) { setBusy(false); setError("This reviewer account must finish multi-factor enrollment before using the console. Enrollment is handled in the next build step."); return; }
