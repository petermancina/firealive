...
// FIREALIVE -- offline Global Dashboard rollback CLI (B6k)
//
// Puts the PREVIOUS build back in front of its data, using a pre-upgrade restore
// point taken by that build. This is the only operation that lets a Global Dashboard
...
// server stopped.
//
//   Usage:  node gd-rollback-apply.js --koa <koa-id> [--restore-point <id>] [--dry-run]
//
...
// BEFORE RUNNING THIS
//
//   1. Stop the Global Dashboard server. This tool refuses while it is alive, because
...
const { applyRollback } = require('../services/gd-rollback-apply');
...
async function main(argv) {
  ...
  if (!args.koaId) {
    process.stderr.write('usage: node gd-rollback-apply.js --koa <koa-id> [--restore-point <id>] [--dry-run]\n');
    return 2;
  }
  ...
  process.stdout.write('Start the PREVIOUS version of the Global Dashboard now.\n');
  return 0;
}
