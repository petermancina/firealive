// ═══════════════════════════════════════════════════════════════════════════
// FIREALIVE — GD server boot posture check  (P1-2c)
//
// The twin of server/lib/data-posture.js with a DIFFERENT owned set. This is the
// CISO's own host.
// ═══════════════════════════════════════════════════════════════════════════
//
// WHAT THIS IS FOR
...
function ownedDirectories() {
  const out = [];
  const gdDataRoot = require('./gd-data-root');
  out.push({ label: 'data root', path: gdDataRoot.gdDataRoot() });
  out.push({ label: 'backups', path: gdDataRoot.backupsDir() });
  // NOT a copy of the Regional owned-set: the GD has NO logger and no log
  // directory (verified -- gd-data-root exports no logsDir, and there is no
  // gd-logger.js). A twin of the Regional check would inspect a path that
  // cannot exist. Do not "restore parity" here.
  ...
  if (process.platform === 'linux') {
    try {
      const ks = require('../services/gd-hardware-keystore-linux');
...
// owner, SYSTEM, or Administrators. Stricter than Everyone, on purpose.
...
const ACL_SCRIPT = [
  ...
  '}',
  '[pscustomobject]@{ owner = $owner; aces = @($aces) } | ConvertTo-Json -Compress -Depth 4',
].join('\n');
...
function readAcl(dir) {
  ...
  try {
    ...
    const out = execFileSync(powershellExe(),
      ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', file, dir],
...
function offendingAces(dir) {
  ...
  try {
    ...
    return [{ sid: null, id: '(acl unreadable)', rights: String((err && err.message) || err).slice(0, 200) }];
  }
  const allowed = new Set(ALLOWED_SIDS);
  if (acl && acl.owner) allowed.add(acl.owner);
