...
//
// HOT-FIX: this endpoint is intentionally disabled and returns 503 until
// the manual-approval flow is wired in by subsequent R3g PR3 commits.
//
// As originally shipped in PR3 C12, this endpoint accepted an api_key in
// the body and immediately inserted the supplied public key into the
// signing_keys trust registry with is_active=1. That made api_key
// compromise equivalent to silent trust takeover — an attacker with a
// stolen api_key could replace the legitimate MC's signing key with
// their own, then sign forged pushes that would verify cleanly.
// Strictly weaker than SOC-grade.
//
// The replacement design (PR3 C14-C22) is manual CISO approval:
//
//   1. MC submits its public key here — but it lands in signing_keys
//      with approval_status='pending_approval', is_active=0.
//   2. A user holding the 'ciso' or 'signing_key_approver' role (the
//      latter is new in R3g PR3, segregated from 'ciso' per ISO 27001
//      A.6.1.2 / NIST 800-53 AC-5) reviews the registration in the GD
//      admin surface, verifies the fingerprint out-of-band with the
//      MC operator, and clicks approve.
//   3. Only an approved row is consulted by mc-signature-verifier
//      when verifying inbound pushes (with a configurable grace window
//      for recently-rotated keys to avoid dropping in-flight work).
//
// While this hot-fix is in place, the endpoint is dark: it logs every
// attempted call (so we see if any production deploy is calling it)
// but returns 503 to every caller regardless of api_key validity. This
// guarantees no api_key-only trust establishment can occur on this
// branch between C13 and C18, even if a tester deploys mid-PR3.
//
// Response timing and shape are identical regardless of whether the
// supplied api_key resolves to a real MC, so callers cannot use this
// endpoint as an oracle for MC api_key validity.
app.post('/api/mc/:id/signing-key', (req, res) => {
  try {
    const { apiKey } = req.body || {};
    const db = getDb();

    // Audit-log every attempt so a CISO reviewing logs sees who hit
    // the disabled endpoint. Both valid and invalid api_key code
    // paths write one INSERT so timing is similar; mc identifier is
    // resolved if possible, otherwise 'unknown'.
    let mcLabel = 'unknown';
    if (apiKey) {
      const mc = db.prepare("SELECT id, name FROM management_consoles WHERE api_key = ? AND status = 'active'").get(apiKey);
      if (mc) mcLabel = `${mc.name} (${mc.id})`;
    }
    db.prepare("INSERT INTO audit_log (event_type, detail, severity) VALUES ('SIGNING_KEY_REGISTRATION_BLOCKED', ?, 'warning')")
      .run(`attempted_mc=${mcLabel} path_id=${req.params.id || 'none'} reason=endpoint_disabled_pending_manual_approval_flow`);

    db.close();

    // 503 (Service Unavailable) is the correct status: the resource
    // exists, the request is well-formed, but the server has
    // intentionally taken it offline. Retry-After is omitted because
    // the endpoint will not be re-enabled until a software update.
    return res.status(503).json({
      error: 'Signing key registration is temporarily unavailable. Manual CISO approval flow is being deployed; this endpoint will return when the new flow is active.',
    });
  } catch (e) {
    console.error('Signing-key endpoint hot-fix error:', e);
    return res.status(503).json({
      error: 'Signing key registration is temporarily unavailable.',
    });
  }
});
