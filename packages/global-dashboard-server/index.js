...
// ── MC Signing Key Registration (R3g PR3) ────────────────────────────────────
//
// POST /api/mc/:id/signing-key
//   Body: { apiKey, public_key, public_key_fingerprint }
//
// Called by an MC during its first GD-push handshake (and on any
// subsequent signing-key rotation) to register its Ed25519 public key
// in the GD's trust registry. After this lands, the MC's outbound
// pushes carry X-FA-Signature headers verified against the row written
// here.
//
// AUTHENTICATION
//
// api_key in the body (same auth model as /api/ingest/metrics) PLUS
// the :id path param must resolve to the same MC. Mismatch rejects
// with 403 so a thief of MC-A's api_key can't register a key under
// MC-B's id.
//
// We intentionally do NOT require X-FA-Signature on this endpoint,
// because at first-registration time the MC has no key in the GD
// trust registry to sign with. KNOWN LIMITATION for v1.0.33: an
// attacker who has stolen an MC's api_key can register their own
// signing key here, replacing the legitimate trust relationship.
// Mitigated by:
//   1. The handshake emits an MC_SIGNING_KEY_REGISTERED audit event
//      with both new and prior fingerprints, so a CISO reviewing the
//      audit stream sees unexpected rotations.
//   2. Rotation events are rare in normal operation — frequent
//      MC_SIGNING_KEY_REGISTERED entries are a red flag.
// Future hardening (post-v1.0): require non-initial registrations to
// be signed by the current active key. Tracked in build plan OPEN
// QUESTIONS.
//
// IDEMPOTENCY + ROTATION
//
//   - Existing active row with the same fingerprint: no-op, return
//     current state. The handshake fires on every gd-config PUT, so
//     idempotency matters.
//   - Different active fingerprint exists: this is a rotation. Demote
//     the prior row (is_active=0, rotated_out_at=now) and insert the
//     new fingerprint as is_active=1, in one transaction.
//   - No active row: this is initial registration. Insert as
//     is_active=1.
//
// FINGERPRINT VALIDATION
//
// The body carries both public_key (PEM) and public_key_fingerprint
// (SHA-256 hex of SPKI DER). We recompute the fingerprint server-side
// and reject if they don't match. Prevents a class of bugs where a
// caller-supplied fingerprint disagrees with the actual key bytes.
app.post('/api/mc/:id/signing-key', (req, res) => {
  try {
    ...
    if (!apiKey || !public_key || !public_key_fingerprint) {
      return res.status(400).json({ error: 'apiKey, public_key, and public_key_fingerprint are required' });
    }
    ...
    if (mc.id !== req.params.id) {
      db.close();
      return res.status(403).json({ error: 'API key does not match path mc_id' });
    }

    // Parse + fingerprint the supplied public key. Rejects garbage PEM
    // and detects fingerprint spoofing.
    let computedFingerprint;
    try {
      const keyObj = crypto.createPublicKey(public_key);
      const der = keyObj.export({ type: 'spki', format: 'der' });
      computedFingerprint = crypto.createHash('sha256').update(der).digest('hex');
    } catch (parseErr) {
      db.close();
      return res.status(400).json({ error: 'public_key did not parse as a valid public key' });
    }
    if (computedFingerprint !== public_key_fingerprint) {
      db.close();
      return res.status(400).json({
        error: 'public_key_fingerprint does not match computed fingerprint of public_key',
        computed: computedFingerprint,
      });
    }

    // Look at current trust state for this MC.
    const activeRow = db.prepare(`
      SELECT id, public_key_fingerprint FROM signing_keys
      WHERE mc_id = ? AND is_active = 1 LIMIT 1
    `).get(mc.id);

    // Idempotent re-registration of the same fingerprint: no DB change.
    if (activeRow && activeRow.public_key_fingerprint === computedFingerprint) {
      db.prepare("INSERT INTO audit_log (event_type, detail, severity) VALUES ('MC_SIGNING_KEY_REGISTERED', ?, 'info')")
        .run(`mc=${mc.name} mc_id=${mc.id} fingerprint=${computedFingerprint} action=idempotent_noop`);
      db.close();
      return res.json({
        success: true,
    ...
    // Either initial registration (no active row) or rotation
    // (different active fingerprint). Wrap in a transaction so the
    // demote-and-insert pair is atomic.
    const insert = db.transaction(() => {
      if (activeRow) {
        db.prepare(`
          UPDATE signing_keys
          SET is_active = 0, rotated_out_at = datetime('now')
          WHERE id = ?
        `).run(activeRow.id);
      }
      const result = db.prepare(`
        INSERT INTO signing_keys
          (mc_id, public_key, public_key_fingerprint, is_active, notes)
        VALUES (?, ?, ?, 1, ?)
      `).run(
        mc.id, public_key, computedFingerprint,
        activeRow ? 'rotation via /api/mc/:id/signing-key' : 'initial registration via handshake'
      );
      return result.lastInsertRowid;
    });
    const newRowId = insert();

    const action = activeRow ? 'rotated' : 'initial';
    const priorFp = activeRow ? activeRow.public_key_fingerprint : 'none';
    db.prepare("INSERT INTO audit_log (event_type, detail, severity) VALUES ('MC_SIGNING_KEY_REGISTERED', ?, 'info')")
      .run(`mc=${mc.name} mc_id=${mc.id} fingerprint=${computedFingerprint} priorFingerprint=${priorFp} action=${action}`);
    ...
    res.json({
      ...
      id: newRowId,
      fingerprint: computedFingerprint,
      priorFingerprint: activeRow ? activeRow.public_key_fingerprint : null,
    });
  } catch (e) {
    console.error('MC signing-key registration error:', e);
    res.status(500).json({ error: 'Signing key registration failed' });
  }
});
