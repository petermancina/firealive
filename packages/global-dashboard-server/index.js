...
function gdResolveLoginDeviceKey(db, user, req) {
  ...
  if (consumed.userId !== user.id) {
    return { ok: false, status: 401, error: 'device-key challenge was not issued for this account' };
  }
  let sig;
  ...
  return { ok: true, cnf: { jkt: gdDeviceKey.jwkThumbprint(active.public_key) } };
}
...
app.post('/api/auth/device-key/challenge', (req, res) => {
  ...
  try {
    ...
    const subject = user ? user.id : 'nouser:' + username;
    const { challenge, challengeToken } = gdDeviceKey.issueDeviceKeyChallenge(subject);
    return res.json({ challenge, challengeToken });
  } catch (e) {
    return res.status(500).json({ error: 'could not issue device-key challenge' });
  } finally {
    try { db.close(); } catch (_) { /* ignore */ }
  }
});
