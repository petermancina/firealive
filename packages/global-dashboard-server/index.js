...
app.post('/api/gd/query', authMiddleware(['ciso', 'vp']), (req, res) => {
  try {
    const { templateId, filterColumn, filterRegex, daysBack } = req.body || {};
    ...
    // Regex filter: optional. If present, both column and pattern required.
    let compiledRegex = null;
    let normalizedFilterColumn = null;
    if (filterColumn || filterRegex) {
      if (!filterColumn || !filterRegex) {
        return res.status(400).json({ error: 'filterColumn and filterRegex must both be provided together' });
      }
      ...
      if (typeof filterRegex !== 'string' || filterRegex.length > 256) {
        return res.status(400).json({ error: 'filterRegex must be a string up to 256 chars' });
      }
      try {
        compiledRegex = new RegExp(filterRegex);
      } catch (err) {
    ...
    // Apply regex post-filter if present
    if (compiledRegex && normalizedFilterColumn) {
      rows = rows.filter(row => {
        const cell = row[normalizedFilterColumn];
        if (cell == null) return false;
        return compiledRegex.test(String(cell));
      });
    }
    ...
    try {
      db2.prepare("INSERT INTO audit_log (user_id, event_type, detail, ip, severity) VALUES (?, 'GD_QUERY', ?, ?, 'info')")
        .run(
          req.user?.id || 'unknown',
          `template=${templateId} days=${days} filter=${normalizedFilterColumn || 'none'} pattern_len=${filterRegex?.length || 0} rows=${rows.length}`,
          req.ip
        );
    } finally {
      db2.close();
