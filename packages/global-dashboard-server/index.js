...
app.post('/api/ingest/compliance-reports', (req, res) => {
  try {
    ...
    db.prepare("INSERT INTO audit_log (event_type, detail, severity) VALUES ('COMPLIANCE_REPORT_INGESTED', ?, 'info')")
      .run(`From ${mc.name}: framework=${fw} reportId=${result.lastInsertRowid} bytes=${summaryJson.length} fingerprint=${sigResult.fingerprint}${sigResult.viaGraceWindow ? ' viaGraceWindow=true' : ''}`);
    db.close();
