...
app.use('/api/', (req, res, next) => {
  // Express strips the mount path, so req.path here is '/health', never
  // '/api/health'. The exclusions must be tested against originalUrl. Getting this
  // wrong is not cosmetic: a load-balancer health probe would stamp client activity,
  // keeping lastClientRequestAt fresh forever, and an active cut off from both its
  // peer and the SOC would look like it was still serving and would never
  // self-fence -- defeating the isolation fence in exactly the case it exists for.
  const fullPath = (req.originalUrl || req.url || '').split('?')[0];
  if (!fullPath.startsWith('/api/ha/peer') && fullPath !== '/api/health') {
