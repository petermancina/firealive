...
// Severity for the SIEM's correlation rules. HA_PROMOTION_REFUSED is critical rather
// than a warning: in cloud mode it means this node could not re-attest at the moment
// it was to take write authority, so the platform is unverified AND the pair may be
// left with no active. HA_PROMOTED and HA_SELF_FENCED are high -- an unplanned takeover
// or a self-fence of the fleet aggregation plane is something a SOC correlates against
// everything else happening at that minute. A graceful demotion is a warning; a
// throttled promotion and a successful segment re-registration are informational.
...
// Append the audit row through the connection this module is mutating, then -- and only
// when that connection IS the durable chain -- stream the event to the operator's SIEM.
//
// Without the stream a GD failover was recorded and otherwise invisible: the Regional
// Server's HA events reach the SIEM from its audit write, and the GD's did not. An
// unplanned promotion of the fleet aggregation plane is exactly the event a CISO expects
// to correlate against everything else in that minute.
//
// isLiveChain is the gate, not "was a handle injected": in production every caller
// supplies a live handle. A promotion, demotion, or self-fence exercised against a
// hermetic clone therefore records where the change happened and emits NOTHING, so a
// drill can neither forge a row into the tamper-evident log nor page a SOC with a fence
// that never occurred.
//
// The SIEM configuration is read synchronously, while the connection is certainly open;
// the dispatch is database-free and deliberately left un-awaited, because promote() and
// demote() are synchronous and their callers close the handle in a finally as soon as
// they return. Neither the audit nor the stream may break a promotion or demotion, so
...
function safeAudit(db, eventType, detail) {
  ...
  try {
    ...
    const alert = {
      type: eventType,
      severity: HA_EVENT_SEVERITY[eventType] || 'warning',
      message: detail,
    };
    Promise.resolve(pushAlertWithConfig(cfg, alert)).catch(function () { /* best-effort */ });
  } catch (streamErr) {
    try { console.error('gd-ha-failover SIEM stream failed:', streamErr && streamErr.message ? streamErr.message : streamErr); } catch (logErr) { /* ignore */ }
  }
}
