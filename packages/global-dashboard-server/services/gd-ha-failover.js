...
function auditRefusalThrottled(cfg, detail) {
  const now = Date.now();
  if ((now - lastRefusalAuditAt) > (cfg.promotionCooldownSec * 1000)) {
    lastRefusalAuditAt = now;
    safeAudit('HA_PROMOTION_REFUSED', detail);
  }
}
...
function promote(db, opts) {
  ...
  try {
    ...
    if (seg && seg.registered) {
      safeAudit('HA_SEGMENT_REREGISTERED', 'Re-registered HA peer segment ' + seg.segment + ' under the new active');
    }
  } catch (segErr) {
    safeAudit('HA_SEGMENT_REREGISTER_FAILED', 'Segment re-registration failed after promotion (non-fatal): '
...
function evaluatePromotion(db, opts) {
  ...
  try {
    ...
    auditRefusalThrottled(cfg, 'Automatic promotion refused: '
