// =============================================================================
// FIREALIVE GD -- Data-Residency Core Service
//
// The decision and reconciliation logic behind the data-residency policy: where
// the bytes physically live across jurisdictions (e.g. GDPR Chapter V). Twins the
// Regional data-residency service.
//
// Synchronous and (for the decision) side-effect-free: callers act on the
// returned verdict and own the audit/alert. better-sqlite3 is synchronous, so
// nothing here awaits.
//
// Exports:
//   loadResidencyConfig(db)                        -> the policy, defaults merged
//   evaluateDestination(db, category, ref)         -> a persisted-destination verdict
//   evaluateConfig(db, category, adapter, config, ref) -> a config-time verdict
...
//   defaultConfig(), foreignLawExposure(), deriveStatus(), isBlockedByPolicy()
//   CATEGORIES, MODES, RECONCILED_CATEGORIES, CONFIG_KEY
//
// The policy is destination-agnostic (data category -> permitted regions). The
// config lives in the config table under CONFIG_KEY -- not seeded; an absent or
// malformed key yields the default (off), mirroring the service-defaults-on-absent
// pattern used elsewhere in the GD server. evaluateConfig / evaluateDestination
// are the residency gates that the storage routing and destination-create paths
// call once wired.
//
// AGPL-3.0-or-later
// =============================================================================
...
// destinations routed to storage_destinations, so each is reconciled and
...
// remote destinations (all route to storage_destinations; snapshot via backup's
// route).
const RECONCILED_CATEGORIES = ['backup', 'audit_log', 'forensic_export', 'snapshot', 'cef_archive'];

// Default policy: off. live_deployment is declare-only (never blocked -- a running
// deployment cannot be relocated). Destination-backed categories start
...
// always receive a complete, well-typed shape. A missing or malformed key yields
// the default (off).
  ...
  if (RECONCILED_CATEGORIES.includes(category)) {
    try {
      ...
      if (dest && typeof dest.config === 'string') {
        let parsed = null;
        try { parsed = JSON.parse(dest.config); } catch (_e2) { parsed = null; }
...
// blocked is true only on a violation/undeclared under enforce mode.
function decide(cfg, category, resolveJur) {
  ...
  // Resolve jurisdiction (declared, else inferred). Undeclared fails closed under
  // enforce.
...
// Is a destination in the given country blocked by the category policy? Only when
// the category is in enforce mode with a non-empty permitted list that the country
// is not in. (Matches evaluateDestination's block logic.)
...
// secondary (snapshot inherits backup's route). gd-storage-routing is required
// lazily to avoid a require cycle (the routing residency gate requires this
// module once wired).
...
// categories only. Cross-border detection requires a declared primary residency;
// without one, none are computed.
...
// { transfers, documented, blocked }. documented = a recorded transfer mechanism
// (anything but 'unset' / 'none'); blocked = a refused write.
