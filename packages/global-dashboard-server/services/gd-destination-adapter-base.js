// =============================================================================
// FIREALIVE GD -- Destination Adapter Base
//
// Common interface, registry, and helpers for the Global Dashboard's
// storage-push destination adapters. Each adapter (local mount, SFTP, S3,
// Azure Blob, GCS) implements the interface contract documented below and
// registers itself via registerAdapter() at module-load time. This is the GD
// trust realm's own adapter surface, twinning the Regional Server's
// destination-adapter-base; the GD routes its own artifacts under its own
// anchor/KEK and is never a write-path into the Regional Server's
// destinations.
//
// TRUST-REALM NOTE: this base class is KEK-agnostic. Destination credentials
// are wrapped with the GD Tier-1 KEK (services/gd-encryption) by the storage-
// destinations registry, which decrypts them and attaches the plaintext to
// the push context before dispatch. Adapters never touch the KEK; they receive
// already-decrypted config/credentials.
//
// THE PUSH ORCHESTRATOR (services/gd-storage-push.js) dispatches based on the
// destination row's adapter column:
//
//   const adapter = getAdapter(destination.adapter);
//   if (!adapter) throw ...
//   const result = await adapter.push(artifactContext, { logger, timeoutMs });
//
...
//     message on failure.
//
//   async push(artifactContext, options) -> pushResult
//     The main upload operation. The GD routes five artifact types
//     (backup, snapshot, audit_log segment, forensic_export, cef_archive
//     segment); artifactContext is generic over all of them. Inputs:
//       artifactContext: {
//         artifactId,              // string identifier of the artifact
//                                  //   (backup id, segment id, export id)
//         sourceDir,               // absolute on-host path to the
//                                  //   directory holding the artifact files
//         files: [{                // metadata for each file to push
//           name,                  //   (one for a sealed segment; several
//           absolutePath,          //   for a full-suite backup or a
//           sizeBytes,             //   forensic export bundle)
//           sha256,
//         }],
//         manifestSha256,          // the artifact's manifest/content hash
//         createdAt,
...
//                                  //   artifact landed (path / URL / key)
//       bytesPushed,               // total bytes uploaded across the files
 ...
 * FireAlive servers; destinations always live on the server's
 * filesystem or remote SFTP, both POSIX.
 */
 ...
 * paths (e.g., joining destination root + artifact directory name)
