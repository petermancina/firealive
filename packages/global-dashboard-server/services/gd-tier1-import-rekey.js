// ─────────────────────────────────────────────────────────────────────────────
// FIREALIVE (Global Dashboard) ── Tier-1 Import Rekey (FA-MIG1)
...
//
// GD twin of server/services/tier1-import-rekey.js; the mirror of B-4 rekeyNode, with three differences: the source KEK comes from a
...
const tier1Kek = require('./gd-tier1-kek');
const tier1Seal = require('./gd-tier1-seal');
const koa = require('./gd-key-op-authorization');

// Every class='tier1' column -- node-local AND replicated. An imported DB has all of
// them sealed under the source's Tier-1 KEK.
const TIER1 = GD_TIER1_COLUMNS.filter((c) => c.class === 'tier1');
