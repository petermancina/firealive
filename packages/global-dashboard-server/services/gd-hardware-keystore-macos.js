// ═══════════════════════════════════════════════════════════════════════════════
// FIREALIVE GLOBAL DASHBOARD — Hardware Keystore (macOS Secure Enclave backend)
// ═══════════════════════════════════════════════════════════════════════════════
//
// The GD Server own macOS Secure Enclave keystore backend, a per-package copy
// of server/services/instance-anchor/hardware-keystore-macos.js (the GD
// backend shares no modules with server/). The Secure Enclave logic is
// identical; it is GD-namespaced -- FIREALIVE_GD_SE_HELPER / FIREALIVE_GD_SWIFT
// for the helper, and the SE key application tags are com.firealive.gd.sign. /
// com.firealive.gd.seal -- so a GD Server co-located with a regional server on
// one host never collides on a Secure Enclave key (the fixed seal tag in
// particular); signing keys are further isolated by the gd-inst- label.
//
...
//
// Helper runner: in production set FIREALIVE_GD_SE_HELPER to a bundled, signed,
// compiled helper binary; in development the Swift source below is run with the
// swift interpreter (FIREALIVE_GD_SWIFT, default "swift"). Either way the protocol
...
let SIGN_TAG = "com.firealive.gd.sign."
let SEAL_TAG = "com.firealive.gd.seal"
...
function swiftExe() {
  return process.env.FIREALIVE_GD_SWIFT || 'swift';
}
...
function preflight() {
  const custom = process.env.FIREALIVE_GD_SE_HELPER;
  if (custom) {
    try {
      fs.accessSync(custom, fs.constants.X_OK);
    } catch (err) {
      throw preflightError('FIREALIVE_GD_SE_HELPER="' + custom + '" is not an executable file (' + (err.code || err.message) + '); point it at the bundled, signed helper binary.');
    }
    return;
  }
  try {
    ...
    throw preflightError('the Swift toolchain (' + swiftExe() + ') could not run (' + (err.code || err.message) + '); install the Xcode command-line tools, or set FIREALIVE_GD_SE_HELPER to the bundled, signed helper binary.');
  }
}
...
function realRunHelper(args) {
  ...
  if (custom) {
    try {
      ...
      if (err && err.code === 'ENOENT') {
        throw preflightError('FIREALIVE_GD_SE_HELPER="' + custom + '" could not be executed (ENOENT); point it at the bundled, signed helper binary.');
      }
      throw err;
    }
  }
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'fa-gd-se-'));
  ...
  try {
    ...
    if (err && err.code === 'ENOENT') {
      throw preflightError('the Swift toolchain (' + swiftExe() + ') could not run (ENOENT); install the Xcode command-line tools, or set FIREALIVE_GD_SE_HELPER to the bundled, signed helper binary.');
    }
    throw err;
  } finally {
    try {
