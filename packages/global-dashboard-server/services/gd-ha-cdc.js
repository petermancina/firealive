...
//
// EXCLUDE-LIST. Per the build rule: tables that exist on BOTH servers copy the
// MC's decision exactly; GD-only tables are decided on the master principle
// (per-node state stays local; shared fleet/operational data replicates).
// Excluded:
//   - the gd_ha_* HA control plane (GD-only; twin of the MC's ha_* exclusion);
//   - gd_instance_identity and system_health: the GD's own hardware identity and
//     per-node runtime observations (GD-only; the MC excludes its instance_identity
//     / instance_observations equivalents -- each node keeps its own);
//   - audit_log, auth_log, audit_chain_checkpoint: the audit/auth hash-chain, the
//     exact shared tables the MC excludes to avoid forking a per-node-signed chain;
//   - every *_signing_keys table (suffix) AND the bare signing_keys table (GD-only,
//     not caught by the suffix): per-node signing-key material, which the MC excludes.
// Everything the MC replicates is replicated here too -- notably backup_chain,
// forensic_export_chain, config_lock_state, system_meta, and sessions -- along with
// the GD-only shared data (connected MCs, regional rollups, compliance aggregates,
// SDN/SASE deployment posture) the passive must serve after promotion.
//
