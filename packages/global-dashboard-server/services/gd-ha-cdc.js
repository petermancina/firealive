// FIREALIVE GLOBAL DASHBOARD -- HA change-data-capture (B6d)
//
// The GD twin of server/services/ha/ha-cdc.js. Trigger-based logical CDC for
// active/passive replication. On the active, an AFTER INSERT/UPDATE/DELETE
// trigger on every replicated table appends the committed row change to
// gd_ha_replication_journal (table, op, primary key, and the full row as JSON).
// gd-ha-replication ships those rows to the passive, which applies them by
// primary key. The triggers are WHEN-gated on the active role, so they are
// completely inert when this node is standalone or passive -- no journal rows, no
// overhead beyond one indexed singleton lookup per write.
//
...
//     chokepoint does not exist on the GD either).
//
...
// writes a fully consistent copy (a plain file copy of a live WAL database is not
// safe). It is used for initial sync at pairing and periodic drift correction;
// gd-ha-replication ships it and the passive restores it.
//
// EXCLUDE-LIST (the operational-vs-local boundary, finalized here). Node-local
// tables that must NEVER be replicated to the peer, twinning the MC's categories
// adapted to the GD's schema:
//   - the gd_ha_* HA control plane (prefix + explicit);
//   - gd_instance_identity: the GD's own hardware-bound identity, which must not
//     be overwritten on the passive (the passive has its own);
//   - system_health / system_meta / config_lock_state: per-node runtime
//     observations, the anti-rollback fuse mirror, and the config-lock state (the
//     MC's instance_observations analogue -- each node keeps its own);
//   - the per-node hash-chain integrity tables (audit_log, auth_log,
//     audit_chain_checkpoint, backup_chain, forensic_export_chain): each is signed
//     with per-node signing keys, so replicating the chain would fork it -- kept
//     local, reconciled operationally, exactly as the MC keeps its audit chain;
//   - every *_signing_keys table (suffix): per-node signing-key material.
// Everything else (the connected MCs, regional rollups, backups metadata, storage
// routing, reports, tickets/training the passive must serve after promotion) is
// replicated.
//
...
const TRIGGER_PREFIX = 'fa_gd_ha_cdc_';
...
const DEFAULT_EXCLUDE_TABLES = [
  ...
  'system_health', 'system_meta', 'config_lock_state',
  'audit_log', 'auth_log', 'audit_chain_checkpoint',
  'backup_chain', 'forensic_export_chain',
];
const DEFAULT_EXCLUDE_PREFIXES = ['gd_ha_', 'sqlite_'];
...
function epochExpr() {
  return "COALESCE((SELECT epoch FROM gd_ha_lease WHERE id = 'current'), 0)";
}

// Only journal when this node is the active writer; inert otherwise.
function activeGuard() {
  return "COALESCE((SELECT role FROM gd_ha_node WHERE id = 'self'), 'standalone') = 'active'";
}
...
function oneTrigger(name, event, table, body) {
  ...
  lines.push('BEGIN');
  lines.push('  INSERT INTO gd_ha_replication_journal (epoch, table_name, op, pk_json, row_json) VALUES (');
...
// Drop every fa_gd_ha_cdc_* trigger (so a removed/renamed table leaves no orphan).
...
// Returns { path, bytes }. gd-ha-replication ships it; the passive restores it.
function createBaselineSnapshot(db, outPath) {
  const dest = outPath || path.join(
    os.tmpdir(),
    'firealive-gd-ha-baseline-' + Date.now() + '-' + crypto.randomBytes(4).toString('hex') + '.sqlite'
  );
