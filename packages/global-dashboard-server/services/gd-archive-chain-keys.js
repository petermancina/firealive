// =============================================================================
// FIREALIVE GD -- Archive Chain Signing Keys Service
//
// Manages the dedicated Ed25519 signing family for the GD's sealed archive-
// segment chain (storage_archive_segments). The GD archive-segment primitive
// signs each segment's manifest with the active key here; verifyChain re-
// verifies every segment manifest against the fingerprint recorded on the
// segment. Twins the Regional B5q archive-chain-keys service for the GD trust
// realm.
//
// This family is intentionally separate from the GD's audit / forensic signing
// families -- it is its own table (archive_chain_signing_keys) -- so a
// compromise of one custody chain's signing key cannot forge another's. The
// private material is wrapped under the GD Tier-1 KEK.
//
// Key material:
//   public_key             PEM SPKI, plaintext (no confidentiality concern)
//   private_key_encrypted  GD-Tier-1-KEK-wrapped via gd-encryption encryptConfig
//                          (AES-256-GCM envelope); decrypted just-in-time per
//                          signing op and never cached at module scope.
 ...
 *
 * Boot-time idempotent helper (called from the server boot hook). If no row in
 * archive_chain_signing_keys has active = 1, generate a fresh Ed25519 keypair,
 * GD-Tier-1-KEK-wrap the private key, and insert it. Safe to call on every start.
 *
 ...
 *
 * Load and decrypt the active private key for signing. The raw key is returned
 * as a KeyObject and never cached -- the caller signs and discards. Throws if
 ...
 *
 * Verify a segment manifest signature by the recorded fingerprint. Returns true
 * only if the fingerprint resolves to a known public key AND the signature is a
 * valid Ed25519 signature over manifestBytes. Returns false (never throws) on a
 * malformed/unknown fingerprint or a signature mismatch, so verifyChain can
 * report the broken segment rather than crash.
 */
 ...
 * fresh active keypair, in one transaction. Old rows are retained so historical
 * segment manifests stay verifiable by fingerprint.
 *
