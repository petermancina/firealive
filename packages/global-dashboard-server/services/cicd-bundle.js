...
//   7. build (docker buildx with --provenance=mode=max --sbom=true
//      for SLSA L3)
//   8. sign (Cosign keyless OIDC default; override via
//      COSIGN_KEY_MODE=key-based + COSIGN_PRIVATE_KEY)
//   9. CVE scan (Trivy with HIGH,CRITICAL exit 1)
...
const GD_CICD_SHAPE = {
  imageName: 'firealive-gd',
  imageRegistry: 'ghcr.io/petermancina/firealive-gd',
  port: 4001,
...
function buildReadme(platform, purpose, snapshot, configId) {
  ...
  return [
    ...
    `7. Build (docker buildx with --provenance=mode=max --sbom=true for SLSA L3)`,
...
function renderGithubActions(snapshot, purpose) {
  return [
    ...
    '',
    '      - name: Build (SLSA L3)',
    `        run: docker buildx build --provenance=mode=max --sbom=true -t ${GD_CICD_SHAPE.imageName}:\${{ github.sha }} .`,
    '',
    '      - name: Install Cosign',
    ...
    `            cosign sign --yes --key /tmp/cosign.key ${GD_CICD_SHAPE.imageName}:\${{ github.sha }}`,
    '            rm /tmp/cosign.key',
    '          else',
    `            cosign sign --yes ${GD_CICD_SHAPE.imageName}:\${{ github.sha }}`,
    '          fi',
    '',
    '      - name: Trivy CVE scan',
    '        uses: aquasecurity/trivy-action@master',
    '        with:',
    `          image-ref: ${GD_CICD_SHAPE.imageName}:\${{ github.sha }}`,
...
function renderGitlabCi(snapshot, purpose) {
  return [
    ...
    '  script:',
    `    - docker buildx build --provenance=mode=max --sbom=true -t ${GD_CICD_SHAPE.imageName}:$CI_COMMIT_SHA .`,
    '',
    'sign:',
    '  stage: sign-verify',
    ...
    '      if [ "${COSIGN_KEY_MODE:-keyless}" = "key-based" ]; then',
    '        echo "$COSIGN_PRIVATE_KEY" > /tmp/cosign.key',
    `        cosign sign --yes --key /tmp/cosign.key ${GD_CICD_SHAPE.imageName}:$CI_COMMIT_SHA`,
    '        rm /tmp/cosign.key',
    '      else',
    `        cosign sign --yes ${GD_CICD_SHAPE.imageName}:$CI_COMMIT_SHA`,
    '      fi',
    '',
    'cve-scan:',
    '  stage: sign-verify',
    '  image: aquasec/trivy:latest',
    `  script: [trivy image --severity HIGH,CRITICAL --exit-code 1 ${GD_CICD_SHAPE.imageName}:$CI_COMMIT_SHA]`,
    '',
    'fuse-counter:',
    '  stage: sign-verify',
    '  script:',
...
function renderJenkinsfile(snapshot, purpose) {
  return [
    ...
    '    }',
    '    stage("Build (SLSA L3)") {',
    `      steps { sh "docker buildx build --provenance=mode=max --sbom=true -t ${GD_CICD_SHAPE.imageName}:\${BUILD_NUMBER} ." }`,
    '    }',
    '    stage("Sign") {',
    "      steps {",
    "        sh '''",
    ...
    "            echo \"$COSIGN_PRIVATE_KEY\" > /tmp/cosign.key",
    `            cosign sign --yes --key /tmp/cosign.key ${GD_CICD_SHAPE.imageName}:\${BUILD_NUMBER}`,
    "            rm /tmp/cosign.key",
    "          else",
    `            cosign sign --yes ${GD_CICD_SHAPE.imageName}:\${BUILD_NUMBER}`,
    "          fi",
    "        '''",
    "      }",
    '    }',
    '    stage("CVE scan") {',
    `      steps { sh "trivy image --severity HIGH,CRITICAL --exit-code 1 ${GD_CICD_SHAPE.imageName}:\${BUILD_NUMBER}" }`,
    '    }',
    '    stage("Fuse-counter") {',
    "      steps {",
    "        sh '''",
...
function renderCircleCi(snapshot, purpose) {
  return [
    ...
    '      - run:',
    '          name: Build (SLSA L3)',
    `          command: docker buildx build --provenance=mode=max --sbom=true -t ${GD_CICD_SHAPE.imageName}:$CIRCLE_SHA1 .`,
    '      - run:',
    '          name: Install Cosign',
    ...
    '            if [ "${COSIGN_KEY_MODE:-keyless}" = "key-based" ]; then',
    '              echo "$COSIGN_PRIVATE_KEY" > /tmp/cosign.key',
    `              cosign sign --yes --key /tmp/cosign.key ${GD_CICD_SHAPE.imageName}:$CIRCLE_SHA1`,
    '              rm /tmp/cosign.key',
    '            else',
    `              cosign sign --yes ${GD_CICD_SHAPE.imageName}:$CIRCLE_SHA1`,
    '            fi',
    '      - run:',
    '          name: CVE scan',
    '          command: |',
    '            curl -sSfL https://raw.githubusercontent.com/aquasecurity/trivy/main/contrib/install.sh | sh -s -- -b /usr/local/bin',
    `            trivy image --severity HIGH,CRITICAL --exit-code 1 ${GD_CICD_SHAPE.imageName}:$CIRCLE_SHA1`,
    '      - run:',
    '          name: Fuse-counter',
    '          command: |',
    "            PRIOR=$(git show origin/main:package.json | node -e \"let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>console.log(JSON.parse(d).fuseCounter))\")",
