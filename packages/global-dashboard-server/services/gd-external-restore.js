...
//  13.  Confirm extracted file is named global-dashboard.db
...
function loadAdapter(sourceType) {
  ...
  const adapter = require(`./gd-external-restore/${adapterName}`);
...
async function executeRestore(db, args) {
  ...
  const log = (typeof args.log === 'function')
    ? args.log
    : (level, msg, meta) => {
        const fn = (typeof logger[level] === 'function') ? logger[level] : console.log;
