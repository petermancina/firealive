// -----------------------------------------------------------------------------
// FIREALIVE Global Dashboard -- Live Database Restore Swap (shared primitive)
// Copyright (C) 2026 Peter Mancina
// SPDX-License-Identifier: AGPL-3.0-or-later
// -----------------------------------------------------------------------------
//
// The one security-reviewed path for replacing the live GD SQLite database with
// the contents of a signed, KEK-wrapped backup archive, the GD twin of the
// Regional Server's live restore-swap primitive. Both the external restore
// orchestrator and the deployment-migration apply call this, so the most
// sensitive operation in the platform has a single implementation rather than
// divergent copies.
//
// Steps, in order (each fatal):
//   1. unwrap the data encryption key under the GD Tier-1 KEK
//   2. decrypt + decompress + untar the archive
//   3. confirm the extracted file is global-dashboard.db
...
//
// The scan runs the EDR dispatcher directly (inspectFile: layer 2 only -- a
// database file is not text, so the layer-1 content sanitizer does not apply).
//
// Contract: the caller MUST close its own database handle before calling, so the
// rename does not write to an unlinked ghost inode. getDb opens a fresh
// connection per call, so after this returns the caller simply calls getDb again
// to operate on the restored database; no process restart is required for
// connection correctness (a restart is still advisable to refresh any process-
// lifetime caches). The short-lived scan connection here is opened and closed
// internally and reads the pre-swap database (where scanner configuration lives).
// -----------------------------------------------------------------------------
...
const EXPECTED_DB_FILENAME = 'global-dashboard.db';
...
//
// Mandatory EDR scan of the extracted database bytes. Opens its own short-lived
// connection (the pre-swap database holds scanner configuration) and closes it.
// Fail-closed: throws on no configured scanner, on a detected threat, and on an
// inconclusive result. Returns the scan result on success.
// -----------------------------------------------------------------------------
async function scanExtractedBytes(payload) {
  ...
  if (scanResult.skipped === true) {
    throw new DbRestoreError(CODES.SCANNER_NOT_CONFIGURED,
      ...
      + 'Configure one under the GD Malware Scanners screen and retry.');
  }
...
//
// options:
//   archiveBytes     Buffer of the encrypted archive
//   wrappedKeyBytes  Buffer of the KEK-wrapped data key
...
//
// Returns { preRestorePath, scan, restored: true }. Throws DbRestoreError on any
// fatal condition. The caller must have closed its own db handle first.
// -----------------------------------------------------------------------------
async function restoreDatabaseFromArchive(options) {
  ...
  console.log('gd-db-restore-swap: live database restored (label=' + label + ', preRestorePath=' + preRestorePath + ')');
