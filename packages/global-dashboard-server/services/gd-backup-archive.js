// =============================================================================
// FIREALIVE GD -- Backup Archive Builder/Extractor
//
// Produces and consumes the archive.tar.gz.enc file in a GD v2 backup directory.
// Twins the Regional backup-archive builder, with one deliberate divergence: the
// GD compresses with gzip (Node zlib, built in) instead of zstd, so the GD
// server package carries no @mongodb-js/zstd dependency. The pipeline:
//
//   buildArchive(buffer, name)
//     buffer     --[ tar with single ustar entry ]-->  tarred bytes
//     tarred     --[ gzip compress ]-->                 compressed bytes
//     compressed --[ AES-256-GCM, fresh ephemeral key, fresh IV ]-->
//       encrypted bytes ([iv | authTag | ciphertext], the gd-encryption
//       byte convention)
//
//   extractArchive(encryptedBytes, ephemeralKey)
//     reverses every step, returns the original buffer + filename
//
// Returns the ephemeral 256-bit key alongside the encrypted bytes so the caller
// (gd-backup-v2) can wrap that key with the GD Tier-1 KEK separately and store
// the wrapped result in wrapped-key.bin. The key is NEVER written to disk by this
// module or persisted in the database -- its only authoritative storage is the
// wrapped form held by the caller.
//
// The SHA-256 of the final encrypted bytes is what the manifest records
// (file_hashes.archive.sha256) and what the restore service verifies before
// decrypting.
//
// MEMORY MODEL
//
// Buffered, not streaming. AES-GCM finalizes its auth tag only after all
// plaintext has been processed, so a true streaming pipeline cannot emit
// verified ciphertext incrementally regardless of how the upstream (tar, gzip)
// is structured. A buffered design also keeps the implementation simple and
// auditable. MAX_SOURCE_BYTES caps the input at 8 GB to bound peak memory at
...
const DEFAULT_GZIP_LEVEL = 6;      // zlib default; good size/speed balance

// -- Tar (POSIX ustar) --------------------------------------------------------

/**
 * Build a single-entry POSIX ustar archive from a Buffer plus a filename. The
 * result is suitable input for any standard tar reader (and for extractTar).
 *
 * Layout:
 *   [ 512-byte header ]
...
function buildTar(payload, filename) {
  ...
  //   0 filename(100) 100 mode(8) 108 uid(8) 116 gid(8) 124 size(12)
  //   136 mtime(12) 148 chksum(8) 156 typeflag(1) 157 linkname(100)
  //   257 magic(6) 263 version(2) 265 uname(32) 297 gname(32)
  //   329 devmajor(8) 337 devminor(8) 345 prefix(155) 500 pad(12)
  ...
  // Checksum field is computed below; for the compute itself, treat the 8-byte
  // chksum field as ASCII spaces.
  ...
  // Write checksum as 6 octal digits + NUL + space (GNU tar convention).
...
/**
 * Extract a single-entry POSIX ustar archive. Returns { name, payload }.
 * Verifies header checksum; rejects multi-entry archives (this module only ever
 * produces single-entry archives).
 */
function extractTar(tarBytes) {
  ...
  // Verify the next block (after padding) is the start of the end-of-archive
  // marker. We don't allow multi-entry archives; a non-zero block here would
  // mean a second entry.
...
/**
 * Build an encrypted, gzip-compressed tar archive from a source buffer.
 *
 * Inputs:
 *   sourceBytes   Buffer  -- the data to archive (typically a SQLite .db file)
 *   sourceName    string  -- the filename to record in the tar header
 *   options               -- { compressionLevel? = 6 }  (gzip level 0-9)
 *
 * Returns:
 *   {
 ...
 *   }
 *
 * Throws on a non-Buffer source, an oversized/empty source, an invalid tar
 * filename, or a (rare) gzip failure.
 */
async function buildArchive(sourceBytes, sourceName, options = {}) {
  ...
  const compressionLevel = options.compressionLevel != null ? options.compressionLevel : DEFAULT_GZIP_LEVEL;

  // 1. Tar
  const tarred = buildTar(sourceBytes, sourceName);

  // 2. Gzip compress
  const compressed = await gzipAsync(tarred, { level: compressionLevel });
  ...
  // Same layout as gd-encryption: [iv | tag | ciphertext]
...
/**
 * Inverse of buildArchive. Takes the encrypted bytes and the (already-unwrapped)
 * ephemeral key, returns { name, payload }. Throws with a precise error on input
 * too short to contain IV + tag, an AES-GCM auth-tag mismatch (any tampering of
 * ciphertext, IV, or tag), a gzip decompression failure, or a tar parsing
 * failure (malformed header, bad checksum, multi-entry, etc.).
 */
async function extractArchive(encryptedBytes, ephemeralKey) {
  ...
  let tarred;
  try {
    tarred = await gunzipAsync(compressed);
  } catch (err) {
    throw new Error(`extractArchive: gzip decompression failed: ${err.message}`);
  }
