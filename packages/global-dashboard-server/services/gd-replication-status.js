// =============================================================================
// FIREALIVE GD -- Replication Status
//
...
// guaranteed. It brings the backup, archive (audit_log, cef_archive), forensic,
// and snapshot push tables into a single per-type view. Twins the Regional
...
//
//   3. Permanent vs transient failure is explicit: a permanent failure is
//      status='failed' AND attempt_count >= MAX_ATTEMPTS (imported from the push
//      engine so the ceiling stays a single source of truth). Permanent failures
//      drive the 'failing' health -- the signal that a copy is not protected.
//
...
const { MAX_ATTEMPTS } = require('./gd-storage-push');

// Per-data-type source: which push table holds its pushes, how to reach the
// parent artifact that identifies the type, and the parent predicate. dataType is
// always one of storageRouting.VALID_DATA_TYPES (a fixed allow-list), so these
// fragments are constants -- no external input is interpolated into SQL. The GD
...
// is of this type. MAX_ATTEMPTS is interpolated as a trusted integer constant; the
// destination ref is bound.
