...
// Verifies the GD-side SSO method distribution. GD has no
// integration_config table; SSO method is stored per-user in
// users.auth_method (values: local | saml | oidc | ldap). If all users
// are 'local' auth, local bcrypt is the only method in use; otherwise
// an external IdP is contributing identities.
//
// Note: this check inspects WHICH auth methods are in use, not whether
// the IdP is currently reachable — there is no health-check loop in
// the GD code that probes the SAML/OIDC/LDAP endpoint. Health is
// inferred from successful auth_log entries with method = saml/oidc/ldap.
//
...
function checkIamIntegrationHealth(db) {
  ...
  if (nonLocal.length === 0) {
    return {
      status: 'pass',
      detail: `All users use local bcrypt auth (${methods[0].c} users). GD supports SAML/OIDC/LDAP via users.auth_method column; no external IdP integration currently configured.`,
    };
  }
  ...
  return {
    status: 'pass',
    detail: `Auth method distribution: ${summary}.${recentSummary} Local bcrypt remains available as failover.`,
  };
}
