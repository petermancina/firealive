...
module.exports = (checks) => ({
  ...
  verifiedControls: [
    ...
    },
    {
      ...
      check: checks.checkEncryption,
      mapping: 'The GD\'s secrets are encrypted at the application layer: every signing-key private key, the GD CA key and integration credentials are AES-256-GCM sealed under a Tier-1 KEK hardware-sealed to the host TPM 2.0 / Secure Enclave, so a copied disk or cloned VM cannot unseal them. General table data is not application-layer encrypted and rests on filesystem-level protection (operator-managed disk encryption: LUKS / FileVault / BitLocker / AWS EBS encryption). §1798.150(a)(1) limits statutory damages under the private right of action to breaches of nonencrypted and nonredacted personal information. Filesystem-level encryption substantially reduces private-right-of-action exposure for that remainder. Extending application-layer encryption to general table data remains future work, and is separate from the key-wrapping registry: that covers the backup archive key, not the database contents.',
    },
    {
      id: '§1798.150 [Key Management]',
      name: 'Encryption Defense -- Key Management',
      check: checks.checkKeyRotation,
      mapping: 'GD_JWT_SECRET rotation is operator-managed (quarterly cadence recommended). MC signing keys rotate through the signing_keys registry with an atomic demote-and-promote and an operator-configured grace window; the management_consoles.api_key identifier rotates per 90-day cadence. Hardware-backed KMS integration the GD backup data key may be wrapped by an external key-management provider (AWS KMS, Azure Key Vault, GCP KMS or HashiCorp Vault) registered in gd_kms_providers, giving FIPS-validated custody, provider-side rotation and provider-side audit for the archive key. The Tier-1 KEK itself is escrowed to no provider: recovering a deployment means re-establishing it from the offline recovery code, so a provider is custody and never a recovery path. The encryption defense under §1798.150 only holds if the encryption keys themselves were not also breached -- key custody and rotation are material, and operator-managed disk-encryption key custody (cloud-provider KMS for managed-disk encryption, secrets-manager for application keys) is the current source of cryptographic isolation.',
    },
  ],
