// ═══════════════════════════════════════════════════════════════════════════════
// FIREALIVE GLOBAL DASHBOARD — Compliance Framework: HIPAA
//
// R3g PR2 (v1.0.33): GD-side HIPAA Security Rule + Breach Notification
// Rule coverage under the Shared Responsibility schema. GD-side
// counterpart to MC PR1's frameworks/hipaa.js. Same metadata, same
...
//
//   FireAlive's Global Dashboard is NOT itself a HIPAA covered entity.
//   The GD aggregates governance-tier data (CISO-tier dashboards
//   across multiple regional SOCs) and does not directly receive
//   analyst-level data containing ePHI; that data flows through the
//   MC. Per the GD's architectural data model, only aggregate metrics
//   (regional_metrics, no analyst-identifying fields) and account
//   data (users) reach the GD.
//
//   This framework definition is provided as a SERVICE to customers
//   (SOC operators) running FireAlive who themselves are HIPAA covered
//   entities or business associates. The GD's compliance posture
//   supports the covered entity's overall HIPAA program at the
//   governance layer; analyst-level ePHI controls are evaluated at
//   the MC, not the GD.
//
...
//   at the GD layer (audit review, access management, password
//   management, backup plan, DR plan, DR testing).
//
...
//   CISO can see the complete HIPAA compliance surface in one report.
//
...
//   FireAlive GD implements both Required and Addressable
//   specifications uniformly where the underlying capability exists
//   — treating Addressable as Required is industry best practice and
//   SOC-grade norm. Honest gaps (features planned but not yet shipped)
//   are surfaced by the underlying check function with reference to
//   the BUILD-PLAN-v16 phase that closes each gap.
//
// AGPL-3.0-or-later
// ═══════════════════════════════════════════════════════════════════════════════
...
module.exports = (checks) => ({
  ...
  verifiedControls: [
    // ── 164.312 Technical Safeguards ─────────────────────────────────────────
    {
      ...
      mapping: 'Role-based access control via users.role (ciso / vp / readonly) + authMiddleware role-array gating on every /api route. Required Standard. The GD enforces unique user IDs (UNIQUE constraint on users.username), explicit role assignment per user, and audit logging of role-relevant events via CONFIG_UPDATED in audit_log.',
    },
    {
      id: '164.312(a)(2)(i)',
      name: 'Unique User Identification (Required)',
      check: checks.checkUniqueUsers,
      mapping: 'Unique username constraint enforced at the database layer (UNIQUE on users.username). Required implementation specification under 164.312(a)(2). The GD\'s user surface is narrow (CISO, VP, readonly auditors); each account is individually attributable.',
    },
    {
      id: '164.312(a)(2)(ii)',
    ...
    },
    {
      ...
      mapping: 'JWT expiresIn hardcoded at "8h" on the GD, matching CISO operational rhythm (infrequent multi-step actions). Addressable specification; the 8h window exceeds the SOC-grade 30-minute norm by design — for shorter idle timeouts, reverse-proxy session cookies with shorter TTL are operator-managed.',
    },
    {
      id: '164.312(a)(2)(iv)',
    ...
    },
    {
      ...
      mapping: 'Request-logging middleware (inline in packages/global-dashboard-server/index.js) records every /api request (except /api/health) to audit_log with user_id, event_type, detail, ip, severity, timestamp. Required Standard. SIEM streaming for external tamper-evident export lands when integration_config + B3 SIEM/SOAR wiring (v1.0.48) ship.',
    },
    {
      id: '164.312(c)(1)',
    ...
    },
    {
      ...
      mapping: 'TLS 1.2 minimum enforced at reverse proxy (operator-configured: ssl_protocols TLSv1.2 TLSv1.3 in nginx, equivalent in other proxies). TLS provides per-segment integrity via MAC authentication. Addressable specification, enforced at the reverse-proxy layer.',
    },
    {
      id: '164.312(e)(2)(ii)',
      name: 'Encryption (Transmission) (Addressable)',
      check: checks.checkEncryption,
      mapping: 'TLS 1.2+ at the reverse proxy encrypts all in-transit traffic. Backup destinations support the encrypted=true flag on backup_schedules; destination-side at-rest encryption (S3 SSE / GCS CMEK / Azure SE / local LUKS) is operator-managed. Addressable specification.',
    },
    // ── 164.308 Administrative Safeguards with technical implementations ────
    {
      ...
      mapping: 'audit_log volume monitoring on the GD detects zero-volume conditions (potential logging failure or upstream reverse-proxy failure) and anomalous spikes (potential DoS or incident). Required implementation specification under 164.308(a)(1).',
    },
    {
      id: '164.308(a)(4)(ii)(C)',
    ...
    },
    {
      ...
      mapping: 'backup_schedules table with active=1 schedules; backups table records completed backups with SHA-256 integrity hash. POST /api/backups/trigger executes a manual backup. Required implementation specification under 164.308(a)(7).',
    },
    {
      id: '164.308(a)(7)(ii)(B)',
      name: 'Disaster Recovery Plan (Required)',
      check: checks.checkBackupMultiDestination,
      mapping: 'At least two distinct destinations across active backup_schedules (different destination values) prevents single-point-of-failure during disaster recovery. Required implementation specification.',
    },
    {
      id: '164.308(a)(7)(ii)(D)',
  ...
  customerResponsibility: [
    ...
    },
    {
      ...
      category: 'procedural',
      detail: 'Implement procedures for terminating access to ePHI when employment ends or when there is no further need. The GD supports account deactivation via users.active=0 soft delete (CISO-only operation), but the procedural trigger -- knowing when to deactivate -- is operator-side.',
    },
    {
      id: '164.308(a)(4)(i)',
    ...
    },
    {
      ...
      category: 'procedural',
      detail: 'Procedures for monitoring log-in attempts and reporting discrepancies. The GD logs all auth attempts (auth_log table); operator establishes the review cadence and escalation process for anomalies.',
    },
    {
      id: '164.308(a)(6)(i)',
    ...
    },
    {
      ...
      detail: 'Obtain satisfactory assurances that the business associate will appropriately safeguard ePHI. Maintain executed BAAs with every business associate (cloud providers, MSPs, software vendors that process ePHI on your behalf, including FireAlive itself if you treat its support engagement as a BA relationship).',
    },
    {
      id: '164.308(b)(3)',
  ...
  note: 'HIPAA is one of the most enforced US federal privacy regimes; OCR conducts random audits and follow-on investigations after breach reports. The 6-year documentation retention requirement is longer than the SOC 2 industry-standard 7-year retention; operators should provision audit_log storage and policy-document storage accordingly. Operators handling ePHI must additionally comply with the Privacy Rule (164.500-534), which is not enumerated in this framework definition because it is workflow-and-disclosure-focused rather than technical-control-focused. FireAlive\'s Global Dashboard sits at the governance layer above analyst-level data; the GD\'s HIPAA posture supports the covered entity\'s overall program but does not itself process ePHI directly.',
});
