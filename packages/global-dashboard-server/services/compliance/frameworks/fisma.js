...
module.exports = (checks) => ({
  ...
  verifiedControls: [
    ...
    },
    {
      ...
      mapping: 'The GD\'s secrets are encrypted at the application layer: every signing-key private key, the GD CA key and integration credentials are sealed with AES-256-GCM under a Tier-1 KEK that is itself hardware-sealed to the host\'s TPM 2.0 / Secure Enclave, so a copied disk or cloned VM cannot unseal them. General table data is not application-layer encrypted; that rests on filesystem-level protection at GD_DB_PATH (operator-managed disk encryption: LUKS / FileVault / BitLocker / AWS EBS). SC-28(1) is therefore satisfied cryptographically for secret material and at the OS/volume layer for the remainder; extending application-layer encryption to general table data under an external KMS the GD backup data key may be wrapped by an external key-management provider (AWS KMS, Azure Key Vault, GCP KMS or HashiCorp Vault) registered in gd_kms_providers, giving FIPS-validated custody, provider-side rotation and provider-side audit for the archive key. The Tier-1 KEK itself is escrowed to no provider: recovering a deployment means re-establishing it from the offline recovery code, so a provider is custody and never a recovery path.',
    },
