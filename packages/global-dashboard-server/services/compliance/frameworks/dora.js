...
module.exports = (checks) => ({
  ...
  verifiedControls: [
    ...
    },
    {
      ...
      mapping: 'The GD can wrap the backup data key through an external key-management provider registered in gd_kms_providers. The GD\'s secrets -- every signing-key private key, the GD CA key and integration credentials -- are AES-256-GCM sealed under a Tier-1 KEK hardware-sealed to the host TPM 2.0 / Secure Enclave, so a copied disk or cloned VM cannot unseal them. General table data is not application-layer encrypted: operator-managed disk encryption provides the at-rest guarantee for that remainder. An external provider gives FIPS-validated custody, provider-side rotation and provider-side audit for the archive key; the Tier-1 KEK itself is escrowed to no provider, so a provider is custody and never a recovery path. Hardware-backed key custody (AWS KMS / Azure Key Vault / GCP KMS / HashiCorp Vault); until then, hardware-backed cryptographic key management is operator-managed at the cloud/infrastructure layer.',
    },
    {
      id: 'Art.10',
