// -----------------------------------------------------------------------------
// FIREALIVE Global Dashboard -- Golden-Baseline Import Validation
// Copyright (C) 2026 Peter Mancina
// SPDX-License-Identifier: AGPL-3.0-or-later
// -----------------------------------------------------------------------------
//
// The import-safety gate for GD golden-baseline bundles, the GD twin of the
// Regional Server's golden-baseline-validate. In the import pipeline it runs
// AFTER the raw-bytes malware scan and the signature verification, and BEFORE the
// automatic pre-import snapshot and the apply. It is pure structural validation:
// no database access, no side effects. Every failure is collected and thrown as
// one typed BaselineValidationError that the route maps to a 4xx response.
//
// WHY THIS LAYER EXISTS (defense in depth)
//   applyBaseline already iterates the allowlist rather than the payload, so an
//   unknown key can never reach the database. This layer sits in front of it to
//   (a) reject a malformed, tampered, or oversized file at the door with a
//   precise, operator-readable reason BEFORE any snapshot or mutation, and
...
//     - format is the expected FA-GDGB1 marker
//     - baselineSchemaVersion is an integer this build supports (refuse on
//       mismatch); appVersion is present (drift is a warning, not a rejection)
//     - the payload, digest, signature, and signing-key block are structurally
//       present and well-shaped enough to verify
//   Payload (validateBaselinePayload):
//     - the two domain sections (configTable, tables) are present objects (the
//       GD has no team_config section)
//     - ONLY allowlisted keys appear -- unknown config-table keys, unknown
//       tables, and unknown columns are rejected, never silently dropped, so a
//       wrong or tampered file is visible
//     - key/value store values are strings; dedicated-table column values are
//       scalars (string, finite number, or null); booleans, objects, and arrays
//       are rejected (our exports never emit them and the database cannot bind
//       them)
//     - SECRETS ARE NEVER IMPORTABLE: a secret column appearing as a data key is
//       rejected outright; only the secretsPresent marker (an array naming known
//       secret columns) is allowed. This is the anti-smuggling control -- it
//       blocks injecting a credential or a base64/binary blob through the import
//       path. Apply re-supplies secrets from the existing row or lands the
//       capability disabled-pending-credentials.
//     - size and shape caps bound resource use: a canonical-payload byte cap, a
//       nesting-depth cap (also protects the canonicalizer from deep recursion),
//       per-string and per-array length caps, and a per-object key cap
//     - string values are screened for disallowed control characters and
//       script-shaped content
//
// The domain (allowlisted keys, table sections, secret columns) is imported from
// gd-golden-baseline.js so the validator can never drift from the capture and
// apply engine -- there is one source of truth. The GD domain uses only
// rows-mode sections.
// -----------------------------------------------------------------------------
...
const {
  ...
  CONFIG_TABLE_KEYS,
  TABLE_SECTIONS,
} = require('./gd-golden-baseline');

// -- Limits + constants --------------------------------------------------------

const LIMITS = {
  FILE_FORMAT: 'FA-GDGB1',
  ...
  MAX_ARRAY_LENGTH: 10000,        // per rows array
  MAX_KEYS_PER_OBJECT: 2000,      // per object
  MAX_SECRETS_MARKERS: 64,        // per secretsPresent array
};

// 64-char lowercase hex: both the payload digest and the signing-key fingerprint
// (SHA-256 over SPKI DER) share this shape.
const HEX64_RE = /^[0-9a-f]{64}$/;

// High-signal, near-zero-false-positive markers for script-shaped content in
// configuration strings. Legitimate configuration (URLs, emails, JSON blobs) does
// not contain these; an injected XSS / scriptlet payload does.
...
// Collects every problem found in one pass so the operator sees the full set of
// reasons a file was rejected, not just the first.
class Issues {
  constructor() {
    this.list = [];
  }
...
// configuration strings. Implemented by code-point scan rather than a regex so
// the source carries no escape sequences (house style).
...
// A bounded walk run before canonicalization and detailed validation. It fails
// fast on pathological input (excessive depth, element count, array or object
// width, string length) so the canonicalizer and the schema checks never process
// an abusive object. Returns false to signal the walk should stop entirely.
...
// A key/value store section (configTable): every key must be in the allowlist and
...
// A single record (a rows entry). Allowed keys are the section's data columns
// plus the secretsPresent marker. A secret column appearing as a DATA key is
// rejected -- the anti-smuggling control.
...
// The GD domain uses only rows-mode sections. A section carrying anything other
// than a rows array (a singleton row, a manifest entries list, or an unexpected
// mode) is therefore rejected here.
...
/**
 * Validate the FA-GDGB1 envelope structure and version compatibility. Returns a
 * normalized descriptor the route uses to verify the signature and then validate
 * the payload. Throws BaselineValidationError on any structural or version
 * problem. The appVersion drift (if any) is returned as a warning, not a
 * rejection.
 */
...
/**
 * Validate and bound-check the configuration payload against the GD
 * golden-baseline domain. Returns { payload, canonical, bytes, warnings }; the
...
function validateBaselinePayload(schemaVersion, payload) {
  ...
  for (const k of Object.keys(payload)) {
    if (k !== 'configTable' && k !== 'tables') {
      issues.add('payload.' + k, 'Unexpected top-level key.');
    }
  }

  for (const key of ['configTable', 'tables']) {
  ...
  issues.throwIfAny();
  return { payload: payload, canonical: canonical, bytes: bytes, warnings: [] };
}
