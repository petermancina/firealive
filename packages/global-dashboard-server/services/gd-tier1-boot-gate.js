...
// FIREALIVE (Global Dashboard) -- Tier-1 boot integrity gate (B-2)
//
// GD twin of server/services/tier1-boot-gate.js: same logic over the GD registry
// and the GD role table (gd_ha_node).
//
...
function skipsReplicated(db) {
  ...
  try {
    ...
    role = null; // no gd_ha_node table yet (fresh DB) -> standalone -> do not skip
  }
