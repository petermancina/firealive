// ═══════════════════════════════════════════════════════════════════════════════
// FIREALIVE GLOBAL DASHBOARD — Hardware Keystore (Linux TPM 2.0 backend)
// ═══════════════════════════════════════════════════════════════════════════════
//
// The GD Server own Linux TPM 2.0 keystore backend, a per-package copy of
// server/services/instance-anchor/hardware-keystore-linux.js (the GD backend
// shares no modules with server/). The TPM logic is identical; it uses
// GD-namespaced configuration -- FIREALIVE_GD_HW_KEYSTORE_DIR (default
// ~/.firealive/gd-hw-keystore) and FIREALIVE_GD_TPM2_TCTI -- so a GD Server
// co-located with a regional server on one host never shares storage or
// collides (keys are further isolated by the gd-inst- label namespace).
//
...
//     FIREALIVE_GD_HW_KEYSTORE_DIR (default ~/.firealive/gd-hw-keystore).
...
// FIREALIVE_GD_TPM2_TCTI), defaulting to the kernel resource manager device.
...
function defaultTcti() {
  if (process.env.FIREALIVE_GD_TPM2_TCTI) {
    return process.env.FIREALIVE_GD_TPM2_TCTI;
  }
...
function storeDir() {
  return process.env.FIREALIVE_GD_HW_KEYSTORE_DIR || path.join(os.homedir(), '.firealive', 'gd-hw-keystore');
}
...
module.exports = {
  ...
  // ask rather than reconstruct the FIREALIVE_GD_HW_KEYSTORE_DIR chain --
  // duplicated path logic is what let this server's migration composer and
  // importer point at different directories, and let its storage health probe
  // report on a directory the backup engine never wrote to. One resolver, one
  // answer.
  storeDir,
  createSigningKey,
  getSigningPublicKey,
