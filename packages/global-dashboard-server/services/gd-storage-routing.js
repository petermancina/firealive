// =============================================================================
// FIREALIVE GD -- Storage Routing Resolver
//
// One resolver every GD artifact-writer calls to learn where a given data type
// should be written. Backs the storage_destination_routes table (one row per
// data type) and resolves each route to its concrete, enabled
// storage_destinations entries. This is the GD trust realm's own routing map,
// twinning the Regional B5q resolver; it routes the GD's own artifacts.
//
// Primary + optional secondary destination per route. A route names a primary
// destination and, optionally, a distinct secondary; the writer pushes the
// same artifact to each. This is bounded (<= 2), admin-chosen, and per-type --
// not unbounded fan-out.
//
// Resolution:
//   getRouteForType(db, dataType) -> { destinations: [primary, secondary], ... }
//   - destinations is an ordered array of 0-2 usable destinations (primary
//     first, then the secondary when set, enabled, and distinct from the
//     primary). If the primary is missing/disabled but the secondary is usable,
//     the secondary stands in (destinations = [secondary]).
//   - 'snapshot' inherits the 'backup' route's pair when its own route has no
//     usable destination (the "Same as backups" behavior), with an elective
//     override: a snapshot route with its own enabled destination(s) wins.
//   - an unconfigured / disabled route, or one whose destinations are all
//     missing/disabled, resolves to { destinations: [] } so the writer keeps
//     its local behavior (no push) rather than failing.
//   - `destination` is also returned as a convenience alias for the primary
//     (destinations[0] || null).
//
// Config-save gate (writeRoute), fail-closed and applied to BOTH the primary
// and the secondary:
...
//     'object-lock' or 'append-only'). Any data type may additionally opt in
//     via options.immutability_required. The second copy must meet the same bar.
//   - The secondary must exist, be enabled (when the route is enabled), and
//     differ from the primary.
//   Typed failures carry a `which: 'primary' | 'secondary'` discriminator.
//
// The config-time data-residency gate (a blocked verdict under an enforce-mode
// category refuses the save) is layered onto validateRouteDestination when the
// GD data-residency service lands later in this phase.
//
// Schema: db-init.js -> storage_destination_routes. Destinations are read
// through services/gd-storage-destinations.js (public view, no credentials).
// =============================================================================
...
// DISABLED / immutability gates only when the route is enabled. `which` is
// 'primary' or 'secondary' and selects the offending field name and the
// discriminator on a failure.
 ...
 *
 * Upsert one route after the immutability gate (run for the primary and the
 * secondary). Returns
 *   { ok: true, route }
 * or a typed failure
 *   { ok: false, code, error, field?, which? }
 * with code one of:
 *   INVALID_DATA_TYPE | DESTINATION_REQUIRED | DESTINATION_NOT_FOUND |
 *   DESTINATION_DISABLED | IMMUTABILITY_REQUIRED | SECONDARY_SAME_AS_PRIMARY
 *
