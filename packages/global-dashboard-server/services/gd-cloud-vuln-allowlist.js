//
// FIREALIVE GD -- Cloud Vulnerability Scanner Allow-List  [B6e]
//
...
//
// THE GD HAD NO EXEMPTION AT ALL. Before B6e its limiter skipped exactly one
// path and nothing else (index.js:194):
//
//     skip: (req) => (...originalUrl...) === '/api/health'
//
// while the Regional Server's skipped three allow-lists (server/index.js:231).
// So a sanctioned GD cloud scan has been throttled as an attack since B1.
//
...
function isAuthorizedCloudScannerIp(ip) {
  ip = normalizeIp(ip || '');
  if (!ip) return false;
...
module.exports = { isAuthorizedCloudScannerIp, _refresh: refresh, _ttlMs: TTL_MS };
