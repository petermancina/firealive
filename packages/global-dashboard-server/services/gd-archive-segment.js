// =============================================================================
// FIREALIVE GD -- Sealed Archive-Segment Primitive
//
// The GD's shared gold-standard archival unit, used by the two net-new GD
// writers (audit-log archival and CEF-stream archival). One sealAndPush call
// turns a plaintext payload into a tamper- AND gap-evident, encrypted, pushed
// segment. Twins the Regional B5q archive-segment primitive for the GD trust
// realm; it seals under the GD KEK and pushes the GD's own artifacts.
//
//   1. resolve the destination route for the category (gd-storage-routing) -- a
//      primary plus an optional secondary, both written on every run
//   2. chain position: prev_hash = the prior segment's this_hash, sequence + 1
//   3. content_sha256 = SHA-256(plaintext); build the canonical signed manifest
//      (category, sequence, prev_hash, content_sha256, range) and the chain hash
//      this_hash = SHA-256(prev_hash || manifest)
//   4. sign this_hash with the dedicated archive-chain key (gd-archive-chain-keys)
//   5. FA-ENC1-encrypt the payload under the GD hardware-rooted KEK
...
//   6. stage the ciphertext in the durable pending dir, then push it to the
//      primary (which advances the chain) and, on every run, the secondary
//   7. record the segment (append-only) only after the primary push succeeds, so
...
//      retained artifact until it lands
//
...
//
// The config-time / per-attempt data-residency write-gate (fail-closed on the
// primary, re-checked per secondary attempt) is layered onto sealAndPush and
// runSecondaryAttempt when the GD data-residency service lands later in this
// phase.
//
...
const archiveChainKeys = require('./gd-archive-chain-keys');
...
// A sealed segment whose secondary copy has not yet landed is retained under
// this directory (one sub-directory per segment) so the retry sweep can re-push
// the same artifact; the sub-directory is removed once the secondary push
// succeeds. Bounded -- only un-replicated-secondary segments are held; the
...
function removePendingDir(dir, logger) {
  try {
    ...
    (logger || console).warn('gd-archive-segment: failed to remove pending dir (encrypted; non-sensitive)', {
...
async function pushSegmentToDestination(dest, ctx) {
  ...
  if (!adapter) {
    throw new Error(`gd-archive-segment: adapter '${dest.adapter}' not loaded in registry`);
  }
  const adapterContext = {
    artifactId: ctx.exportId,
 ...
 * and by retryPendingSegmentPushes. Mirrors the push engine attempt/backoff model
 ...
 * once attempts are exhausted) and the artifact is retained. Returns
...
async function runSecondaryAttempt(db, pushId, options = {}) {
  ...
  if (!row.source_artifact_path || !fs.existsSync(row.source_artifact_path)) {
    logger.error('gd-archive-segment: retained artifact missing; secondary copy cannot be re-pushed', {
...
async function sealAndPush(db, category, payloadBuf, rangeMeta = {}, options = {}) {
  ...
  if (!ARCHIVE_CATEGORIES.includes(category)) {
    throw new Error(
      `gd-archive-segment: unsupported category '${category}' (expected ${ARCHIVE_CATEGORIES.join(', ')})`
    );
  }
  if (!Buffer.isBuffer(payloadBuf)) {
    throw new Error('gd-archive-segment: payloadBuf must be a Buffer');
  }
  ...
  // 2. Chain position, content hash, manifest, signature.
  ...
  // 3. FA-ENC1-seal the payload. The exportId binds category + chain position
  // into the GCM AAD.
  ...
  // 4. Stage the sealed file in the durable pending dir (one sub-dir per
  ...
  if (!primaryDest) {
    throw new Error(`gd-archive-segment: primary destination '${primaryView.id}' vanished before push`);
  }
  ...
  // 5. Primary push advances the chain. A failure throws (no segment recorded);
  ...
  // 6. Record the segment (append-only) and the primary push row atomically, now
  ...
  // 7. Second copy. With no secondary configured the staged artifact is not
...
async function retryPendingSegmentPushes(db, options = {}) {
  ...
  for (const r of due) {
    try {
      ...
      failed += 1;
      logger.error('gd-archive-segment: retry sweep attempt crashed', { pushId: r.id, error: sweepErr.message });
    }
  }
