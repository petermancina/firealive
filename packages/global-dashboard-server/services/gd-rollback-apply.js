// ═══════════════════════════════════════════════════════════════════════════
// FIREALIVE — Offline rollback apply (Global Dashboard)  (B6k)
...
// ever invoked:
//
//   services/gd-key-op-authorization.js mintKoa refuses without an APPROVED
...
//      gd-db-restore-swap renames the restored bytes over the live database, and
...
//      gd-db-restore-swap, which refuses a foreign-KEK bundle BEFORE the swap.
//   5. MALWARE SCAN, FAIL-CLOSED. Inherited from gd-db-restore-swap: no scanner,
...
// database closes that window, and the pre-restore snapshot gd-db-restore-swap
...
const pidfile = require('./gd-pidfile');
const koaSvc = require('./gd-key-op-authorization');
...
async function applyRollback(opts) {
  if (!opts || typeof opts.openDb !== 'function') {
    throw new RollbackError('INVALID_INPUT', 'gd-rollback-apply: openDb required');
  }
  if (typeof opts.koaId !== 'string' || opts.koaId === '') {
    throw new RollbackError('INVALID_INPUT', 'gd-rollback-apply: koaId required');
  }
  ...
  if (held && held.alive) {
    throw new RollbackError(CODES.SERVER_RUNNING,
      'the Global Dashboard server is running (pid ' + held.pid + '). Stop it before rolling back: '
  ...
  // gd-db-restore-swap enforces the KEK match (a foreign-KEK bundle is refused
