// ═══════════════════════════════════════════════════════════════════════════════
// FIREALIVE GLOBAL DASHBOARD — Instance Anchor (hardware)
// ═══════════════════════════════════════════════════════════════════════════════
//
// The GD Server's own hardware-bound instance anchor (B5e, decision D26) -- the
// GD analogue of the regional server's hardware-anchor. Until now the GD Server
// was the anti-clone verifier (it tracks MC instance bindings and collisions in
// mc_instance_bindings / mc_instance_collisions) but held no anchor of its own;
// this gives it a verifiable hardware identity.
//
// The identity is an ECDSA P-256 signing key created in, and never leaving, the
// platform root of trust (TPM 2.0 on Linux/Windows, Secure Enclave on macOS)
// via the GD hardware-keystore seam (gd-hardware-keystore) -- so a copied GD
// disk cannot reconstitute the GD instance or sign as it. ECDSA P-256 is the
// one algorithm common to all three roots of trust (the Secure Enclave has no
// Ed25519). The public key and its SHA-256-over-SPKI-DER fingerprint are the
// verifiable identity.
//
// Identity rows live in gd_instance_identity; signing keys are labelled
// gd-inst-<uuid> -- GD-namespaced, so a GD Server co-located with a regional
// server shares no keystore key. anchor_seal carries only a non-secret marker
// (the backend kind and the key label); signatures are ECDSA P-256 raw r||s
// (IEEE P1363) over a SHA-256 digest, verified with crypto.verify('sha256',
// ..., { dsaEncoding: 'ieee-p1363' }); sealState envelopes data under a
// hardware-sealed AES-256-GCM key.
//
// Fail-closed (D26): with no hardware root of trust, establish refuses; there
// is no software fallback.

const crypto = require('crypto');
const keystore = require('./gd-hardware-keystore');
...
function load(options) {
  ...
  let row;
  try {
    row = db.prepare("SELECT * FROM gd_instance_identity ORDER BY id LIMIT 1").get();
  } catch (err) {
    return null;
  }
  if (!row) {
...
function establish(options) {
  ...
  if (!isAvailable()) {
    throw new Error('gd instance anchor: no hardware root of trust (TPM 2.0 / Secure Enclave) available; refusing to establish GD identity (fail-closed, D26)');
  }
  const instanceId = 'gd-inst-' + crypto.randomUUID();
  ...
  db.prepare(
    "INSERT INTO gd_instance_identity " +
  ...
  if (options.logger && typeof options.logger.info === 'function') {
    options.logger.info('GD instance identity established', {
  ...
  const row = db.prepare("SELECT * FROM gd_instance_identity WHERE instance_id = ?").get(instanceId);
  return descriptorFromRow(row, db);
}
...
function verify(options) {
  ...
  if (!identity) {
    return { valid: false, reason: 'no GD instance identity established' };
  }
  try {
...
function ratchet(options) {
  ...
  const identity = options.identity || load({ db: db });
  if (!identity) {
    throw new Error('ratchet: no GD instance identity established');
  }
  db.prepare("UPDATE gd_instance_identity SET ratchet_counter = ratchet_counter + 1, last_attested_at = datetime('now') WHERE instance_id = ?").run(identity.instanceId);
  const row = db.prepare("SELECT ratchet_counter FROM gd_instance_identity WHERE instance_id = ?").get(identity.instanceId);
