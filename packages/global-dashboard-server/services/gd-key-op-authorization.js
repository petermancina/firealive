...
// FIREALIVE (Global Dashboard) -- Key-Operation Authorization (KOA) token primitive (B6h B-3)
//
// GD twin of server/services/key-op-authorization.js: same mint/verify/consume over
// the GD anchor, gd-restore-approvals, and gd_instance_identity.
//
...
function anchorPublicPem(db) {
  const row = db.prepare('SELECT anchor_public FROM gd_instance_identity ORDER BY id LIMIT 1').get();
