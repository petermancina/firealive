// =============================================================================
// FIREALIVE GD -- Google Cloud Storage Destination Adapter
//
// Pushes artifacts (full-suite backups, snapshots, sealed audit-log / CEF
// segments, forensic-export bundles) to Google Cloud Storage using the
// @google-cloud/storage SDK. The SDK's File.save + createWriteStream handle
// resumable uploads automatically for large files (a full-suite backup
// archive can reach GB).
//
// Rounds out the major-cloud coverage alongside the S3 and Azure Blob
// adapters. EU privacy-first deployments use the S3 adapter with a custom
// endpoint pointed at Hetzner / OVHcloud / Scaleway.
//
// AUTH MODES (only two)
//
//   1. Service account JSON  { service_account_json: "..." | {...} }
//                              Accepts the SA JSON either as a string
//                              (parseable) or as an object. The adapter
//                              extracts client_email + private_key for the
//                              SDK's credentials option.
//
//   2. ADC                   null/empty -> Application Default Credentials.
//                              Tries: GOOGLE_APPLICATION_CREDENTIALS env var,
//                              GCE/GKE/Cloud Run metadata server,
//                              `gcloud auth application-default login`.
//                              Recommended for FireAlive-GD-on-GCP -- no GCP
//                              secrets in the FireAlive database.
//
// CONFIG SCHEMA
//
//   {
//     "project_id":   "firealive-gd-prod-12345",    (required)
...
//   }
//
// project_id: GCP project ID hosting the bucket (6-30 lowercase chars, etc.).
//
// bucket:     GCS bucket name. GCS naming rules: 3-63 chars (without periods;
//             longer with periods up to 222); lowercase letters, digits,
//             dashes, underscores; must start and end with letter or digit.
//             Cannot contain 'goog' or start with 'google'. We validate the
//             simpler form (3-63, no periods) which covers most operator use
//             cases. The SDK rejects malformed names at API time.
//
// prefix:     Optional path prefix inside the bucket (e.g. "production/").
//             Same shape as the S3 / Azure adapters.
//
// Note: NO region in config because GCS region is bucket-level (set at bucket
// creation by the operator), not a per-request parameter.
//
// OBJECT LAYOUT
//
...
//
// _complete.flag pattern matches the S3 and Azure Blob adapters for
// cross-cloud consistency.
//
// IMMUTABILITY VIA GCS OBJECT RETENTION
//
// supportedImmutabilityModes includes 'object-lock'. GCS Object Retention is
// the GCP analog to S3 Object Lock and Azure's immutable storage. When
// destination.immutability_mode = 'object-lock', the adapter sets a Locked
// per-object retention policy via the retention metadata
// ({ mode: 'Locked', retainUntilTime }).
//
// REQUIREMENT: the bucket must have Object Retention Lock enabled at bucket
// creation time. Without this, per-object retention cannot be set. Locked
// retention is permanent: once set, the retention period can only be EXTENDED,
// never shortened or removed -- the desired posture for ransomware-protected
// archives and the immutability-required audit-log / CEF segments.
//
// CHECKSUM VERIFICATION
//
// Every upload sets the md5Hash metadata; GCS verifies on receipt and rejects
// with 400 if the body doesn't match. (GCS also supports crc32c; we use md5
// because it's simpler to compute and adequate for transit verification --
// the actual cryptographic integrity is on the artifact's own AES-GCM tag and
// the manifest signature.)
//
// SDK LAZY-LOADED
//
// @google-cloud/storage is a GD server dependency. This module loads cleanly
// even if it is somehow absent (require is lazy inside _getSdk()); any push or
// probe call then throws DestinationAdapterError with a clear "npm install"
// instruction. The other adapters are unaffected.
// =============================================================================
...
async function push(artifactContext, options = {}) {
  ...
  try {
    ...
    if (exists) {
      logger.info(`gd-destination-adapter-gcs: artifact ${sourceDirName} already complete on gs://${config.bucket}/${objectPrefix} (skipping)`);
      return {
  ...
  try {
    for (const file of files) {
      ...
      logger.info(`gd-destination-adapter-gcs: uploaded ${file.name} (${fileSize} bytes) to gs://${config.bucket}/${objectName}`);
    }
    ...
    logger.info(`gd-destination-adapter-gcs: pushed ${sourceDirName} (${bytesPushed} bytes) to gs://${config.bucket}/${objectPrefix}`);
...
const adapter = {
  name: ADAPTER_NAME,
  description: 'Push artifacts to Google Cloud Storage via @google-cloud/storage. Supports service account JSON or Application Default Credentials. Object Lock equivalent via GCS Object Retention (Locked).',
