// ══════════════════════════════════════════════════════════════════════════════
// FIREALIVE GLOBAL DASHBOARD -- Per-Request Proof-of-Possession Verifier (Phase B5e)
//
// A GD session token is an 8-hour bearer JWT, so binding the device key only at
// login would still leave a stolen token replayable from any machine. Following
// the shape of DPoP (RFC 9449), every /api/ request must also carry a fresh,
// single-use proof that the caller still holds the bound hardware device key.
//
// The proof is a base64url(JSON) header carrying { iat, jti, sig }; the device key
// signs the request method, the request path, that iat and jti, and the bound
// key's RFC 7638 thumbprint (so a captured proof cannot be moved to another
// request, key, or token). This service is the policy-neutral verifier; the auth
// middleware looks up the bound key and calls verifyPopProof on every request.
// ══════════════════════════════════════════════════════════════════════════════

const gdDeviceKey = require('./gd-device-key');
...
// Domain separation: a per-request proof signature can never be a login proof
// (which signs a different versioned prefix) or vice versa.
const POP_SIGNING_PREFIX = 'firealive-gd-device-pop-v1:';
...
// freshness window closes, so the same proof cannot be replayed while still fresh.
// Sufficient for the GD's single-backend deployment.
...
// method, the request path, the proof's freshness (iat) and uniqueness (jti), and
// the bound key's thumbprint. The client reconstructs this from the same prefix
// and field order.
...
function verifyPopProof(args) {
  ...
  if (!gdDeviceKey.verifyDeviceKeySignature(publicKeyPem, message, sig)) {
