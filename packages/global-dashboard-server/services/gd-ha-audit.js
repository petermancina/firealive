...
// after returning.
//
// When the event has an actor, the actor is carried into the CEF message. A
// HA_MANUAL_FAILOVER at high severity that does not say who triggered it is a worse SOC
// signal than no event at all: the analyst sees an unplanned-looking takeover and cannot
// tell it from an operator drill. The audit row keeps the actor in its own column.
function streamHaEvent(db, eventType, detail, userId) {
  try {
    if (!isLiveChain(db)) {
    ...
    const alert = {
      ...
      message: userId ? (String(detail) + ' [actor=' + String(userId) + ']') : detail,
    };
...
// Record an HA event attributed to an operator. Append the audit row through the
// connection the caller is mutating, then stream. The append comes first and is
// independent: a SIEM that is down, misconfigured, or absent never costs the chain its
// record of what happened.
function auditHaEventBy(db, userId, eventType, detail, ip) {
  try {
    appendGdAuditEntry(db, { userId: userId || null, eventType: eventType, detail: detail, ip: ip || null });
  } catch (err) {
    try { console.error('gd-ha-audit append failed:', err && err.message ? err.message : err); } catch (logErr) { /* ignore */ }
  }
  streamHaEvent(db, eventType, detail, userId || null);
}

// Record a SYSTEM-originated HA event -- a promotion, a self-fence, a pairing, a rejected
// peer certificate. No actor exists, and saying so explicitly is the point: an operator
// action that reached this entry point by accident would silently lose its actor, and
// the SIEM would show an unattributed takeover. The HA control plane calls
// auditHaEventBy instead.
function auditHaEvent(db, eventType, detail, ip) {
  auditHaEventBy(db, null, eventType, detail, ip);
}
