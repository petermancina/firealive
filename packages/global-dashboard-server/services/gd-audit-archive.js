// =============================================================================
// FIREALIVE GD -- Audit-Log Archival Writer
//
// Continuously seals new audit_log rows into the audit_log segment chain via the
// shared GD archive-segment primitive, so the GD's audit trail is archived to
// the routed destination(s) -- FA-ENC1-encrypted, gap-evident, tamper-evident,
// and dual-written -- in addition to the live DB table. One scheduled call seals
// the next batch of un-archived rows. Twins the Regional B5q audit-archive
// writer for the GD trust realm.
//
// Self-describing cursor: the last archived audit_log id is MAX(range_end)
// across recorded audit_log segments (range_end holds the last id a segment
// covers). There is no separate cursor state to drift or corrupt, and the
// append-only segment table forbids deleting a segment, so the cursor cannot
// move backward. Each run reads audit_log rows with id > cursor in id order,
// canonical-serializes them, and calls sealAndPush(db, 'audit_log', payload,
// { rangeStart, rangeEnd }) -- which advances the chain, encrypts, pushes to the
// primary + secondary, and retries the secondary until it lands. If sealAndPush
// throws (a primary seal/push failure) the cursor is unchanged (no segment
// recorded), so the same rows are re-covered next run; the live audit_log row is
// never touched.
//
// The sealed record enumerates the GD audit_log canonical columns (id,
// timestamp, user_id, event_type, detail, ip, severity). The in-DB hash-chain
// columns are intentionally excluded: the sealed segment carries its own
// independent hash chain, so the audit content is what is archived.
// =============================================================================
...
// the GD audit_log canonical columns.
function fetchNewRows(db, cursor, maxRows) {
  return db.prepare(
    'SELECT id, timestamp, user_id, event_type, detail, ip, severity FROM audit_log ' +
...
async function archiveNewAuditEntries(db, options = {}) {
  ...
  logger.info('gd-audit-archive: sealed audit_log segment', {
