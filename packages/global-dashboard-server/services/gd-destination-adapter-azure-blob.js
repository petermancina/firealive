// =============================================================================
// FIREALIVE GD -- Azure Blob Storage Destination Adapter
//
// Pushes artifacts (full-suite backups, snapshots, sealed audit-log / CEF
// segments, forensic-export bundles) to Azure Blob Storage using
// @azure/storage-blob and @azure/identity for authentication.
// BlockBlobClient.uploadFile handles multipart uploads automatically for large
...
//                         Most common for storage accounts; account keys grant
//                         FULL access to the storage account so principle of
//                         least privilege favors SAS tokens or service
//                         principals.
//
//   2. SAS token         { sas_token: "?sv=2024-...&sig=..." }
//                         Scoped, time-limited. Operators issue container-
//                         scoped SAS tokens with write/read permissions only.
//                         Token expires; operators must rotate before expiry.
//
//   3. Service principal { tenant_id, client_id, client_secret }
//                         Azure AD authentication via ClientSecretCredential.
//                         Recommended for enterprise deployments with RBAC on
//                         the storage account.
//
//   4. Managed identity  null / empty -> DefaultAzureCredential
//                         For FireAlive-GD-on-Azure (App Service, AKS, VM)
//                         where the host has a managed identity with
//                         appropriate RBAC.
//
...
//
//   {
//     "account_name":     "firealivegdbackups",     (required;
//                                                   3-24 lowercase
//                                                   alphanumeric)
...
//
// _complete.flag pattern matches the S3 and GCS adapters for cross-cloud
// consistency. Restore code treats the absence of _complete.flag as
// "in-progress / aborted push" and skips.
//
// IMMUTABILITY VIA AZURE'S "IMMUTABLE STORAGE WITH VERSIONING"
//
// Azure's analog to S3 Object Lock. supportedImmutabilityModes includes
// 'object-lock'. When destination.immutability_mode='object-lock', the adapter
// sets a Locked time-based retention policy on each blob via
// setImmutabilityPolicy AFTER upload.
//
// Container-level vs per-blob: Azure supports both. We use per-blob
// (setImmutabilityPolicy on each blob) because (a) it works regardless of
// container-level policy, (b) it gives explicit verification, and (c) the same
// retention applies whether the container has a default policy or not. The
// container must have version-level immutability enabled to set per-blob
// policies -- this is the write-once posture the audit-log and CEF segments
// require.
//
// CHECKSUM VERIFICATION
//
// Azure Blob storage supports Content-MD5 transit verification (we compute MD5
// client-side, send via BlobHTTPHeaders, Azure verifies on receipt). MD5 is
// weak cryptographically but adequate for transit integrity (the actual
// cryptographic integrity is on the FireAlive manifest signature and the
// artifact's own AES-GCM authentication tag).
//
// SDK LAZY-LOADED
//
// @azure/storage-blob + @azure/identity are GD server dependencies. This
// module loads cleanly even if they are somehow absent (require is lazy inside
// _getSdks()); any push or probe call then throws DestinationAdapterError with
// a clear "npm install" instruction. The other adapters are unaffected.
// =============================================================================
...
async function push(artifactContext, options = {}) {
  ...
  // Idempotency check: if _complete.flag exists, this artifact was already pushed.
  try {
    const flagBlob = containerClient.getBlockBlobClient(completeFlagBlobName);
    const exists = await flagBlob.exists();
    if (exists) {
      logger.info(`gd-destination-adapter-azure-blob: artifact ${sourceDirName} already complete on https://${config.account_name}.blob.${config.endpoint_suffix || 'core.windows.net'}/${config.container_name}/${blobPrefix} (skipping)`);
      return {
  ...
  try {
    for (const file of files) {
      ...
      logger.info(`gd-destination-adapter-azure-blob: uploaded ${file.name} (${fileSize} bytes) to ${blobName}`);
    }
    ...
    logger.info(`gd-destination-adapter-azure-blob: pushed ${sourceDirName} (${bytesPushed} bytes) to ${config.container_name}/${blobPrefix}`);
...
const adapter = {
  name: ADAPTER_NAME,
  description: 'Push artifacts to Azure Blob Storage via @azure/storage-blob. Supports account key, SAS token, service principal, or managed identity. Sovereign clouds via endpoint_suffix. Object Lock equivalent via Azure immutable storage with versioning.',
