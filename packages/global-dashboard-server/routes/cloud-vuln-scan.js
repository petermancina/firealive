...
const { verifyMachineCert } = require('../services/gd-machine-cert-auth');
...
// ── List authorizations ──────────────────────────────────────────────────────
// ── B6e: scan policy ───────────────────────────────────────────────────────
// The master switch and permitted-scanner list for this surface. It had
// neither: O3 hardened credentials, not policy, so an operator could revoke
// authorizations one at a time but could not turn the surface off.
//
// Stored in the GD's own `config` table, which is (key, value) ONLY -- there is
// no updated_by column as team_config has on the Regional Server, so the actor
// is recorded in the audit trail instead.
...
// Writing policy carries the same step-up as minting a credential: silently
// widening the permitted-scanner list is how an intruder re-opens a surface an
// operator believes is closed.
router.put('/config', gdMfaStepUp(), (req, res) => {
...
router.post('/authorizations', gdMfaStepUp(), (req, res) => {
  ...
  if (!policy.allowedScanners.includes(scanner_type)) {
    ...
    return res.status(403).json({
      error: 'scanner_type is not permitted by the current scan policy',
  ...
  try {
    const log = (outcome, fields) => {
      ...
      try {
        ...
        logErr('access log write FAILED', { outcome, error: err.message });
        return null;
      }
    };
    ...
    const certCheck = verifyMachineCert(db, req, gdCa.SCANNER_CONSUMER_OU);
    if (!certCheck.ok) {
