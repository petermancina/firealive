...
const approvalPolicy = require('../services/gd-restore-approval-policy');
// R3l C66: chain walker/validator/replayer for incremental + differential restores
const restoreChainSvc = require('../services/gd-restore-chain');
const dbRestoreSwapSvc = require('../services/gd-db-restore-swap');
const { gdMfaStepUp } = require('../services/gd-mfa-stepup');

// Self-contained audit helper: opens its own db and appends to the GD tamper-
// evident audit chain, so the many auditLog(userId, eventType, detail, ip) call
// sites below read exactly as the Regional source. Best-effort; never throws.
function auditLog(userId, eventType, detail, ip) {
  let adb;
  try { adb = getDb(); appendGdAuditEntry(adb, { userId, eventType, detail, ip }); }
  catch (e) { console.error('gd-restore audit error:', e.message); }
...
function consumeApprovalGated(db, {
  ...
  if (preValidateResult.autoCreate) {
    ...
    try {
      ...
      console.error('disabled-mode auto-approval create failed', {
  ...
  try {
    ...
    console.error('approval consume unexpected error', {
...
router.get('/points', (req, res) => {
  try {
    ...
    const backups = db.prepare(`
      ...
      LIMIT 30
    `).all();
    const configSnapRows = db.prepare(
      "SELECT id, name, origin, created_at, created_by FROM config_snapshots ORDER BY created_at DESC LIMIT 30"
    ).all();
    db.close();

    const configSnapshots = configSnapRows.map(c => ({
      id: c.id,
      name: c.name,
      origin: c.origin,
    ...
    console.error('List restore points error', { error: err.message });
...
router.get('/preview/:id', (req, res) => {
  try {
    ...
    console.error('Preview restore error', { error: err.message });
...
router.post('/execute/:id', gdMfaStepUp(), async (req, res) => {
  ...
  try {
    ...
    if (backup.format_version === 1) {
      ...
      try {
        ...
        db.close();
        console.error('chain RESTORE_REQUEST append failed (v1); refusing restore', {
      ...
      console.warn('DATABASE RESTORED (v1)', {
    ...
    if (backup.format_version === 2) {
      ...
      try {
        ...
        db.close();
        console.error('chain RESTORE_REQUEST append failed; refusing restore', {
          backupId: backup.id,
      ...
      // 7-10. Restore via the GD db-restore-swap primitive: unwrap the DEK under
      //        the KEK, decrypt/decompress/untar, confirm the DB filename,
      //        mandatory layer-2 EDR scan (fail-closed on skipped/threat/
      //        inconclusive), pre-restore snapshot, and atomic-rename over
      //        DB_PATH. The handler db was closed above to release the file lock.
      const { DB_PATH } = require('../db-init');
      let swapResult;
      try {
        swapResult = await dbRestoreSwapSvc.restoreDatabaseFromArchive({
          archiveBytes,
          wrappedKeyBytes,
        ...
        const swapCode = swapErr && swapErr.code;
        if (swapCode === dbRestoreSwapSvc.CODES.SCANNER_NOT_CONFIGURED) {
          auditLog(userId, 'RESTORE_REJECTED_NO_SCANNER',
            `backup=${backup.id} format=v2 reason=no_scanner_configured`, req.ip);
          return res.status(422).json({
            error: 'restore requires at least one configured malware scanner. Configure one under CISO > Malware Scanners and retry.',
            code: 'SCANNER_NOT_CONFIGURED',
          });
        }
        if (swapCode === dbRestoreSwapSvc.CODES.MALWARE_DETECTED) {
          auditLog(userId, 'RESTORE_REJECTED_MALWARE',
            `backup=${backup.id} format=v2 detail=${swapErr.message}`, req.ip);
          return res.status(422).json({
            error: 'malware detected in extracted backup bytes',
            code: 'MALWARE_DETECTED',
            detail: swapErr.message,
          });
        }
        if (swapCode === dbRestoreSwapSvc.CODES.SCAN_FAILED) {
          auditLog(userId, 'RESTORE_REJECTED_SCAN_FAILED',
            `backup=${backup.id} format=v2 detail=${swapErr.message}`, req.ip);
          return res.status(500).json({
        ...
        return res.status(500).json({
          error: 'v2 restore failed',
          detail: swapErr.message,
          code: swapCode || null,
        });
      }
      const preRestorePath = swapResult.preRestorePath;
      const restoredSizeBytes = fs.statSync(DB_PATH).size;
      ...
      try {
        ...
        try {
          const result = chainSvc.appendChainEntry(newDb, {
            ...
            payload: {
              restore_completed_at: new Date().toISOString(),
              restored_db_size_bytes: restoredSizeBytes,
              pre_restore_filename: path.basename(preRestorePath),
              backup_signing_key_id: backup.signing_key_id,
              backup_signing_key_fingerprint: signingKeyFingerprint,
              backup_created_at: backup.created_at,
      ...
      console.warn('DATABASE RESTORED (v2)', {
        backupId: backup.id,
    ...
    console.error('Execute restore error', { error: err.message });
...
router.post('/execute-chain/:id', gdMfaStepUp(), async (req, res) => {
  const { confirmHash } = req.body;
  const userId = req.user && typeof req.user.id === 'string' ? req.user.id : null;
  if (!userId) {
  ...
  try {
    ...
    console.warn('DATABASE RESTORED (chain)', {
    ...
    console.error('execute-chain handler crashed', { error: err.message, stack: err.stack });
