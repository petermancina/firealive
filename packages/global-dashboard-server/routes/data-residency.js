...
// Fleet-residency: the residency posture of the managed management-console fleet.
// Each active console's jurisdiction is its declared country, else inferred from its
// region (cloud region -> country), else undeclared. An MC-to-GD flow is cross-border
// when the console's jurisdiction differs from the GD's declared primary residency.
// Record-and-surface only: this never blocks an MC push (a hard invariant).
function buildFleetResidency(db, declaredCountry) {
  const rows = db.prepare(
    "SELECT id, name, region, country, regulatory_framework, status FROM management_consoles "
    + "WHERE status = 'active' ORDER BY name"
  ).all();
  const consoles = rows.map(function (mc) {
    let country = mc.country ? regions.normalizeToken(mc.country) : null;
    if (!country && mc.region) {
      const hit = regions.regionToCountry(mc.region);
      country = hit ? hit.country : null;
    }
    const crossBorder = !!(country && declaredCountry && country !== declaredCountry);
    return {
      id: mc.id,
      name: mc.name,
      region: mc.region,
      country: country,
      regulatoryFramework: mc.regulatory_framework || 'none',
      crossBorder: crossBorder,
    };
  });
  const byCountry = {};
  consoles.forEach(function (c) {
    const k = c.country || 'undeclared';
    byCountry[k] = (byCountry[k] || 0) + 1;
  });
  return {
    declaredResidency: declaredCountry || null,
    total: consoles.length,
    crossBorder: consoles.filter(function (c) { return c.crossBorder; }).length,
    byCountry: byCountry,
    consoles: consoles,
    note: 'fleet residency is recorded and surfaced for review; MC-to-GD flows are never blocked',
  };
}
...
async function buildPosture(db) {
  ...
  return {
    ...
    fleet: buildFleetResidency(db, declaredCountry),
