...
export default function GlobalDashboard() {
  ...
  const [newSchedule, setNewSchedule] = useState({ name: "", frequency: "daily", time: "02:00", day_of_week: 0, day_of_month: 1, interval_minutes: 60, backup_kind: "full-suite", backup_strategy: "full", retention_days: 30, encrypted: true });
  const loadSchedules = async () => { setSchedulesLoading(true); setSchedulesError(null); const r = await api.get("/api/backup-schedules"); setSchedulesLoading(false); if (r.error) { setSchedulesError(r.error); } else { setSchedules(r.schedules || []); } };
  ...
  const navItems=[
    ...
    {id:"backup",label:"Backup & Restore"},{id:"restore",label:"Restore"},{id:"backup_schedules",label:"Backup Schedules"},{id:"data_sov",label:"Data Sovereignty"},{id:"recert",label:"Recertification"},
        ...
        <div style={{flex:1,padding:24,overflowY:"auto",animation:"fadeIn 0.3s ease"}}>
          ...
          {tab==="backup_schedules"&&(<div>
            ...
            <Card style={{marginBottom:16}}>
              ...
              <div style={{marginTop:8}}><Input label="Retention (days)" value={newSchedule.retention_days} onChange={e=>setNewSchedule(p=>({...p,retention_days:parseInt(e.target.value,10)||0}))} type="number" min={1}/></div>
              <label style={{display:"flex",alignItems:"center",gap:8,padding:"10px 0"}}><input type="checkbox" checked={!!newSchedule.encrypted} onChange={e=>setNewSchedule(p=>({...p,encrypted:e.target.checked}))}/><M style={{color:C.t}}>Encrypt backup (AES-256-GCM)</M></label>
              <Btn primary disabled={addBusy} onClick={async()=>{if(!newSchedule.name.trim()){setAddError("Name is required.");return;}setAddBusy(true);setAddError(null);const body={name:newSchedule.name.trim(),frequency:newSchedule.frequency,interval_minutes:newSchedule.interval_minutes,time:newSchedule.time,day_of_week:newSchedule.day_of_week,day_of_month:newSchedule.day_of_month,backup_kind:newSchedule.backup_kind,backup_strategy:newSchedule.backup_strategy,type:newSchedule.backup_strategy,destination:"local",retention_days:newSchedule.retention_days,encrypted:newSchedule.encrypted,active:true};const r=await api.post("/api/backup-schedules",body);setAddBusy(false);if(r.error){setAddError(r.error);}else{addA("SCHED_CREATED",newSchedule.name.trim());setSchedFb({success:"Schedule added."});setNewSchedule(p=>({...p,name:""}));loadSchedules();}}}>{addBusy?"Adding...":"+ Add Schedule"}</Btn>
            </Card>
          </div>)}
          {tab==="data_sov"&&(<div>
