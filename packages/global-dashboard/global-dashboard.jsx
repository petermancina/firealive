                ...
                {order.map(fw=><Card key={fw} style={{marginBottom:10}}>
                  ...
                  {grouped[fw].map((cell,i)=>{
                    ...
                    return <div key={cell.mc_id}>
                      <div onClick={()=>setSelectedCell(isOpen?null:{mc_id:cell.mc_id,framework:cell.framework})} style={{borderBottom:!isOpen&&i<grouped[fw].length-1?`1px solid ${C.b}`:"none",display:"flex",alignItems:"center",gap:10,cursor:"pointer",background:isOpen?"rgba(110,231,183,0.04)":"transparent",margin:isOpen?"0 -10px":"0",padding:isOpen?"8px 10px":"8px 0",borderRadius:isOpen?6:0}}>
                        <div style={{flex:1,minWidth:0}}>
                      ...
                      {isOpen&&<div style={{padding:"10px 10px 4px",margin:"0 -10px 8px",background:"rgba(0,0,0,0.15)",borderLeft:`2px solid ${C.a}`,borderRadius:"0 6px 6px 0"}}>
                        ...
                        {reportsList&&!reportsLoading&&reportsList.length===0&&<M style={{color:C.td,fontStyle:"italic",display:"block"}}>No full reports yet for this MC + framework. C9 will add a Request Full Report button that writes a mailbox row; the MC fulfills on its next compliance tick.</M>}
                        {reportsList&&!reportsLoading&&reportsList.length>0&&<div>
                          {reportsList.map((rep,j)=>{
                            ...
                            return <div key={rep.id} style={{padding:"6px 0",borderBottom:j<reportsList.length-1?`1px solid ${C.b}`:"none",display:"flex",alignItems:"center",gap:8}}>
                              <div style={{flex:1,minWidth:0}}>
