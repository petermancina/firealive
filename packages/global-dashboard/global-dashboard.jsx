        ...
        <div style={{flex:1,padding:24,overflowY:"auto",animation:"fadeIn 0.3s ease"}}>
          ...
          {tab==="compliance_xregion"&&(<div>
            ...
            {rollupData&&!rollupLoading&&rollupData.length>0&&(()=>{
              // Group flat rollup list into frameworks, then list MCs under each.
              // Server returns rows ordered by framework ASC, mc_id ASC so grouping
              // by walking the list preserves the natural order.
              const grouped = {};
              const order = [];
              ...
              return <>
                <Card style={{marginBottom:12,padding:"10px 14px"}}>
                  <M style={{color:C.tm}}>{rollupData.length} rollup cells across {order.length} framework{order.length===1?"":"s"}.</M>
                </Card>
                {order.map(fw=><Card key={fw} style={{marginBottom:10}}>
                  ...
                  {grouped[fw].map((cell,i)=>{
                    ...
                    return <div key={cell.mc_id} style={{padding:"8px 0",borderBottom:i<grouped[fw].length-1?`1px solid ${C.b}`:"none",display:"flex",alignItems:"center",gap:10}}>
                      <div style={{flex:1,minWidth:0}}>
