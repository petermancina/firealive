          ...
          {tab==="vuln_scan"&&(<div>
            ...
            <Card>
              ...
              {vsList.map(a=>{
                ...
                return(<div key={a.id} style={{padding:"12px 0",borderTop:`1px solid ${C.b}`,display:"flex",alignItems:"center",gap:12,opacity:a.enabled?1:0.55}}>
                  ...
                  <div style={{display:"flex",gap:6,flexShrink:0}}>
                    <Btn small onClick={async()=>{const r=await stepUp("/api/vuln-scan/authorizations/"+a.id,{enabled:!a.enabled},"put");if(r&&!r.error){showGdToast(a.enabled?"Scanner disabled":"Scanner enabled");reloadVulnScan();}else setCvError((r&&r.error)||"update failed");}}>{a.enabled?"Disable":"Enable"}</Btn>
                    ...
                    <Btn small style={{color:C.d,borderColor:C.d+"50"}} onClick={async()=>{if(!window.confirm("Revoke authorization \""+a.display_name+"\"? The scanner's token will stop working."))return;const r=await stepUp("/api/vuln-scan/authorizations/"+a.id,{},"del");if(r&&!r.error){showGdToast("Authorization revoked");reloadVulnScan();}else setCvError((r&&r.error)||"revoke failed");}}>Revoke</Btn>
                  </div>
                </div>);
              })}
            </Card>
            ...
            <Card>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:8}}>
                ...
                <div style={{display:"flex",gap:6,alignItems:"center"}}>
                  {cvChain&&<Badge color={cvChain.intact?C.a:C.d}>{cvChain.intact?("CHAIN OK ("+cvChain.count+")"):("CHAIN BROKEN @"+cvChain.brokenAt)}</Badge>}
                  <Btn small onClick={async()=>{const r=await api.get("/api/vuln-scan/access-log/verify");if(r&&!r.error)setCvChain(r);else setCvError((r&&r.error)||"verify failed");}}>Verify chain</Btn>
              ...
              <div onClick={e=>e.stopPropagation()} style={{background:C.bg,border:`1px solid ${C.b}`,borderRadius:14,padding:28,maxWidth:540,width:"90%",maxHeight:"85vh",overflowY:"auto"}}>
                ...
                <div style={{display:"flex",gap:8,marginTop:16,justifyContent:"flex-end"}}>
                  ...
                  <Btn primary onClick={async()=>{
                    ...
                    if(!vsForm.scanner_type){setCvError("Scanner is required");return;}
                    if(!vsForm.display_name.trim()){setCvError("Display name is required");return;}
                    if(cidrs.length===0){setCvError("At least one source IP / CIDR is required");return;}
                    if(vsForm.mode==="add"){
                      ...
                      if(r&&!r.error&&r.token){showGdToast("Scanner authorized");setVsForm(null);setCvError(null);setVsNewToken(r);reloadVulnScan();}
                      else setCvError((r&&r.error)||"create failed");
                    }else{
                      ...
                      if(r&&!r.error){showGdToast("Authorization updated");setVsForm(null);setCvError(null);reloadVulnScan();}
                      else setCvError((r&&r.error)||"save failed");
                    }
