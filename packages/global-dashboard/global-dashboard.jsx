...
export default function GlobalDashboard() {
  ...
  const [queryFilterRegex, setQueryFilterRegex] = useState("");
          ...
          {tab==="query"&&(<div>
            ...
            <Card style={{marginBottom:16}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:12}}>
                ...
                <Input label="Filter regex (optional)" value={queryFilterRegex} onChange={e=>setQueryFilterRegex(e.target.value)} placeholder="e.g., ^North.*" maxLength={256} disabled={configLocked||!queryTemplateId||!queryFilterColumn} style={{fontFamily:"'IBM Plex Mono',monospace"}}/>
              </div>
              <M style={{color:C.td,display:"block",fontSize:10,marginBottom:10,lineHeight:1.5}}>SQL is template-defined and parameterized. Regex compiles server-side and applies to results in JavaScript, never to SQL. Audit-logged with template id, parameters, and result count.</M>
              <div style={{display:"flex",gap:8}}>
                <Btn primary disabled={configLocked||!queryTemplateId||queryRunning} onClick={async()=>{
                  ...
                  if(queryFilterColumn&&queryFilterRegex){payload.filterColumn=queryFilterColumn;payload.filterRegex=queryFilterRegex;}
            ...
            {queryResults&&<div>
              <Card style={{marginBottom:12}}>
                ...
                <M style={{color:C.tm,fontSize:11,display:"block"}}>Returned {queryResults.resultCount} rows from {queryResults.regionCount} regions. Window: last {queryResults.daysBack} days. {queryResults.filterColumn?"Filter: "+queryResults.filterColumn+" /"+queryResults.filterRegex+"/":"No filter applied."}</M>
              </Card>
