...
async function main() {
  ...
  // A launch that never happened cannot support any assertion below. The first
  // version reported "ok: zero files created inside the application directory"
  // after a spawn error -- vacuously true, and a harness that claims an
  // assertion it never made is the disease it exists to catch.
  if (why === 'spawn-error') {
    fail(spec.label + ': the app never launched, so nothing below this was asserted. '
       + 'Fix the launch before reading any other line of this report.');
    report(spec);
    return;
  }
