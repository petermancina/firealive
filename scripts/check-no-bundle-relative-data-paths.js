...
//
// ── NO EXCEPTIONS: __dirname JOINS ONLY WITH LITERALS ────────────────────
//
...
// lies about its own coverage. Two files legitimately need to join __dirname
// with a variable: they iterate a literal array of code-relative .js paths to
// scan their own source (the HA self-scans).
//
// An earlier version suppressed the flag for those two files via a
// SOURCE_SCANNERS allow-list, and stated the gap that left: a future
// path.join(__dirname, stateVar) added to one of them would not be caught. That
// gap is now CLOSED. Source scanning goes through the sourceFile() chokepoint
// (server/lib/source-file.js and its GD twin), which resolves a code-relative
// source path and REFUSES any that escapes the server root or is not a .js
// file. So invariant B is absolute -- no file-level exception -- and the two
// properties are guarded twice over:
//
//   - a variable routed through sourceFile() is refused at runtime if it
//     escapes; and
//   - a variable joined to __dirname inline, bypassing sourceFile(), is caught
//     here, because nothing is suppressed.
//
// A sourceFile(__dirname, ...) call is itself not a join|resolve site, so the
// matcher does not flag the chokepoint it depends on. A defect must defeat both
// mechanisms.
// ═══════════════════════════════════════════════════════════════════════════
...
function sitesIn(src) {
  ...
  for (let k = 0; k < toks.length; k++) {
    ...
    // The callee is the id immediately before the '('. Only a path-building
    // call (join|resolve) is a bundle-relative-path site. This is what the
    // check has always meant -- earlier it matched on any '(' before __dirname,
    // which also flags sourceFile(__dirname, ...), the sanctioned chokepoint
    // that RESOLVES such paths rather than building one inline. The chokepoint
    // is not the defect; joining __dirname to a non-literal inline is.
    const callee = toks[k - 2];
    if (!callee || callee.type !== 'id' || (callee.value !== 'join' && callee.value !== 'resolve')) continue;
    const segs = [];
    let nonLiteral = null;
    let depth = 1;
    let j = k + 1;
  ...
  for (const root of SCAN_ROOTS) {
    for (const file of jsFiles(path.join(REPO, root), [])) {
      ...
      for (const s of sitesIn(src)) {
        ...
        if (s.nonLiteral) {
          // ABSOLUTE: __dirname may join only with literals. There is no
          // file-level exception. Reading the server's own source goes through
          // the sourceFile() chokepoint (server/lib/source-file.js and its GD
          // twin), which resolves such a path and refuses any that escapes the
          // server root -- and a sourceFile(__dirname, ...) call is not a
          // join|resolve site, so the matcher does not flag it. Anything still
          // joining __dirname to a variable inline is a defect.
          fail('B: ' + where + ' joins __dirname with the non-literal `' + s.nonLiteral
             ...
             + 'read lies about its own coverage. Use literal segments, or route it through '
             + 'sourceFile() if it reads the server\'s own source.');
          continue;
        }
...
function reconcile() {
  // Independent recount of the class the matcher visits: an id __dirname that is
  // the first argument to a join|resolve call, i.e. preceded by '(' whose callee
  // is join or resolve. Computed here directly from the token stream rather than
  // by calling sitesIn(), so a bug in sitesIn() is still caught by a mismatch.
  //
  // raw counts every __dirname token (any use); joinSites counts only the
  // path-building ones. The gap between them is the legitimate non-join uses --
  // the sourceFile(__dirname, ...) chokepoint calls and fromDir arguments -- and
  // is reported so a reader can see the matcher is not simply missing sites.
  let raw = 0;
  let inComments = 0;
  let joinSites = 0;
  for (const root of SCAN_ROOTS) {
    for (const file of jsFiles(path.join(REPO, root), [])) {
      ...
      const toks = tokenize(src);
      const seen = toks.filter((t) => t.type === 'id' && t.value === '__dirname').length;
      ...
      for (let k = 0; k < toks.length; k++) {
        ...
        if (prev && prev.type === 'punct' && prev.value === '(' &&
            callee && callee.type === 'id' && (callee.value === 'join' || callee.value === 'resolve')) {
          joinSites += 1;
        }
      }
    }
  }
  return { raw, inComments, joinSites };
}
...
function selfTest() {
  ...
  const cases = [
    ...
    // the chokepoint the absolute rule depends on must NOT be flagged: its callee
    // is sourceFile, not join|resolve, so it is not a path-building site.
    [F, "const abs = sourceFile(__dirname, __dirname, rel);", [], 'a sourceFile() call is not a join/resolve site'],
    [F, "const abs = sourceFile(pathMod.join(__dirname, '..'), __dirname, rel);", ['green'], 'the Regional chokepoint call: inner join has a literal, resolves, not a finding'],
  ];
...
if (process.argv.includes('--self-test')) {
  ...
  // The reconciliation: an independent recount of join|resolve sites must equal
  // what the matcher visited. Computed without calling sitesIn(), so a matcher
  // blind spot still shows as a mismatch.
  if (r.joinSites !== sites) {
    fail('RECONCILIATION: an independent recount finds ' + r.joinSites + ' __dirname join/resolve '
       + 'sites but the matcher visited ' + sites + '. The matcher has a blind spot -- the one '
       + 'failure a scanner cannot report about itself.');
  }
  ...
  console.log('check-no-bundle-relative-data-paths passed: ' + sites + ' __dirname sites across '
    ...
    + r.raw + ' raw __dirname, ' + r.inComments + ' in comments, ' + (r.raw - r.inComments - r.joinSites) + ' non-join uses).');
}
