...
**Version:** v1.0.94 | **License:** AGPL-3.0-or-later | **Author:** Peter Mancina  
**E-fuse counter:** 87 (anti-rollback) | **Build:** 20260810.1
...
**Download installers:** Pre-built installers for Mac (.dmg), Windows (.exe), and Linux (.AppImage) are available on the [Releases page](https://github.com/petermancina/firealive/releases/tag/v1.0.94) under Tags.
...
|Restore Points        |Pre-upgrade full-suite capture stored outside the data root; offline one-version rollback |
...
**v1 API (25 endpoints):** Signal recording, baseline retrieval, training recommendations, assessment CRUD, backup create/history/schedule/restore, pre-upgrade restore points, compliance scan, regression test, integration save/test/status, system health, client heartbeat, notifications, config lock, SLA, shift handoff, config snapshots.
