...
| `MFA_SESSION_HMAC_KEY` | 32+ bytes hex-encoded (≥64 hex chars). Keys HMAC-SHA256 of mfa_login_sessions tokens for defense-in-depth against database-only compromise. If unset, the key is HKDF-derived from `JWT_SECRET` with domain separation. | MFA session hash key isolation (recommended for production) |
...
The **`MFA_SESSION_HMAC_KEY`** is recommended for production deployments but not strictly required. mfa_login_sessions tokens are always HMAC-keyed — when the env var is unset, the key is HKDF-derived from `JWT_SECRET` with the domain-separation label `firealive:mfa-session-token-hmac:v1`. Setting `MFA_SESSION_HMAC_KEY` explicitly gives you independent secrets for JWT signing and MFA session hashing, so a leak of one doesn't cascade to the other. Generate a fresh key:

```bash
node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"
```

Then set `MFA_SESSION_HMAC_KEY=<output>` on the server (env var, systemd unit, container env, etc.). Rotation requires a process restart and invalidates active in-flight MFA challenges (5-minute TTL bound — affected users retry login).
