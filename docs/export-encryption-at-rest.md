...
**Global Dashboard (GD) server.** The GD carries its own key-wrapping-providers registry (`gd_kms_providers`). By default the data key is wrapped under the GD's own derived Tier-1 KEK via the GD configuration-encryption module (scheme `gd-tier1`), and the header `kek` envelope carries `{ v: 1, scheme: "gd-tier1", wrapped: <envelope string> }`. A CISO may instead register an external provider — AWS KMS, Azure Key Vault, GCP KMS or HashiCorp Vault — and the envelope then records that scheme and key reference instead. **Restore dispatches on what the manifest recorded, never on current configuration**, so an archive wrapped by a provider that has since been retired still opens. The GD KEK is independent of the MC KEK: a key from one server cannot open an artifact sealed by the other, and the two registries are separate — a provider configured on one is not visible to the other.

**A provider is custody, not recovery.** It wraps the per-backup data key and nothing else. The Tier-1 KEK is escrowed to no provider anywhere, because a key an IAM principal can unwrap over the network is a key a compromised deployment can unwrap over the network — which would reduce the anti-clone guarantee from what the TPM is worth to what the cloud IAM policy is worth. Recovering a deployment on replacement hardware still means re-establishing the KEK from the offline recovery code.
