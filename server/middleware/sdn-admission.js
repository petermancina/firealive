...
// FireAlive-managed permitted segments (e.g., the paired HA peer link), kept
// SEPARATE from the operator-declared permitted segments so a later operator
// network-map edit cannot silently drop them. Stored as { segment, label } in
...
// Register a FireAlive-managed permitted segment (e.g., the paired HA peer's
// endpoint) so the admission gate admits it alongside the operator's segments.
// Idempotent by segment string; refreshes the cache so the change is effective
// immediately rather than at the next TTL.
...
module.exports = { sdnAdmission, registerSystemSegment, _refreshSegments: refreshSegments, _ipMatchesEntry: ipMatchesEntry, _readSystemSegments: readSystemSegments, _cacheTtlMs: CACHE_TTL_MS };
