...
function serializeRequest(r) {
  ...
  // The CISO's signed decision token, present once the relay records it. The
  // reviewer's device verifies this against its OWN independently pinned CISO
  // public key before producing an export; the server only relays it and is not
  // the authority. Exposed only to the requesting reviewer (the route 404-masks
  // others), so this is not a new disclosure surface.
  if (r.approval_signature) {
    out.approval = {
      decision: r.approval_decision || null,
      payloadCanonical: r.approval_payload_canonical || null,
      signature: r.approval_signature || null,
      keyFingerprint: r.approval_key_fingerprint || null,
      nonce: r.approval_nonce || null,
    };
  }
  return out;
}
