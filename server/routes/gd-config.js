...
router.post('/test', async (req, res) => {
  ...
  // SSRF safety: resolve the hostname and reject if it lands on loopback,
  // "this network", or link-local addresses (including cloud metadata
  // services like AWS 169.254.169.254). Done immediately before fetch so
  // the validation reflects the same DNS state the request will use. Note
  // there is a small TOCTOU window between this check and the actual
  // socket connect — DNS rebinding attacks could theoretically exploit
  // it. For a single admin-triggered test from inside a corporate
  // network this is an acceptable tradeoff; deeper protection (e.g.
  // resolving once and binding to that IP for the connect) is left for a
  // future hardening pass if the threat model expands.
  let parsedUrl;
  try { parsedUrl = new URL(url); }
  catch (err) { return res.status(400).json({ error: 'Constructed test URL is invalid: ' + err.message }); }
