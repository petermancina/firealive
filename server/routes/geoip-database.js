...
router.post(
  ...
  async (req, res) => {
    ...
    // express.raw populates req.body with a Buffer only when the Content-Type
    // matched; reject anything else so the value is a known Buffer type from
    // here on. The instanceof test is a type barrier (resolves CWE-843).
    const body = req.body;
