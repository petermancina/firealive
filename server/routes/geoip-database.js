...
router.post(
  ...
  async (req, res) => {
    ...
    // CWE-843 (type confusion through parameter tampering): a tampered request
    // can make req.body an array or a string instead of the Buffer express.raw
    // yields. Reject those two shapes first with the array/typeof type tests,
    // then confirm a non-empty Buffer before treating the body as raw bytes.
    const body = req.body;
