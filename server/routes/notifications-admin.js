...
// ── Send a real test message to one configured external channel ────────
// Admin-only (the whole router is guarded by authMiddleware(['admin']) at the
// mount point). Delivers to the channel's SAVED destination only; distinct from
// the read-only integration-health probe, which sends nothing. Audit-logged.
router.post('/test-channel', async (req, res) => {
  const channel = (req.body && req.body.channel) || '';
  const allowed = ['email', 'webhook', 'pagerduty'];
  if (allowed.indexOf(channel) === -1) {
    return res.status(400).json({ error: 'channel must be one of: ' + allowed.join(', ') });
  }
  const db = getDb();
  try {
    const { sendTestToChannel } = require('../services/notifications-pipeline');
    const result = await sendTestToChannel(db, channel);
    auditLog(req.user?.id, 'NOTIFICATION_TEST_SEND', 'channel=' + channel + ' status=' + (result && result.status) + ' ok=' + !!(result && result.ok), req.ip);
    res.json({ success: !!(result && result.ok), channel: channel, status: (result && result.status) || 'error', detail: (result && result.detail) || null });
  } catch (err) {
    logger.error('Notification test-send error', { error: err.message });
