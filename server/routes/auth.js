...
/**
 * HMAC-SHA256 of an mfa_login_sessions token, keyed by a server-side
 * secret. The input is NOT a password -- it's 256 bits of cryptographic
 * entropy from crypto.randomBytes(32) generated by createMfaLoginSession
 * below. The plaintext only ever exists in memory long enough to be
 * returned to the client; the database stores only this HMAC.
 *
 * Why HMAC + server key (and not plain sha256, and not bcrypt/Argon2):
 *
 *   1. DEFENSE-IN-DEPTH AGAINST DB-ONLY COMPROMISE. With plain
 *      sha256(token), an attacker who steals the database can verify
 *      any guessed token (by hashing and comparing) and can validate
 *      tokens recovered from logs / memory / wire captures. With HMAC
 *      + server key, the leaked rows are inert without the key. The
 *      key lives in MFA_SESSION_HMAC_KEY env var (or is HKDF-derived
 *      from JWT_SECRET as a fallback for dev/test) and is never
 *      written to disk by the application.
 *
 *   2. NOT A PASSWORD-HASHING SCHEME. bcrypt / scrypt / Argon2 exist
 *      to slow brute force against low-entropy human-chosen inputs.
 *      A 256-bit cryptographically-random token is infeasible to
 *      brute force regardless of hash speed -- rainbow tables don't
 *      exist for that keyspace. Cost-stretched hashing here would
 *      add no security.
 *
 *   3. INDEXED LOOKUP REQUIREMENT. Per-call salting (bcrypt et al.)
 *      breaks WHERE token_hash = ? indexed lookups
 *      (idx_mfa_login_sessions_token_hash). That would force the
 *      login path to scan every active row and bcrypt.compare
 *      individually -- a DoS vector on an unauthenticated endpoint.
 *      HMAC is deterministic for a fixed key, so the index works.
 *
 *   4. SHORT TTL + SINGLE USE. Sessions expire after 5 minutes and
 *      consume_at marks them spent on first use. Even a leaked DB
 *      row + the server key has a bounded exploitation window.
 *
 * Production hardening note: set MFA_SESSION_HMAC_KEY explicitly
 * (32 random bytes hex-encoded, 64 hex chars). Without it, the
 * key is HKDF-derived from JWT_SECRET, which means a JWT_SECRET
 * leak is sufficient to compromise both layers; with explicit
 * keys, the two secrets are independent.
 */

const SESSION_HMAC_DOMAIN = 'firealive:mfa-session-token-hmac:v1';
let _sessionHmacKey = null;

function getSessionHmacKey() {
  if (_sessionHmacKey) return _sessionHmacKey;

  // Path 1: explicit env var. Must decode to >= 32 bytes.
  const explicit = process.env.MFA_SESSION_HMAC_KEY;
  if (explicit && explicit.length > 0) {
    let buf = null;
    try { buf = Buffer.from(explicit, 'hex'); } catch (_e) {}
    if (!buf || buf.length < 32 || buf.length * 2 !== explicit.length) {
      throw new Error('MFA_SESSION_HMAC_KEY must be hex-encoded with at least 32 bytes (64 hex chars)');
    }
    _sessionHmacKey = buf;
    return _sessionHmacKey;
  }

  // Path 2: HKDF-derive from JWT_SECRET with domain separation.
  // Mirrors the JWT_SECRET fallback pattern in middleware/auth.js
  // (insecure default for dev; loud about needing override for prod).
  const jwtSecret = process.env.JWT_SECRET || 'CHANGE_ME_INSECURE_DEFAULT';
  const derived = crypto.hkdfSync(
    'sha256',
    Buffer.from(jwtSecret, 'utf8'),
    Buffer.alloc(0),                              // salt (empty -- domain separation lives in info)
    Buffer.from(SESSION_HMAC_DOMAIN, 'utf8'),     // info: domain separation label
    32                                            // 32 bytes for HMAC-SHA256
  );
  _sessionHmacKey = Buffer.from(derived);
  return _sessionHmacKey;
}

function hashSessionToken(plaintext) {
  return crypto.createHmac('sha256', getSessionHmacKey()).update(plaintext).digest('hex');
}
