...
CREATE TABLE IF NOT EXISTS users (
  ...
  mfa_enrollment_required INTEGER NOT NULL DEFAULT 0
    CHECK (mfa_enrollment_required IN (0, 1)),  -- 1 = login refuses to issue JWT
                                                -- when totp_enrolled_at IS NULL.
                                                -- Set for admin/lead/developer
                                                -- roles at install / migration.
...
function initDb() {
  ...
  try {
    ...
    if (!usersCols.includes('mfa_enrollment_required')) {
      db.exec(`
        ALTER TABLE users
          ADD COLUMN mfa_enrollment_required INTEGER NOT NULL DEFAULT 0
          CHECK (mfa_enrollment_required IN (0, 1));
      `);
      // Set the flag for existing admin/lead/developer rows. Analysts
      // are not forced to enroll -- they don't perform admin operations
      // that warrant MFA gating, and forced enrollment for hundreds of
      // analysts on upgrade is an operational burden without security
      // payoff (the admin/lead roles are the privileged ones).
      const setResult = db.prepare(`
        UPDATE users
        SET mfa_enrollment_required = 1
        WHERE role IN ('admin', 'lead', 'developer')
      `).run();
      console.log(`R3f migration: added users.mfa_enrollment_required and set it for ${setResult.changes} admin/lead/developer row(s)`);
    }
