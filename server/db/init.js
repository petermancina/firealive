...
CREATE TABLE IF NOT EXISTS storage_destination_routes (
  ...
  secondary_destination_ref TEXT,          -- optional failover; soft ref to storage_destinations.id
  path_prefix TEXT,
...
function initDb() {
  ...
  // Migration (B5q v3): primary + secondary destinations. storage_destination_routes
  // gains an optional secondary (failover) destination; storage_archive_segments
  // gains the second copy's destination ref + path. PRAGMA-guarded so a fresh
  // database (which already has the columns from the CREATE above) no-ops, while a
  // database created under the single-destination schema (commit 20) is patched.
  // ADD COLUMN is DDL and is permitted by the storage_archive_segments append-only
  // triggers, which block only UPDATE and DELETE.
  try {
    ...
    if (!segCols.includes('secondary_destination_ref')) {
      db.exec('ALTER TABLE storage_archive_segments ADD COLUMN secondary_destination_ref TEXT');
    }
    if (!segCols.includes('secondary_dest_path')) {
      db.exec('ALTER TABLE storage_archive_segments ADD COLUMN secondary_dest_path TEXT');
    }
  } catch (b5qSecondaryErr) {
    console.error('B5q primary+secondary column migration failed:', b5qSecondaryErr.message);
  }
