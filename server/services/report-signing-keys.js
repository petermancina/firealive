 ...
 *
 * Parse and sanity-check a foreign deployment's report-signing PUBLIC key
 * before accepting it for registration. Enforces:
 *   - parses as a valid SPKI public key
 *   - is an Ed25519 key (matching what FireAlive's report signer uses)
 *   - is a public key (not a private-key paste)
 *
 * Returns { publicKeyPem, publicKeyFingerprint } on success, where
 * publicKeyPem is the canonicalized (re-exported) form. The fingerprint is
 * what the operator confirms out of band before registering, and what a
 * received baseline bundle is verified against.
 *
...
function validateExternalPublicKey(pem) {
  ...
  // Reject a private-key paste outright. crypto.createPublicKey() would
  // silently derive the public half from a private key, so without this an
  // operator could accidentally submit (and expose) their own private key in
  // the public-key field. Public keys cannot be loaded as private keys, so
  // this only ever rejects actual private-key input.
