// ═══════════════════════════════════════════════════════════════════════════════
// FIREALIVE — Compliance Framework: POPIA (South Africa)
//
// R3g (v1.0.33): coverage of South Africa\'s Protection of Personal
// Information Act 4 of 2013 under the Shared Responsibility schema.
// This is the final framework in the v1.0.33 compliance suite,
// bringing the FRAMEWORKS object to 16 entries.
//
// APPLICABILITY
//
...
//   and have adopted FireAlive in their SOC operations. The
...
//   §72 transborder information flows. The customer (the
//   Responsible Party) remains responsible for §8 accountability,
//   §11 consent management, §18 notification at collection, §20
//   Information Officer designation, §22 IR notification workflow,
//   §55-58 IO registration with the Information Regulator, and
//   §69-71 direct marketing controls.
//
...
//   enforcement.
//
//   verifiedControls map platform implementations to §19 security
//   safeguards (Condition 7), §21 operator obligations, §22
//   compromise notification, §23-24 data subject participation
  ...
  verifiedControls: [
    {
      ...
      mapping: 'AES-256-GCM at rest via TIER1/TIER3 keys (distinct); TLS 1.2+ at reverse proxy in transit. §19 (Condition 7) requires responsible parties to secure the integrity and confidentiality of personal information by taking appropriate, reasonable technical and organisational measures.',
    },
    {
      id: '§19 [Access]',
      name: 'Security Safeguards -- Access Control',
      check: checks.checkAccessControl,
      mapping: 'Role-based access control via users.role; route-level authMiddleware; scoped API keys. §19 access controls prevent loss of, damage to, or unauthorised destruction of personal information AND prevent unlawful access to or processing of personal information.',
    },
    {
      id: '§17',
      name: 'Information Quality',
      check: checks.checkAuditControls,
      mapping: 'auditMiddleware records every /api/ request including data-quality-relevant updates (USER_UPDATED, PERSONAL_INFO_CORRECTED). §17 (Condition 5) requires responsible parties to take reasonably practicable steps to ensure personal information is complete, accurate, not misleading, and updated where necessary.',
    },
    {
      id: '§21',
      name: 'Operator Obligations',
      check: checks.checkIntegrationHealth,
      mapping: 'integration_config tracks all third-party operator relationships; last_test_at evidences ongoing health monitoring. §21 requires processing by operators (POPIA equivalent of GDPR processor) only with knowledge or authorisation of responsible party, treated as confidential, and only as specified by responsible party.',
    },
    {
      id: '§22',
      name: 'Compromise Notification Timing',
      check: checks.checkNotificationTiming,
      mapping: 'sla_config tracks internal MTTA/MTTR; notification_config provides multi-channel alert delivery. §22 requires notification of compromise to Information Regulator AND data subjects "as soon as reasonably possible" after discovery of the compromise; delays only permitted for criminal investigation cooperation.',
    },
    {
      id: '§23',
    ...
    },
    {
      ...
      check: checks.checkDataSubjectRights,
      mapping: 'User-update endpoints permit correction per §24(1)(a); offboarding workflow (active=0 soft delete) supports deletion per §24(1)(b) where information is inaccurate, irrelevant, excessive, out of date, incomplete, misleading, or obtained unlawfully.',
    },
    {
      id: '§72',
      name: 'Transborder Information Flows',
      check: checks.checkCrossBorderTransferControls,
      mapping: 'cross_border_transfer_controls table records the bases for transborder transfers. §72 permits transborder flow only if: recipient subject to law/binding rules providing adequate protection AND including PoPI principles, OR data subject consents, OR transfer necessary for performance of contract, OR transfer for the benefit of the data subject and consent impractical, OR transfer for other limited purposes.',
    },
  ],
