// ═══════════════════════════════════════════════════════════════════════════════
// FIREALIVE — Compliance Framework: LGPD (Brazil)
//
// R3g (v1.0.33): coverage of Brazil\'s Lei Geral de Proteção de
// Dados (LGPD) -- Law No. 13.709/2018 -- under the Shared
...
//   Art.48 incident notification. The customer remains responsible
//   for Art.7-11 legal bases, Art.18 response workflow, Art.37
//   records of processing activities, Art.41 Encarregado (DPO)
//   designation, Art.48 ANPD notification process, and Art.52
...
//
//   verifiedControls map platform implementations to security
//   measures, data subject rights, prevention practices, and
//   incident-handling controls. customerResponsibility covers
  ...
  verifiedControls: [
    {
      ...
      mapping: 'AES-256-GCM at rest via TIER1/TIER3 keys (distinct); TLS 1.2+ at reverse proxy in transit. Art.6(VII) requires use of technical and administrative measures to protect personal data from unauthorized access and accidental or unlawful destruction, loss, alteration, communication, or dissemination.',
    },
    {
      id: 'Art.6(VIII)',
      name: 'Prevention Principle',
      check: checks.checkAuditControls,
      mapping: 'auditMiddleware records every /api/ request to audit_log; SIEM streaming (CEF format) externalizes events. Art.6(VIII) requires adopting measures to prevent harm to data subjects; comprehensive audit logging supports preventive monitoring.',
    },
    {
      id: 'Art.18(II)',
    ...
    },
    {
      ...
      check: checks.checkDataSubjectRights,
      mapping: 'User-update endpoints permit correction of incomplete, inaccurate, or outdated personal data per Art.18(III). Pseudonym rotation supports correction of behavioral data linkage.',
    },
    {
      id: 'Art.18(IV)',
    ...
    },
    {
      ...
      mapping: 'Role-based access control via users.role; route-level authMiddleware; scoped API keys. Art.46 requires controllers/operators to adopt security, technical, and administrative measures suitable to protect personal data.',
    },
    {
      id: 'Art.46 §1',
      name: 'Pseudonymisation',
      check: checks.checkPseudonymization,
      mapping: 'pseudonym_uuid + pseudonym_rotated_at sever identity linkage for analyst wellbeing/behavioral data. Art.46 §1 introduces pseudonymisation as an example of security measure that supports the security principle.',
    },
    {
      id: 'Art.47',
      name: 'Prevention Practices',
      check: checks.checkChangeManagement,
      mapping: 'Anti-rollback fuse_counter + Config Lock + audit-logged configuration changes evidence preventive practices that address risks to data subject rights. Art.47 requires processing agents to ensure personal data security throughout the processing lifecycle.',
    },
    {
      id: 'Art.48',
      name: 'Security Incident Notification',
      check: checks.checkNotificationTiming,
      mapping: 'sla_config tracks internal MTTA/MTTR; notification_config provides multi-channel alert delivery. Art.48 requires controllers to notify ANPD and affected data subjects of incidents that may pose relevant risk or damage. ANPD Resolution 15/2024 specifies the notification template and procedure.',
    },
  ],
