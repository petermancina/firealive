...
const REMEDIATIONS = {
  ...
  checkMfaEnforcement: {
    ...
    steps: [
      'Navigate to MC -> Admin -> User Management',
      'For each active user without TOTP enrolled, set mfa_enrollment_required = 1',
      'Affected users will be prompted to enroll TOTP on next login',
      'Bulk SOC-grade default: run the migration that sets mfa_enrollment_required = 1 for all users',
      'TOTP enrollment uses the standard Google Authenticator / Authy QR code flow',
    ],
    uiPath: 'mc:admin/users',
  },
  ...
  checkRoleSeparation: {
    ...
    steps: [
      ...
      'Unlock the configuration (admin + TOTP) then re-lock from a current admin account',
  ...
  checkBackupFrequency: {
    ...
    steps: [
      ...
      'Assign at least one destination',
  ...
  checkBackupMultiDestination: {
    summary: 'Add a second backup destination of a different adapter type',
    steps: [
      'Navigate to MC -> Admin -> Backup -> Destinations',
      'Click "Add Destination" and select an adapter different from your existing one',
      'Examples of redundant pairs: local + S3, S3 + GCS, SFTP + Azure Blob',
      'Single-destination configurations cannot survive a destination failure',
      'Configure both destinations with retention_days and run a manual backup to verify',
    ],
    uiPath: 'mc:admin/backup',
  },
  ...
  checkDrTestRecency: {
    ...
    steps: [
      ...
      'Provide a second admin\'s TOTP for second-person approval (if strict mode)',
  ...
  checkConfigLockState: {
    ...
    steps: [
      ...
      'Verify with TOTP MFA (admin role required)',
      'Subsequent configuration changes will require unlock + admin role + TOTP',
