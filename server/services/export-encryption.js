...
// via the shared backup-key-wrapping registry (env-var by default; the cloud-KMS
// schemes -- aws-kms, azure-keyvault, gcp-kms, hashicorp-vault -- are supported
// by the same path). The raw data key never touches disk.
//
...
//     iv:        "<base64, 12 bytes>",
//     tag:       "<base64, 16 bytes>",
//     kek:       <backup-key-wrapping envelope {v,scheme,ref,wrapped}> | null
//   }
//
...
const backupKeyWrapping = require('./backup-key-wrapping');
...
const DEFAULT_SCHEME = 'env-var';
const DEFAULT_KEK_REFERENCE = 'TIER1_ENCRYPTION_KEY';
...
async function sealArtifact(plaintext, opts) {
  ...
  assertRole(role);
  assertExportId(exportId);
  const scheme = o.scheme || DEFAULT_SCHEME;
  const kekReference = o.kekReference || DEFAULT_KEK_REFERENCE;

  const dataKey = crypto.randomBytes(KEY_LENGTH_BYTES);
  const envelopeBytes = await backupKeyWrapping.wrapKey(dataKey, {
    scheme: scheme,
    kekReference: kekReference,
    db: o.db,
  });
  const kek = backupKeyWrapping.parseEnvelope(envelopeBytes);
  const framed = buildFrame(plaintext, dataKey, { role: role, exportId: exportId, kek: kek });
  dataKey.fill(0);
  return { framed: framed, scheme: kek.scheme, kekRef: kek.ref };
}
...
async function openArtifact(framed, opts) {
  ...
  const envelopeBytes = backupKeyWrapping.serializeEnvelope(kek);
  const dataKey = await backupKeyWrapping.unwrapKey(envelopeBytes, kek.scheme, kek.ref, { db: o.db });
  try {
