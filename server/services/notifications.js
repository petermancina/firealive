...
function resolvePreference(db, userId, eventType) {
  ...
  // Returns booleans; downstream notify() uses these to decide which channels
  // to enqueue per notification.
...
function getEligibleRecipients(eventType, opts = {}) {
  ...
  try {
    ...
    for (const u of users) {
      ...
      // Eligibility = at least one of the 4 channels is on.
      if (pref.in_app || pref.email || pref.sms || pref.desktop) eligible.push(u.id);
    }
    return eligible;
  } finally {
    db.close();
