...
// ═══════════════════════════════════════════════════════════════════════════════
//
// Unified interface for internal AI inference. Features that route through
// the dispatcher read their per-feature configuration from the
// ai_provider_config table; the dispatcher runs each request on the internal
// local LLM. FireAlive uses internal AI only — there is no external-provider
// path.
//
// Usage:
//
...
//   // result.provider — the provider (always 'internal')
...
//   - 'AI_INTERNAL_UNAVAILABLE' — internal model not loaded
...
/**
 * Generate text via the internal model for the given feature.
 *
...
function readProviderConfig(featureId) {
  ...
  try {
    const row = db.prepare(`
      SELECT feature_id, provider, model_name, max_tokens, temperature, updated_at
 ...
 * Used by the Internal AI tab to show current per-feature settings.
 */
 ...
 * Used by the Internal AI tab to show recent activity and errors.
 */
...
// ── Internal Provider ─────────────────────────────────────────────────────────
// internal-llm.js is loaded lazily so the dispatcher can be imported even when
// the model service isn't present yet.
