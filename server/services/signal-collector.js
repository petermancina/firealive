...
class SignalCollector {
  ...
  _generateSignals(analyst) {
    ...
    // Cognitive load: derived from ticket count in the last hour.
    const ticketCount = this._getTicketCount(analyst.id);
    signals.push({ signal: 'cognitive_load', value: Math.min(100, ticketCount * 8) });
    // Task switching: count of different ticket categories handled in the last hour.
    ...
    // Shift overtime: scheduled hours over full-time + actual after-hours (cached).
