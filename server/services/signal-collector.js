...
//
// B5d1 (analyst-private data architecture): at collection time each analyst's
// reading snapshot is sealed to that analyst's own public key and stored in
// analyst_private_data, which the server can write but cannot read back. A
// de-identified copy of each signal value, tagged only by coarse operational
// grouping (SOC tier and shift), is written to analyst_metrics_deidentified
// for team aggregation; that table has no identity column by design. A
// metric-free routing cap (max_complexity) is derived from absolute-threshold
// strain. No per-analyst burnout score or risk tier is persisted server-side.
...
class SignalCollector {
  ...
  async collectAll() {
    ...
    for (const analyst of analysts) {
      ...
      // Plaintext signal path (server-side baseline and drift). Retired later
      // in this phase once on-device interpretation replaces it; kept here so
      // existing dashboards keep working across the sub-phase PRs.
      for (const sig of signals) {
        engine.recordSignal(analyst.id, sig.signal, sig.value);
      }

      // (a) Seal the reading snapshot to the analyst's own public key. The
      // server stores only ciphertext it cannot open. Skipped when the analyst
      // has not yet enrolled a key; that pre-cutover history is resealed at
      // enrollment by the migration in a later sub-phase.
      this._sealReading(analyst.id, signals);

      // (b) De-identified team/shift-tagged copy for aggregation. No identity
      // column exists on this table; tags are coarse operational groupings and
      // small groups are suppressed by k-anonymity downstream.
      this._writeDeidentified(analyst, signals);

      // (c) Metric-free routing cap from absolute-threshold strain.
      this._applyRoutingCap(analyst, signals);
  ...
  // Seal one snapshot (all six signals plus a timestamp) to the analyst's
  // active public key and store it as a private 'reading'. Never blocks the
  // collection run on a single analyst's seal failure.
