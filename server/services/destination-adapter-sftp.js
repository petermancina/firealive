// ═══════════════════════════════════════════════════════════════════════════════
// FIREALIVE — Destination Adapter: SFTP
//
// Pushes the four backup files (archive.tar.zst.enc, wrapped-key.bin,
// manifest.json, manifest.sig) to an SFTP server via the ssh2 library
// (already a dep from R3d foundation).
//
// SECURITY POSTURE
//
// HOST KEY PINNING IS REQUIRED, NOT OPTIONAL. The operator must
// provide config.host_key as an OpenSSH-format public key string
// (e.g. "ssh-ed25519 AAAAC3...comment" or
// "ssh-rsa AAAAB3...comment"). On every connection, the adapter
// verifies the SFTP server's presented host key matches the pinned
// key. Mismatch is a PERMANENT failure -- could be MITM. No TOFU
// mode, no warn-and-continue mode.
//
...
//
// Credentials are decrypted by the push orchestrator from the
// destination row's credentials_encrypted column (Tier-1
// AES-GCM). The adapter sees plaintext credentials and never
// persists or logs them.
//
// REMOTE LAYOUT MIRRORS SOURCE
//
// On host: <BACKUP_DIR>/firealive-backup-<ts>/
// On dest: <config.remote_path>/firealive-backup-<ts>/
//
// Same four files. Restoring would require pulling the directory
// back to a host's BACKUP_DIR and running the standard restore
// flow.
//
// ATOMICITY ON REMOTE
//
// Uploads all four files into `.firealive-backup-<ts>.tmp/` under
// remote_path, then sftp.rename() to the final name. POSIX rename
// of a directory is atomic on the same filesystem; the SFTP server
// enforces this. A crash mid-push leaves a hidden remote temp dir,
// never a partial visible backup directory.
//
// REMOTE-SIDE VERIFICATION (NOT DONE)
//
// The adapter does NOT re-download files to verify SHA-256 round
// trip. Trusting SSH transport integrity (TCP + SSH MAC layer).
// Alternatives considered and rejected:
//
//   - Re-download every file: doubles bandwidth, expensive
//   - Run remote `sha256sum` via SSH exec: requires shell access,
//     which SFTP-only servers (the hardened security posture) do
//     not offer
//   - Hash-after-write via SFTP: SFTP protocol does not support
//     server-side hashing as a standard operation
//
// R3d-4's S3 adapter will use HTTP-level checksums where the
// object-storage protocol natively supports it. The SFTP integrity
// gap is a known limitation; SOC operators wanting cryptographic
// verification of off-host pushes should prefer S3 destinations.
//
// CONNECTION LIFECYCLE
//
// Each push opens a new SSH/SFTP connection, transfers files,
// closes. No connection pooling. Pushes are infrequent (one per
// backup per destination); pooling complexity not justified.
//
// RETRYABLE TAXONOMY
//
...
//   - Disk full on remote (typically surfaces as a write error
//     code; included to be safe)
//   - Unknown errors (default retryable; scheduler eventually
//     escalates to permanent if persistent)
// ═══════════════════════════════════════════════════════════════════════════════
...
async function push(backupContext, options = {}) {
  ...
  try {
    ...
    remoteTempDir = base.safeJoinSegment(config.remote_path, '.' + sourceDirName + '.tmp');
  } catch (err) {
    throw new base.DestinationAdapterError(
  ...
  try {
    ...
    if (alreadyPresent) {
      logger.info(`destination-adapter-sftp: ${sourceDirName} already exists at destination; treating as idempotent success`, { destination: destination.id });
      let totalSize = 0;
    ...
    try {
      for (const f of files) {
        ...
        logger.info(`destination-adapter-sftp: uploaded ${f.name} (${f.sizeBytes} bytes)`);
      }
    ...
    logger.info(`destination-adapter-sftp: pushed ${sourceDirName} (${bytesPushed} bytes) to ${config.host}:${remoteFinalDir}`);
...
const adapter = {
  name: ADAPTER_NAME,
  description: 'Push backups to an SFTP server. Host key pinning is required (config.host_key).',
