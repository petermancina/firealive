// ─────────────────────────────────────────────────────────────────────────────
// FIREALIVE ── Instance Anchor (cross-platform hardware keystore seam)
...
//   linux  -> ./hardware-keystore-linux    (TPM 2.0; kernel resource manager / tpm2-tools)
//   win32  -> ./hardware-keystore-windows   (TPM 2.0; CNG Platform Crypto Provider)
//   darwin -> ./hardware-keystore-macos     (Secure Enclave; SecKey)
//
...
function backendModuleForPlatform(platform) {
  if (platform === 'linux') {
    return './hardware-keystore-linux';
  }
  if (platform === 'win32') {
    return './hardware-keystore-windows';
  }
  if (platform === 'darwin') {
    return './hardware-keystore-macos';
  }
  return null;
}
