// ─────────────────────────────────────────────────────────────────────────────
// FIREALIVE ── Instance Anchor (Windows TPM 2.0 / CNG keystore backend)
...
//
// The PowerShell executable is FIREALIVE_POWERSHELL (default "powershell").
...
const SIGN_PREFIX = 'FIREALIVE_SIGN_';
const SEAL_KEY_NAME = 'FIREALIVE_SEAL';
...
function realRunPowerShell(script) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'fa-ps-'));
