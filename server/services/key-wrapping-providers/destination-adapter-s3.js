// ═══════════════════════════════════════════════════════════════════════════════
// FIREALIVE — S3 Destination Adapter
//
// Pushes v2 backups to AWS S3 OR any S3-compatible object storage service.
// Uses @aws-sdk/client-s3 + @aws-sdk/lib-storage. The lib-storage Upload
// helper handles multipart uploads automatically for large files (the
// archive.tar.zst.enc, which can reach GB).
//
...
// backup pipeline.
//
// CONFIG SCHEMA (kms_providers.config JSON for s3 destination rows)
//
//   {
...
//   }
//
// Note: SSE is defense-in-depth on top of FireAlive's wrapped-key
// encryption. With env-var KEK + S3 SSE-KMS, you have two independent
// keys protecting the same data; rotate either without re-encrypting
// the other.
//
// CREDENTIALS SCHEMA
//
...
//
// _complete.flag is written ONLY after every other file has been
// confirmed uploaded. Restore code (R3d-5) treats the absence of
// _complete.flag as "in-progress / aborted push" and skips the
// directory. Operators can configure S3 lifecycle rules to expire
// directories that lack _complete.flag after N days.
//
// IMMUTABILITY VIA OBJECT LOCK
//
// supportedImmutabilityModes includes 'object-lock'. When the
// destination's immutability_mode = 'object-lock', the adapter sets
// ObjectLockMode + ObjectLockRetainUntilDate on every PutObject so
// writes are immutable until the retention period expires.
//
// Object Lock probe (commit 13 of this phase) verifies the bucket
// has Object Lock CONFIGURATION enabled before allowing pushes;
// destinations declared as 'object-lock' but pushing to a non-locked
// bucket are rejected at probe time.
//
// SDK NOT YET INSTALLED
//
// @aws-sdk/client-s3 + @aws-sdk/lib-storage are added to package.json
// in commit 23 of this phase. Until then, this module loads cleanly
// (require is lazy inside _getSdks()). Any push call before commit 23
// throws DestinationAdapterError with a clear "npm install"
// instruction. Existing local + sftp adapters continue working.
//
// CHECKSUM VERIFICATION
//
// PutObject calls include ChecksumSHA256 so S3 verifies on receipt.
// We compute the sha256 from disk before upload and compare against
// the response's ChecksumSHA256 (returned by S3 after successful
// receipt). Verifies end-to-end integrity at the HTTP level.
// ═══════════════════════════════════════════════════════════════════════════════
...
function validateConfig(config) {
  ...
  if (config.endpoint !== undefined && config.endpoint !== null) {
    ...
    try {
      ...
      // should use https. We log a warning at probe time (commit 13) but
      // don't block here.
    } catch {
...
function buildClient(config, credentials) {
  ...
  const clientConfig = {
    ...
    maxAttempts: 1,   // backup-push layer handles retry
  };
  if (config.endpoint) {
...
function buildObjectLockParams(immutabilityMode, retentionDays) {
  ...
  // (R3d-3 schema) but we accept it here only when in object-lock mode.
...
// Object Lock verification (commit 13) extends this with a HeadBucket
// + GetObjectLockConfiguration sequence.
...
async function push(backupContext, options = {}) {
  ...
  const credentials = destination.credentials;   // already decrypted by orchestrator
  ...
  // backup was already pushed -- treat as no-op success.
  try {
    await client.send(new s3.HeadObjectCommand({ Bucket: config.bucket, Key: completeFlagKey }));
    logger.info(`destination-adapter-s3: backup ${sourceDirName} already complete on s3://${config.bucket}/${keyPrefix} (skipping)`);
    try { client.destroy && client.destroy(); } catch { /* swallow */ }
    return {
      ...
      destinationMetadata: {
        backupId, sourceDirName, bucket: config.bucket, keyPrefix,
    ...
    // 404 is the expected case: backup not yet present, proceed with push.
  }
  ...
  try {
    for (const file of files) {
      ...
      logger.info(`destination-adapter-s3: uploaded ${file.name} (${fileSize} bytes) to s3://${config.bucket}/${objectKey}`);
    }
    ...
    logger.info(`destination-adapter-s3: pushed ${sourceDirName} (${bytesPushed} bytes) to s3://${config.bucket}/${keyPrefix}`);
...
const adapter = {
  name: ADAPTER_NAME,
  description: 'Push backups to AWS S3 or any S3-compatible service (Hetzner / OVHcloud / Scaleway / MinIO / Wasabi / B2 / R2 / DO Spaces). Endpoint config field enables EU privacy-first deployments.',
