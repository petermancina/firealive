...
const SCENARIOS = [
...

  {
    ...
    category: 'Identity & Authentication',
    title: 'MFA Bypass Attempt or TOTP Secret Theft',
    summary: 'An attacker attempts to authenticate without completing the MFA challenge, replays stolen TOTP seeds, or exploits MFA recovery flows to bypass second-factor authentication.',
    indicators: [
      'Authentication completion events without preceding MFA challenge events',
      'TOTP codes accepted from IPs different from the user\'s normal source',
      'MFA-recovery flow invocations not initiated by the legitimate user',
      'Multiple rapid TOTP attempts (could be brute force on a stolen seed)',
      'WebAuthn challenges completing from unexpected device fingerprints',
    ],
    quickRef: {
      trigger: 'Successful authentication recorded without corresponding MFA challenge, OR user reports unauthorized account access despite having MFA enabled.',
      firstActions: [
        'Revoke all sessions for the affected account.',
        'Reset MFA for the affected account: invalidate existing TOTP seed (or revoke WebAuthn registrations) and require re-enrollment.',
        'Pull auth_log entries showing MFA-related events (mfa_challenge_issued, mfa_challenge_completed) for the user. Confirm pairing — every successful auth should follow a successful challenge.',
        'If TOTP seed theft is suspected: rotate the user\'s authenticator app entry (delete old, generate new QR), and investigate where the seed was stored insecurely.',
        'Audit recent MFA-recovery flow uses for the account. If recovery was used and not initiated by the user, treat as compromised.',
      ],
      escalation: 'If multiple users show MFA bypass patterns, assume the MFA implementation itself may have a flaw or the underlying authenticator infrastructure is compromised. Engage AppSec.',
    },
    fullRunbook: {
      identification: [
      ...
      containment: [
        ...
        'Disable the user\'s account temporarily via Offboarding → Mark Inactive Pending Investigation.',
        'Invalidate the user\'s MFA enrollment: delete TOTP seed from the database, revoke WebAuthn registrations, force re-enrollment on next login.',
        'If the attack pattern suggests a flaw in FireAlive\'s MFA implementation rather than just credential theft: temporarily switch the org to enforce MFA at the IdP level instead of FireAlive\'s built-in MFA, until the implementation is reviewed.',
      ],
      eradication: [
        'Force MFA re-enrollment with a fresh seed/registration. The user generates a new TOTP via the MFA tab\'s wizard.',
        'Audit the user\'s authenticator device: if their phone was compromised (malware exporting TOTP seeds), the device needs cleanup before re-enrollment is meaningful.',
        'Review the MFA recovery flow audit trail: how is recovery initiated, who can approve it, what verification is required? Strengthen if a recovery flow was abused.',
      ],
      recovery: [
        'User completes fresh MFA enrollment. Verify the new seed is stored only in the user\'s authenticator app, not in any shared location.',
      ...
      postIncident: [
        'Document the bypass mechanism. If FireAlive\'s implementation had a flaw, file a security ticket and patch promptly.',
        'Review whether the MFA grace-login configuration was being abused. Tighten if necessary.',
