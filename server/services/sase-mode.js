// FIREALIVE -- SASE Mode configuration and posture (B5k SASE Mode)
//
...
//     field in the existing sase_config blob (team_config key 'sase_config'),
//     alongside the provider and ZTNA endpoint; it is topology, not a secret, and
//     has a single writer (the /api/sase/config route).
//
//   - The read-only POSTURE log -- an append-only record in sase_posture_events of
...
function getSaseConfig(db) {
  const row = db.prepare("SELECT value FROM team_config WHERE key = ?").get(SASE_CONFIG_KEY);
...
function recordPostureEvent(db, opts) {
  ...
  db.prepare('INSERT INTO sase_posture_events (event_type, severity, detail) VALUES (?, ?, ?)').run(
    o.eventType,
    severity,
    detail
  );
...
function getPosture(db, opts) {
  ...
  const lastRestore = db.prepare("SELECT observed_at FROM sase_posture_events WHERE event_type = 'posture_restored' ORDER BY observed_at DESC, rowid DESC LIMIT 1").get();
  // The three boundary-failure events; degraded while one exists after the restore.
  let breach;
  if (lastRestore) {
    breach = db.prepare("SELECT event_type, observed_at FROM sase_posture_events WHERE event_type IN ('direct_exposure_refused', 'passthrough_violation_refused', 'posture_degraded') AND observed_at > ? ORDER BY observed_at DESC, rowid DESC LIMIT 1").get(lastRestore.observed_at);
  } else {
    breach = db.prepare("SELECT event_type, observed_at FROM sase_posture_events WHERE event_type IN ('direct_exposure_refused', 'passthrough_violation_refused', 'posture_degraded') ORDER BY observed_at DESC, rowid DESC LIMIT 1").get();
  }
  const degraded = !!breach;
  const recent = db.prepare('SELECT event_type, severity, detail, observed_at FROM sase_posture_events ORDER BY observed_at DESC, rowid DESC LIMIT ?').all(limit);
  return {
    degraded: degraded,
