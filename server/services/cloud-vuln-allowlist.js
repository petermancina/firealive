// ═══════════════════════════════════════════════════════════════════════════════
// FIREALIVE — Cloud-Vuln Scanner IP Allow-List Cache (B1)
//
// Used by the API rate limiter's skip() so that a high-volume scan from an
// AUTHORIZED cloud scanner is not throttled as if it were an attack. An IP is
// exempt only when it falls inside the allow-list of an ENABLED authorization
// in cloud_vuln_scanner_authorizations (the same registry the scan-access
// recorder gates on). This is the ONLY defense relaxed for authorized scans:
//   - the token + IP gate on /api/cloud-vuln-access still applies,
//   - the append-only scan-access log still records every scan,
...
// routes/cloud-vuln-scan.js so exemption and access-gating agree.
...
function isAuthorizedScannerIp(ip) {
  ip = normalizeIp(ip || '');
  if (!ip) return false;
...
module.exports = { isAuthorizedScannerIp, _refresh: refresh, _ttlMs: TTL_MS };
