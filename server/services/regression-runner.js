...
class RegressionRunner {
  ...
  run() {
    ...
    check('schema', 'Foreign-key integrity', () => {
    ...
    check('auth', 'Auth flow round-trip (bcrypt + JWT + TOTP)', () => {
    ...
    check('integrations', 'GD-push signing round-trip (Ed25519 + fingerprint)', () => {
    ...
    // ── Category: Helper-pay ───────────────────────────────────────
    check('helper-pay', 'Points-ledger append + balance-cache invariant', () => {
    ...
    check('peer', 'Peer skill-share E2E envelope round-trip', () => {
    ...
    check('system', 'In-memory schema-clone harness', () => {
