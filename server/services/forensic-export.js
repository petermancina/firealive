 ...
 */
// ── B5q: push a completed forensic export to its routed destination ─────────
//
...
// confidential content). The export's files are staged under a unique
// exportId-named directory so the destination adapter mirrors them as a
// self-contained subdir. The local copy and decrypt-on-download path are
// unchanged; a push failure never fails the export.
//
// Returns one of:
//   { attempted: false, configured: false }                    no route
//   { attempted: true, pushed: false, blocked: true, reason }  residency refused
//   { attempted: true, pushed: false, reason }                 no adapter / no files
//   { attempted: true, pushed: true, destinationRef,
//     destinationPath, bytesPushed, immutabilityVerified }     pushed
async function pushForensicExport(db, ctx) {
  ...
  const residency = dataResidency.evaluateDestination(db, 'forensic_export', route.destination.id);
  ...
  if (!adapter) {
    return { attempted: true, pushed: false, reason: 'adapter \'' + dest.adapter + '\' not loaded in registry' };
  }
  ...
  try {
    ...
    const adapterContext = {
      ...
      sourceDir: sourceDir,
      files: files,
      manifestSha256: sliceSha256(ctx.manifestBytes),
    ...
    const result = await adapter.push(adapterContext, {
      logger: logger,
