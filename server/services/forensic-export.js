...
//
// Best-effort and additive. The forensic archive + manifest are already FA-ENC1
...
// confidential content). The export's files are staged once under a unique
// exportId-named directory and the same set is pushed to the primary and, when
// configured, the secondary (failover) destination (Revision v3). The local
// copy and decrypt-on-download path are unchanged; a push failure never fails
// the export.
//
// The primary is authoritative; the secondary is best-effort -- residency-gated
// (a blocked secondary is skipped) and never fatal (a failed secondary is
// logged and the outcome records the primary copy only).
//
// Returns one of:
//   { attempted: false, configured: false }                    no route
//   { attempted: true, pushed: false, blocked: true, reason }  primary residency refused
//   { attempted: true, pushed: false, reason }                 no adapter / no files
//   { attempted: true, pushed: true, destinationRef, destinationPath,
//     bytesPushed, immutabilityVerified, secondaryDestinationRef,
//     secondaryDestinationPath }                               pushed
async function pushForensicExport(db, ctx) {
  ...
  // Residency gate on the PRIMARY (fail closed). The secondary is gated
  // separately below and skipped if blocked.
  const residency = dataResidency.evaluateDestination(db, 'forensic_export', primaryView.id);
  ...
  if (!primaryDest) {
    return { attempted: true, pushed: false, reason: 'destination not found' };
  }
  const primaryAdapter = destinationBase.getAdapter(primaryDest.adapter);
  if (!primaryAdapter) {
    return { attempted: true, pushed: false, reason: 'adapter \'' + primaryDest.adapter + '\' not loaded in registry' };
  }

  const stagingParent = fs.mkdtempSync(path.join(os.tmpdir(), 'fa-forensic-'));
  const sourceDir = path.join(stagingParent, ctx.exportId);
  try {
    fs.mkdirSync(sourceDir, { recursive: true });
    const files = [];
    for (const src of ctx.files) {
    ...
    if (files.length === 0) {
      return { attempted: true, pushed: false, reason: 'no files to push' };
    }

    const pathPrefix = route.pathPrefix ? String(route.pathPrefix) : null;
    const manifestSha256 = sliceSha256(ctx.manifestBytes);
    const contextFor = (dest) => ({
    ...
    // Primary push (authoritative).
    const result = await primaryAdapter.push(contextFor(primaryDest), {
      logger: logger,
      timeoutMs: FORENSIC_PUSH_TIMEOUT_MS,
    });

    // Secondary push (best-effort failover): residency-gated and never fatal.
    let secondaryRef = null;
    let secondaryPath = null;
    if (secondaryView) {
      const secResidency = dataResidency.evaluateDestination(db, 'forensic_export', secondaryView.id);
      if (secResidency.blocked) {
        logger.warn('forensic export secondary destination blocked by residency; failover copy skipped', {
        ...
        try {
          ...
          const secondaryAdapter = secondaryDest ? destinationBase.getAdapter(secondaryDest.adapter) : null;
          if (!secondaryDest || !secondaryAdapter) {
            logger.warn('forensic export secondary destination unavailable; failover copy skipped', {
              exportId: ctx.exportId,
              destinationRef: secondaryView.id,
            });
          } else {
            const secondaryResult = await secondaryAdapter.push(contextFor(secondaryDest), {
              logger: logger,
              timeoutMs: FORENSIC_PUSH_TIMEOUT_MS,
            });
          ...
          logger.error('forensic export secondary push failed (non-fatal); pushed to the primary only', {
            exportId: ctx.exportId,
