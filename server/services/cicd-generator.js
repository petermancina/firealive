...
function renderGithubActions(snapshot, purpose) {
  return [
    ...
    '        run: docker buildx build --provenance=mode=max --sbom=true -t firealive:${{ github.sha }} .',
    '',
    '      - name: Install Cosign',
    '        uses: sigstore/cosign-installer@v3',
    '',
    '      - name: Sign image (keyless OIDC by default)',
    '        env:',
    '          COSIGN_EXPERIMENTAL: "1"',
    '        run: |',
    '          if [ "${COSIGN_KEY_MODE:-keyless}" = "key-based" ]; then',
    '            echo "$COSIGN_PRIVATE_KEY" > /tmp/cosign.key',
    '            cosign sign --yes --key /tmp/cosign.key firealive:${{ github.sha }}',
    '            rm /tmp/cosign.key',
    '          else',
...
function renderGitlabCi(snapshot, purpose) {
  return [
    ...
    '    - docker:24-dind',
    '  script:',
    '    - docker buildx build --provenance=mode=max --sbom=true -t firealive:$CI_COMMIT_SHA .',
    '',
    'sign:',
    '  stage: sign-verify',
    ...
    '      if [ "${COSIGN_KEY_MODE:-keyless}" = "key-based" ]; then',
    '        echo "$COSIGN_PRIVATE_KEY" > /tmp/cosign.key',
    '        cosign sign --yes --key /tmp/cosign.key firealive:$CI_COMMIT_SHA',
    '        rm /tmp/cosign.key',
    '      else',
    '        cosign sign --yes firealive:$CI_COMMIT_SHA',
    '      fi',
    '',
    'cve-scan:',
    '  stage: sign-verify',
...
function renderJenkinsfile(snapshot, purpose) {
  return [
    ...
    '        stage("Build (SLSA L3)") {',
    '            steps {',
    '                sh "docker buildx build --provenance=mode=max --sbom=true -t firealive:${BUILD_NUMBER} ."',
    '            }',
    '        }',
    '        stage("Sign (Cosign)") {',
    '            steps {',
    ...
    '                        echo "$COSIGN_PRIVATE_KEY" > /tmp/cosign.key',
    '                        cosign sign --yes --key /tmp/cosign.key firealive:${BUILD_NUMBER}',
    '                        rm /tmp/cosign.key',
    '                    else',
    '                        cosign sign --yes firealive:${BUILD_NUMBER}',
    '                    fi',
    '                \'\'\'',
    '            }',
    '        }',
...
function renderCircleCi(snapshot, purpose) {
  return [
    ...
    '            if [ "${COSIGN_KEY_MODE:-keyless}" = "key-based" ]; then',
    '              echo "$COSIGN_PRIVATE_KEY" > /tmp/cosign.key',
    '              cosign sign --yes --key /tmp/cosign.key firealive:$CIRCLE_SHA1',
    '              rm /tmp/cosign.key',
    '            else',
