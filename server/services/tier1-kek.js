...
// and install it for immediate use. Called from ha-failover.promote() before any epoch
// or role change, so a hardware or persistence failure aborts the promotion (fail-closed).
...
// Boot reload: if this node adopted a shared KEK (it was promoted), unseal it on this
...
// cache, so sharedKek() falls back to ownKek() (this node is standalone again). The caller
// (ha-pairing.unpair) MUST first confirm no replicated data still depends on the shared KEK
// -- shedding it while data is under it would strand that data (that case is refused at the
// unpair guard, pending Part B's offline rekey). Safe no-op when no shared KEK was adopted.
