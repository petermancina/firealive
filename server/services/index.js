...
// NEW in R3g commit 01: replaces the prior undefined `checkIntegrity`
// reference in HIPAA 164.312(c)(1). Verifies the audit log chain hash
// is intact end-to-end. R3g commit 06 (checks/audit.js) supersedes
// this with a fuller checkAuditIntegrity implementation that also
// verifies the chain hash registry; the version here is the minimum
// viable check that resolves the latent bug.
function checkAuditIntegrity(db) {
  const chainExists = db.prepare("SELECT COUNT(*) AS c FROM audit_log WHERE chain_hash IS NOT NULL").get();
  const total = db.prepare("SELECT COUNT(*) AS c FROM audit_log").get();
  if (total.c === 0) {
    return { status: 'pass', detail: 'No audit log entries yet; chain integrity vacuously holds.' };
  }
  const ratio = chainExists.c / total.c;
  return {
    status: ratio >= 0.99 ? 'pass' : (ratio >= 0.9 ? 'warning' : 'fail'),
    detail: `Audit log chain: ${chainExists.c} of ${total.c} entries chain-hashed (${(ratio * 100).toFixed(1)}%). SHA-256 prev-hash linkage.`,
  };
}
...
function generateComplianceReport(framework) {
  ...
  // Run all verified-control checks
