...
// system_meta.fuse_high_water (seeded by the B5e migration) and refuses to let
...
function readHighWater(db) {
  let row;
  try {
    row = db.prepare("SELECT value FROM system_meta WHERE key = 'fuse_high_water'").get();
  } catch (err) {
    return null;
  }
  if (!row) {
...
function persistHighWater(db, value) {
  db.prepare("INSERT INTO system_meta (key, value) VALUES ('fuse_high_water', ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value").run(String(value));
}
