...
function _connectOnly(host, port, useTls, timeoutMs = 4000) {
  return new Promise((resolve) => {
    ...
    if (useTls) sock = tls.connect({ host, port, rejectUnauthorized: false, timeout: timeoutMs }, onConnect);
    else sock = net.connect({ host, port, timeout: timeoutMs }, onConnect);
    sock.on('error', (e) => { try { sock.destroy(); } catch (_) {} finish({ ok: false, error: e.message }); });
...
async function siemProbe(db) {
  ...
  if (r.ok) return { ok: true, status: 'ok', detail: `${useTls ? 'TLS' : 'TCP'} connect ok to ${host}:${port}` };
...
module.exports = { registry, kmsProbe, storageProbe, ldapProbe, siemProbe };
