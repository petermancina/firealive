...
async function pushBackup(db, backupId, options = {}) {
  ...
  // secondary failover). Prefer options.destinationRefs; fall back to the
  // singular options.destinationRef for back-compat.
  let destinationRefs = [];
  ...
  // the secondary failover). One backup_pushes row per destination; the primary
  // outcome is authoritative and the secondary is best-effort redundancy. Each
  // push is independent -- a primary failure does not skip the secondary, so
  // both copies are attempted.
  const results = [];
