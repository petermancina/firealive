...
async function pushBackup(db, backupId, options = {}) {
  ...
  const destinationRef = options.destinationRef || null;
  if (!destinationRef) {
    logger.info(`backup-push: no destination configured; backup ${backupId} stays on-host only`);
    return { ok: true, destinations: [] };
  }

  const destination = storageDestinations.getDestinationWithCredentials(db, destinationRef);
  if (!destination) {
    logger.warn(`backup-push: resolved destination ${destinationRef} not found; backup ${backupId} stays on-host only`);
    return { ok: true, destinations: [] };
  }
  if (!destination.enabled) {
    logger.warn(`backup-push: resolved destination ${destinationRef} is disabled; backup ${backupId} stays on-host only`);
    return { ok: true, destinations: [] };
  }
  ...
  // Single push to the one resolved destination (no fan-out loop).
