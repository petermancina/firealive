 ...
 * notification_preferences, and lead_notification_contacts -- and, for an
 * analyst, crypto-shreds the key material via cryptoEraseAnalyst (recovery wraps
 * + analyst_private_data deleted, analyst_keys set to 'erased'), the only way to
 * render blobs already in immutable backups permanently unreadable. The users
 * row is KEPT but its identifiers are tombstoned (external_id -> NULL;
 * name/username -> a non-reversible 'erased-<id>' under the NOT NULL / UNIQUE
 * constraints), which preserves the audit_log foreign key while removing the
 * identity. The append-only audit_log is RETAINED (security-audit basis,
 * de-identified by the tombstoned link) and analyst_metrics_deidentified is left
 * untouched (it carries no identity). Throws an error with code SUBJECT_NOT_FOUND
 * when there is no such user. Returns a receipt; the route writes the
 * DATA_SUBJECT_ERASURE audit event.
 */
