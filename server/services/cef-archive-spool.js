// ═══════════════════════════════════════════════════════════════════════════════
// FIREALIVE -- CEF-Archive Spool (B5q)
//
// Durably archives the CEF stream FireAlive emits to a SIEM. Unlike the audit
// trail -- whose CEF is already persisted and hash-chained in audit_log
// (audit-chain.js) -- the alert/SIEM push feed (siem-adapter.js, used by
// siem-push.js and the websocket server) is fire-and-forget: it is sent over
// syslog and not retained anywhere. This spool gives that feed a durable,
// FA-ENC1-encrypted, gap-evident, tamper-evident, dual-written archive on the
// routed cef_archive destination(s), so there is a defensible record of exactly
// what was forwarded to the SIEM and when, independent of SIEM availability.
//
// Two halves:
...
//                        chain via archive-segment.sealAndPush (encrypt + push +
...
function appendLine(line, options = {}) {
  ...
  try {
    ...
    logger.error('cef-archive-spool: append failed (non-fatal)', { error: err.message });
