// ═══════════════════════════════════════════════════════════════════════════════
// FIREALIVE — Data-Residency Region Taxonomy
//
// Static, offline mapping from cloud-provider region codes to the ISO 3166-1
// alpha-2 country the region is physically in, plus the provider's legal
// domicile (the jurisdiction whose law can compel the provider, distinct from
// where the bytes sit -- residency is not sovereignty). Also defines legal
// blocs (EU / EEA / UK / US) so a residency policy may permit a bloc as
// shorthand for any member country.
//
// Used by services/data-residency.js to:
//   - resolve a backup destination's jurisdiction (regionToCountry),
...
//
// The provider tables are intentionally best-effort, NOT exhaustive: an
// unmapped region resolves to a null country and the operator declares the
// jurisdiction manually rather than the system guessing wrong. ALL of
// AWS / GCP / Azure are US-domiciled (US CLOUD Act reach), the same
// "whose law can compel" axis as the AI-model provenance policy.
//
// AGPL-3.0-or-later
// ═══════════════════════════════════════════════════════════════════════════════
...
// Provider region -> ISO 3166-1 alpha-2 country. domicile is the provider's
// legal home. AWS / GCP / Azure are all US-domiciled.
const PROVIDERS = {
  aws: {
    domicile: 'US',
    regions: {
...
// Legal blocs -> the ISO country set they expand to. A residency policy may
// list a bloc name (e.g. 'EU') as shorthand for any member country. 'US' is a
...
// Given an ISO country code, return it plus the blocs it belongs to (a bloc
// whose name equals the country, e.g. the US alias for US, is not listed).
...
// Best-effort region auto-suggestion from a backup destination's adapter config.
// Returns { region, country, domicile, provider } (any field may be null) or
// null when nothing can be inferred and the operator must declare. Reads config
...
//   - s3 without an endpoint: real AWS S3; resolve the region to a country and
//     the AWS US domicile where the region is mapped.
