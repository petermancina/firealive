...
async function performBackup(type = 'on-demand', options = {}) {
  ...
  // B5q (C12): resolve the single destination for this backup's type via the
  // storage-routing resolver. Backups go to the one destination an admin
  // designates for the 'backup' type (snapshots inherit the backup route
  // unless a 'snapshot' route is set), and nowhere else -- the legacy fan-out
  // to every enabled destination and the per-schedule destination_filter
  // (R3l C58/C59) are retired. If no route is configured the backup is still
  // created and chain-attested on-host; it simply is not pushed (the existing
  // graceful-degradation posture) until an admin configures the route.
  const backupCategory = type === 'snapshot' ? 'snapshot' : 'backup';
  let backupDestinationRef = null;
  try {
    const route = storageRouting.getRouteForType(db, backupCategory);
    if (route.configured && route.destination) {
      backupDestinationRef = route.destination.id;
    }
  } catch (routeErr) {
    ...
    if (options.awaitPush) {
      try {
        pushResult = await backupPushSvc.pushBackup(db, backupId, {
          logger,
          // B5q (C12): push to the single resolved destination for this type.
          destinationRef: backupDestinationRef,
        });
      } catch (pushErr) {
      ...
      void (async () => {
        ...
        try {
          ...
          const r = await backupPushSvc.pushBackup(pushDb, backupId, {
            logger,
            // B5q (C12): same single resolved destination on background dispatch
            destinationRef: backupDestinationRef,
          });
