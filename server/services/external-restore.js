...
//  14.  Pre-restore snapshot of CURRENT live DB
//  15.  Atomic-rename extracted bytes over DB_PATH
//  16.  Open a FRESH db connection against the now-restored DB_PATH
//  17.  Append RESTORE_COMPLETE to the chain in the restored DB
//  18.  Update source last_used_at on the restored DB
//  19.  Close the fresh db connection
//
// Steps 8 + 9 are FATAL: if either fails, abort before destructive
// work. Steps 17 + 18 are logged-non-fatal: if they fail, the
// restore itself succeeded; only audit-trail bookkeeping is missing.
//
// AGPL-3.0-or-later
// ═══════════════════════════════════════════════════════════════════════════════
...
async function executeRestore(db, args) {
  ...
  // 16-19. Open a fresh connection against the now-restored DB and
  ...
  try {
    // 17. RESTORE_COMPLETE chain entry on the restored DB. Logged-
    ...
    // 18. Update source last_used_at on the restored DB. Same
    ...
    // 19. Always close the fresh connection. The original db was
