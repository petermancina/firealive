// FIREALIVE -- HA lease / epoch authority (B5o)
//
// The lease/epoch is the INTERNAL failover authority. A monotonic epoch lives in
// ha_lease (id = 'current'); the holder of the CURRENT epoch is the sole writer.
// The org's external load balancer routes traffic, but only the epoch holder may
// write, so a write misrouted to the wrong node is refused -- never split-brain,
// never divergence.
//
...
//              BEFORE-UPDATE monotonic trigger on ha_lease permits the increase
//              and aborts any decrease, so a recovered old active that re-claims a
//              lower epoch cannot win.
//
...
const DEFAULT_LEASE_TTL_SEC = 30; // matches ha-pairing.finalizeRole's default

class HAWriteRefused extends Error {
  constructor(message) {
    super(message || 'ha-lease: write refused -- this node is not the active lease holder');
...
function ensureLeaseRow(db) {
  ...
  db.prepare("INSERT OR IGNORE INTO ha_lease (id, epoch, holder) VALUES ('current', 0, 'none')").run();
}

function getLease(db) {
  ensureLeaseRow(db);
  return db.prepare(
    "SELECT epoch, holder, lease_expires_at, last_heartbeat_at, term_started_at FROM ha_lease WHERE id = 'current'"
  ).get();
}

function currentEpoch(db) {
  ensureLeaseRow(db);
  const row = db.prepare("SELECT epoch FROM ha_lease WHERE id = 'current'").get();
...
function renewLease(db, ttlSec) {
  ...
  const info = db.prepare(
    "UPDATE ha_lease SET holder = 'self', lease_expires_at = ?, last_heartbeat_at = datetime('now'), updated_at = datetime('now') WHERE id = 'current' AND holder = 'self'"
  ).run(expires);
...
function recordPeerHeartbeat(db, peerEpoch, peerLeaseExpiry) {
  ...
  db.prepare(
    "UPDATE ha_lease SET epoch = ?, holder = 'peer', lease_expires_at = ?, last_heartbeat_at = datetime('now'), updated_at = datetime('now') WHERE id = 'current'"
...
function claimNextEpoch(db, ttlSec) {
  ...
  const expires = new Date(Date.now() + ttl * 1000).toISOString();
  db.prepare(
    "UPDATE ha_lease SET epoch = epoch + 1, holder = 'self', term_started_at = datetime('now'), lease_expires_at = ?, last_heartbeat_at = datetime('now'), updated_at = datetime('now') WHERE id = 'current'"
  ).run(expires);
