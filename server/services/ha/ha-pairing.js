...
function buildIdentityBundle(db, selfEndpoint) {
  ...
  return {
    ...
    certThumbprint: haPeerLink.localCertThumbprint(),
...
function verifyPeerBundle(bundle) {
  ...
  return haKeys.verifyPeerWrapKey(bundle.wrapPublicPem, bundle.wrapPubkeyAnchorSig, bundle.anchorPublicPem);
}
...
function pinPeer(db, bundle, certThumbprint, status) {
  ...
  db.prepare(
    "INSERT INTO ha_peer (peer_endpoint, peer_anchor_fingerprint, peer_anchor_public_pem, peer_wrap_public_pem, peer_cert_fingerprint, status, paired_at) "
    + "VALUES (?, ?, ?, ?, ?, ?, ?)"
  ).run(
    bundle.endpoint || '',
...
function pairingRequest(endpoint, body, opts) {
  return new Promise(function (resolve, reject) {
    ...
    const req = https.request(reqOpts, function (res) {
      ...
      res.on('end', function () {
        ...
        try { resolve({ json: text ? JSON.parse(text) : {}, peerCertThumbprint: peerThumb }); }
    ...
    req.on('socket', function (socket) {
      socket.on('secureConnect', function () {
        ...
        peerThumb = cert ? haPeerLink.normalizeThumb(cert.fingerprint256) : null;
        req.write(payload);
        req.end();
      });
    });
...
async function beginPairing(db, peerEndpoint, token, opts) {
  const selfEndpoint = (opts && opts.selfEndpoint) || null;
  const myBundle = buildIdentityBundle(db, selfEndpoint);
  const init = await pairingRequest(peerEndpoint, { token: token, bundle: myBundle }, opts);
  ...
  if (peerBundle.certThumbprint && init.peerCertThumbprint && peerBundle.certThumbprint.toLowerCase() !== init.peerCertThumbprint) {
    throw new Error('ha-pairing: peer certificate thumbprint mismatch');
  }
  if (!peerBundle.endpoint) {
    peerBundle.endpoint = peerEndpoint;
  }
  pinPeer(db, peerBundle, init.peerCertThumbprint, 'pairing');
