// FIREALIVE -- HA per-mode tailoring (B5o)
//
// HA composes with the deployment mode without weakening that mode's trust
// boundary. This module is the single seam the pairing flow consults so the
// mode-specific rules live in one place:
//
...
//       re-verified from its kernel (cloud-attestation.verifyAttestation) and the
//       peer's asserted report is verified against the bundled roots
//       (cloud-attestation.verifyPeerAttestation). A non-confidential local node,
//       or a peer that cannot prove a current confidential guest (a spot /
...
//
// Pure orchestration over the injected db handle; the attestation and
// segment-admission mechanics live in cloud-attestation and sdn-admission. ASCII
// only; no template literals. Require depths from server/services/ha/: ../ for
// services/ siblings, ../../ for server/ (middleware).

const mode = require('../deployment-mode');
const cloudAttestation = require('../cloud-attestation');
const sdnAdmission = require('../../middleware/sdn-admission');
...
function assertModePairingAllowed(db, peerAssertedAttestation) {
  ...
  if (!localR || localR.verified !== true) {
    ...
    throw new Error('ha-modes: local node is not a verified confidential VM in Cloud Mode; cannot pair (' + reason + ')');
  }
  ...
  if (!peerR || peerR.verified !== true) {
    ...
    throw new Error('ha-modes: HA peer is not an attested confidential VM; refusing to pair (' + reason + ')');
  }
...
function registerHaSegments(db) {
  ...
  const peer = db.prepare("SELECT peer_endpoint FROM ha_peer WHERE status = 'paired' LIMIT 1").get();
