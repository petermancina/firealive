...
//
// Trust is by CERTIFICATE THUMBPRINT PINNING, not CA-chain validation. Each node
// presents its TLS server certificate -- the same one its HTTPS listener serves,
// persisted at <data>/server-tls.crt|key -- as its mTLS client certificate when
// it connects to the peer. At pairing each node records the peer's exact
// certificate thumbprint in ha_peer.peer_cert_fingerprint. Because a node serves
// and presents the SAME certificate, one pinned fingerprint covers both
// directions. The listener already runs requestCert:true, rejectUnauthorized:
// false, so the chain is not validated -- the pin is the trust decision.
//
//   Outbound (sendToPeer): opens a fresh TLS connection (agent:false), verifies
//   the peer's server-cert thumbprint on secureConnect BEFORE writing the body
//   (a wrong certificate never receives replication data), then POSTs framed
//   JSON and returns the parsed response.
...
function getPeer(db) {
  return db.prepare("SELECT peer_endpoint, peer_cert_fingerprint, status FROM ha_peer WHERE status = 'paired' LIMIT 1").get();
}
...
function sendToPeer(db, pathSuffix, payload, opts) {
  return new Promise(function (resolve, reject) {
    ...
    const reqOpts = {
      ...
      cert: mat.cert,
      key: mat.key,
      rejectUnauthorized: false, // trust is by thumbprint pin, verified on secureConnect
      agent: false,              // fresh connection so secureConnect always fires; no pooled socket
      timeout: timeout,
      headers: {
    ...
    req.on('socket', function (socket) {
      socket.on('secureConnect', function () {
        ...
        const got = cert ? normalizeThumb(cert.fingerprint256) : null;
        if (!got || !pinned || got !== pinned) {
          req.destroy(new Error('ha-peer-link: peer certificate pin mismatch'));
          return;
        }
        // Pin verified -- only now does any request data leave this node.
        req.write(body);
        req.end();
      });
    });
