// FIREALIVE -- HA replication key-agreement (B5o)
//
// Active/passive HA needs the passive to read the active's Tier-1-encrypted
// columns and validate its JWT_SECRET-signed sessions AFTER it promotes -- but
// the Tier-1 KEK is sealed to each node's own hardware (tier1-kek.js: "a copied
// disk or cloned VM cannot unseal it"), so the pair cannot simply share a sealed
// KEK. The decided design (B5o pre-flight, Q1.b) is anchor-to-anchor key-
// agreement: each node holds a purpose-built X25519 key-agreement keypair whose
// PUBLIC key is signed by the hardware anchor (binding it to that hardware
// identity), and whose PRIVATE key is sealed to local hardware. At pairing the
// active wraps the shared Tier-1 material (the raw KEK + JWT_SECRET bundle) to
// the passive's X25519 public key via ephemeral-static ECDH; the passive stores
// the wrapped blob and unwraps it ONLY at promotion, with its own hardware. This
// preserves the anti-clone guarantee for every non-paired deployment: no raw KEK
// is ever shared, and the wrapped blob is opaque without the passive's sealed key.
//
// The hardware anchor key (hardware-anchor.js) is sign-only (ECDSA P-256, IEEE
// P1363 over SHA-256); it cannot perform ECDH/decrypt itself, which is exactly
// why a separate X25519 key authenticated by an anchor signature does the
// wrapping. The X25519 private key is sealed under a key HKDF-derived from the
// local hardware-sealed Tier-1 KEK (domain-separated), so it is recoverable only
// on this hardware.
//
...
const HKDF_LEN = 32;
const WRAP_SEAL_INFO = 'firealive-ha-wrapkey-seal-v1'; // derive the X25519-private-key sealing key from the Tier-1 KEK
const WRAP_SEAL_SALT = 'firealive-ha-wrapkey-seal-salt-v1';
const KEK_WRAP_INFO = 'firealive-ha-kek-wrap-v1';      // derive the ECDH KEK-wrapping key
...
function sealToHardware(secretBuf) {
  if (!Buffer.isBuffer(secretBuf)) {
    throw new Error('ha-keys.sealToHardware: secret must be a Buffer');
  }
...
// ---------------------------------------------------------------------------
// ha_node 'self' row + the local X25519 wrap keypair.
// ---------------------------------------------------------------------------

function ensureSelfRow(db) {
  db.prepare("INSERT OR IGNORE INTO ha_node (id) VALUES ('self')").run();
}

// Ensure this node has an X25519 key-agreement keypair, the anchor signature
// binding its public key to this hardware identity, and its ha_node row.
// Idempotent. Returns { wrapPublicPem, wrapPubkeyAnchorSig }.
function ensureWrapKeypair(db) {
  ensureSelfRow(db);
  const existing = db.prepare("SELECT wrap_public_pem, wrap_pubkey_anchor_sig FROM ha_node WHERE id = 'self'").get();
  ...
  if (!sig) {
    throw new Error('ha-keys.ensureWrapKeypair: cannot anchor-sign the wrap key -- no hardware instance identity (fail-closed)');
  }
  const sigB64 = Buffer.from(sig).toString('base64');
  db.prepare(
    "UPDATE ha_node SET wrap_public_pem = ?, wrap_private_sealed = ?, wrap_pubkey_anchor_sig = ?, updated_at = datetime('now') WHERE id = 'self'"
...
function getLocalWrapPublic(db) {
  const row = db.prepare("SELECT wrap_public_pem, wrap_pubkey_anchor_sig FROM ha_node WHERE id = 'self'").get();
...
// SHA-256 -- the shape hardware-anchor uses). Returns a boolean.
...
function wrapKekToPeer(secretBuf, peerWrapPublicPem) {
  if (!Buffer.isBuffer(secretBuf)) {
    throw new Error('ha-keys.wrapKekToPeer: secret must be a Buffer');
  }
...
function unwrapKekWithLocal(db, wrappedB64) {
  const row = db.prepare("SELECT wrap_private_sealed FROM ha_node WHERE id = 'self'").get();
  if (!row || !row.wrap_private_sealed) {
    throw new Error('ha-keys.unwrapKekWithLocal: no sealed wrap private key on this node');
  }
