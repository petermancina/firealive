// FIREALIVE -- HA replication engine (B5o)
//
// The active is the sole writer. ha-cdc triggers append each committed row
// change to ha_replication_journal; this module ships those rows to the passive
// and applies them there. The split:
//
...
// silently drop FK children. BLOB columns arrive hex-encoded (ha-cdc) and are
...
//
// shipOnce takes the send function by injection (the peer link is a later
// commit), so this module has no transport dependency. ASCII-only; no template
// literals.
...
function ensureStateRow(db) {
  db.prepare("INSERT OR IGNORE INTO ha_replication_state (id) VALUES ('self')").run();
}

function getState(db) {
  ensureStateRow(db);
  return db.prepare("SELECT * FROM ha_replication_state WHERE id = 'self'").get();
}

function currentEpoch(db) {
  const r = db.prepare("SELECT epoch FROM ha_lease WHERE id = 'current'").get();
  return r ? r.epoch : 0;
}
...
function collectBatch(db, maxRows) {
  ...
  const rows = db.prepare(
    "SELECT lsn, epoch, table_name, op, pk_json, row_json FROM ha_replication_journal WHERE lsn > ? ORDER BY lsn LIMIT ?"
...
function recordAck(db, ackedLsn, opts) {
  ensureStateRow(db);
  db.prepare(
    "UPDATE ha_replication_state SET last_shipped_lsn = MAX(last_shipped_lsn, ?), last_acked_lsn = MAX(last_acked_lsn, ?) WHERE id = 'self'"
  ).run(ackedLsn, ackedLsn);
  db.prepare("UPDATE ha_replication_journal SET shipped = 1 WHERE lsn <= ?").run(ackedLsn);
...
function pruneJournal(db, opts) {
  ...
  const st = getState(db);
  db.prepare("DELETE FROM ha_replication_journal WHERE shipped = 1 AND lsn <= (? - ?)").run(st.last_acked_lsn, retain);
}
...
function computeLag(db) {
  ...
  const oldest = db.prepare(
    "SELECT created_at FROM ha_replication_journal WHERE lsn > ? ORDER BY lsn LIMIT 1"
  ...
  db.prepare(
    "UPDATE ha_replication_state SET lag_seconds = ?, last_journaled_lsn = (SELECT COALESCE(MAX(lsn), 0) FROM ha_replication_journal) WHERE id = 'self'"
  ).run(lag);
  return lag;
}
...
async function shipOnce(db, sendFn, opts) {
  ...
  if (!ack || ack.ok === false) {
    const err = new Error('ha-replication: peer rejected batch' + (ack && ack.reason ? ' (' + ack.reason + ')' : ''));
...
function applyBatch(db, batch) {
  ...
  // from the schema by ha-cdc), and the canonical schema name -- an element of
  ...
  const replicated = haCdc.listReplicatedTables(db);
  for (let i = 0; i < rows.length; i++) {
  ...
  try {
    ...
    db.prepare("UPDATE ha_replication_state SET last_applied_lsn = ?, last_apply_at = datetime('now') WHERE id = 'self'").run(lastLsn);
    db.exec('COMMIT');
