...
// where the ref is a storage_destinations.id). Reads config only, never credentials.
...
function listDestinationsForCategory(db, category) {
  if (category === 'backup') {
    const rows = db.prepare('SELECT id FROM storage_destinations WHERE enabled = 1').all();
