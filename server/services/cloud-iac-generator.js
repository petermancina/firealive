// ═══════════════════════════════════════════════════════════════════════════════
// FIREALIVE — Cloud & IaC Generator (R3k C13)
//
...
function buildReadme(provider, iacTool, snapshot, packageId, signingKeyFingerprint) {
  const lines = [
    ...
    `- \`sbom.spdx.json\` — Software Bill of Materials for the FireAlive install being deployed.`,
    `- \`bundle-manifest.json\` — per-file SHA-256 integrity attestation.`,
    `- \`README.md\` — this file.`,
    ``,
    `## Verifying the signature`,
    ``,
    `The archive \`bundle.tar.gz\` is signed by FireAlive's Cloud & IaC signing key. Verify with cosign:`,
    ``,
    `\`\`\`bash`,
    ...
    ``,
    `Retrieve the public key via \`GET /api/cloud/packages/${packageId}/public-key\` on the originating FireAlive install.`,
    ``,
    `## Secrets mapping`,
    ``,
    ...
    ``,
    `- \`TIER1_ENCRYPTION_KEY\` — 32-byte hex KEK for the v2 backup engine (must match the source install or be re-keyed during restore).`,
    `- \`JWT_SECRET\` — at least 16 random characters, never share.`,
    `- \`BACKUP_DIR\` (optional) — override default \`/data/backups\`.`,
    `- \`CLOUD_PACKAGES_DIR\` (optional) — override default \`/data/cloud-packages\`.`,
    ``,
    ...
    `4. After first boot, run \`POST /api/regression/run\` against the new install to confirm health.`,
    ``,
  ];
  return lines.join('\n');
}
...
function buildBundleManifest(packageId, provider, iacTool, snapshot, files, sbomEntry, signingKeyId, signingKeyFingerprint) {
  return {
    format: 'firealive-cloud-iac-bundle-v1',
...
function generatePackage(db, provider, iacTool, options = {}) {
  ...
  let templateModule;
  try {
    templateModule = require('./cloud-iac-templates/' + iacTool);
  } catch (e) {
    throw new Error(
