// ═══════════════════════════════════════════════════════════════════════════
// FIREALIVE — Pre-upgrade restore points  (B6k)
...
// WHY THIS EXISTS
//
// FireAlive never installs an update. services/update-check.js detects a newer
// release and routes/auto-update.js notifies; the operator downloads and runs
...
// makes that specific: services/fuse-high-water.js records the highest fuse
...
// composes services/backup-full-suite.js and adds exactly two things: it writes
// the bundle to a store OUTSIDE the data root, and it records why the bundle
// was taken.
//
// WHERE IT GOES, AND WHY THAT IS THE WHOLE POINT
//
// lib/data-root.restorePointsDir() is a SIBLING of the data root, not a child.
...
const { appendAuditEntry } = require('./audit-chain');
...
function sourceFuseCounter(db) {
  ...
  if (Number.isInteger(version.fuseCounter) && version.fuseCounter >= 0) return version.fuseCounter;
...
async function createRestorePoint(db, opts) {
  ...
  let backup;
  try {
    backup = await performFullSuiteBackup({ type: BACKUP_TYPE, backupDir: store });
  } catch (err) {
  ...
  const bundleDir = path.dirname(backup.manifest_path);
  ...
  db.prepare(
    ...
    fuse, version.version, createdAt, opts.userId,
  ...
  try {
    appendAuditEntry(db, {
      ...
      detail: 'restore_point=' + id + ' backup=' + backup.id + ' fuse=' + fuse
        + ' version=' + version.version + ' dir=' + bundleDir,
      ip: opts.ip,
    });
  } catch (auditErr) {
