// ═══════════════════════════════════════════════════════════════════════════════
// FIREALIVE — Cosign Signer (R3k C10)
//
...
// CosignNotInstalledError. The caller in routes/cloud.js (C14)
...
function signBlob(blobPath, privateKeyPem, outputSignaturePath, options = {}) {
  ...
  const tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'firealive-cosign-' + crypto.randomBytes(8).toString('hex') + '-'),
  );
