// ============================================================================
// server/services/restore-chain.js
//
...
//
// The INCR-v1 bundle format (read here, written by C63 backup-incremental
// and C64 backup-differential) is documented in backup-incremental.js.
//
...
// INCR-v1 format constants — must match backup-incremental.js. Duplicated
// here rather than imported to avoid a runtime require dependency on
// backup-incremental (this module is also used by the restore route which
// may run in a context where backup-incremental hasn't loaded).
...
async function replayChain(db, chain, targetDbPath, options = {}) {
  ...
  try {
    ...
    logger.error('restore-chain: anchor replay failed', { anchorId: anchor.id, error: err.message });
  ...
  for (let i = 1; i < chain.length; i++) {
    ...
    if (strategy === 'full' || strategy === 'snapshot') {
      ...
      logger.warn('restore-chain: unexpected full/snapshot mid-chain; skipping', { id: link.id, strategy });
      continue;
    }
    ...
    try {
      ...
      logger.error('restore-chain: link replay failed', { id: link.id, strategy, error: err.message });
      return {
        ok: false,
