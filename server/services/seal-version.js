// ─────────────────────────────────────────────────────────────────────────────
// FIREALIVE ── Seal-Version High-Water (Anti-Rollback) + straggler report
// Copyright (C) 2026 Peter Mancina
// SPDX-License-Identifier: AGPL-3.0-or-later
// ─────────────────────────────────────────────────────────────────────────────
//
// Boot-time seal-format anti-rollback (B6h B-5), mirroring fuse-high-water.js.
...
// Read-only decode of a stored value to the envelope buffer for a version peek,
// per the column's storage format. This mirrors tier1-seal.decodeStorage but only
// to read the version byte -- no key, no decrypt. (GD stores the envelope as the
// value itself, so its twin needs no decode.)
function toEnvelopeBuffer(stored, storage) {
  if (stored === null || stored === undefined) return null;
  if (Buffer.isBuffer(stored)) return stored;
  if (storage === 'base64') return Buffer.from(String(stored), 'base64');
  if (storage === 'hex') return Buffer.from(String(stored), 'hex');
  return Buffer.from(String(stored));
}
...
function reportStragglers(db) {
  ...
  for (let i = 0; i < cols.length; i++) {
    ...
    for (let j = 0; j < rows.length; j++) {
      total++;
      const buf = toEnvelopeBuffer(rows[j].val, c.storage);
      const v = (buf === null) ? cur : tier1Envelope.sealVersionOf(buf);
      if (v < cur) {
        below++;
