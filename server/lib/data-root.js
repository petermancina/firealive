...
// application bundle in a packaged build. If a deployment still has one there,
// starting against an empty new root would present the operator with an empty
// console and every appearance of catastrophic data loss. Refuse instead, and
// name both paths.
//
// This refuses whenever the legacy database exists, not only when the new root
// is empty. A second database full of analyst records sitting at an abandoned
// path is not merely a startup ambiguity: a data-subject crypto-erase wipes the
// live database and leaves that copy intact, backups do not cover it, and
// nothing answers which is the system of record. Removing it is a deliberate,
// one-time operator action, and it must be deliberate.
//
...
function legacyDbPath() {
  return path.join(__dirname, '..', '..', 'data', 'firealive.db');
}
...
function assertNoLegacyDatabase() {
  ...
  throw new Error(
    'FireAlive refuses to start: a database exists at the pre-P1 location.\n'
    + '  legacy:  ' + legacy + '\n'
    ...
    + (currentExists
      ? 'Both are present. Confirm which is authoritative, then remove the legacy '
        ...
        + 'and is not erased by a data-subject request.'
      : 'Starting now would create an empty database and look like total data '
        ...
        + 'FireAlive will not move a KEK-sealed database for you.')
  );
}
