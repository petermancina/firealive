...
// on purpose.
//
// The owner and the process user are listed separately because on Windows they
// are separate fields and need not agree: a directory created by a member of
// Administrators is OWNED by BUILTIN\\Administrators while the creating user
// holds its own explicit ACE. On POSIX the process user simply is the owner and
...
function offendingAces(dir) {
  ...
  // See the $me note in the script: the process user and the owner are two
  // different fields on Windows and either may be the POSIX "owner".
