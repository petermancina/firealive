...
async function start() {
  try {
    ...
    // the CA and other long-lived keys are minted. Verify entropy first
    // (fail-closed); identity-store errors are non-fatal (the mint gate then
    // refuses). Idempotent -- re-boots load the existing identity.
    try {
      ...
      try {
        ...
        let preferVtpm = false;
        try {
          const modeRow = identityDb.prepare("SELECT value FROM system_meta WHERE key = 'deployment_mode'").get();
          preferVtpm = !!(modeRow && modeRow.value && modeRow.value !== 'bare-metal');
        } catch (modeErr) {
          preferVtpm = false;
        }
        const identity = instanceAnchor.establish({ db: identityDb, logger: logger, preferVtpm: preferVtpm });
      ...
      logger.error('Instance identity establishment failed', { error: instanceIdentityErr.message });
    }
    ...
    // this block by design, so it is not closed here).
    try {
