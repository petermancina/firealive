        ...
        {tab==="restore"&&(<div>
          ...
          <Card style={{marginBottom:16,borderColor:C.d+"30"}}>
            ...
            {erPreview&&(<Card style={{marginTop:12,padding:12,background:"rgba(0,0,0,0.2)"}}>
              ...
              <div style={{display:"flex",gap:6,marginBottom:8,flexWrap:"wrap"}}><Badge color={erPreview.manifestSigOk?C.g:C.r}>Sig: {erPreview.manifestSigOk?"VERIFIED":(erPreview.signingKeyKnown?"FAILED":"UNKNOWN KEY")}</Badge><Badge color={erPreview.structure&&erPreview.structure.ok?C.g:C.r}>Files: {erPreview.structure?erPreview.structure.present.length:0}/4</Badge><Badge color={C.tm}>Key: {(erPreview.signingKeyId||"").slice(0,12)}…</Badge></div>
              {!erPreview.signingKeyKnown&&(<M style={{color:C.r,display:"block",marginBottom:8}}>⚠ Signing key {erPreview.signingKeyId} is not registered locally. Register the public key in backup_signing_keys before restore can proceed.</M>)}
