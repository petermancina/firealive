        ...
        {tab==="backup"&&(<div>
          ...
          <div style={{background:C.s,border:`1px solid ${C.b}`,borderRadius:10,overflow:"hidden"}}>{backups.map(b=>{
            ...
            <div key={b.id} id={"backup-row-"+b.id} style={{padding:"10px 14px",borderBottom:`1px solid ${C.b}`}}>
              ...
              {isChainLink&&(
                <div style={{marginTop:6,paddingTop:6,borderTop:`1px dashed ${C.b}`,display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:6,fontSize:10,color:C.td}}>
                  <span>
                    ↳ parent:&nbsp;
                    <span style={{cursor:"pointer",color:C.i,textDecoration:"underline"}} onClick={()=>jumpToRow(b.parent_backup_id)}>{String(b.parent_backup_id||"").slice(0,12)}</span>
                    {b.parent_full_backup_id&&b.parent_full_backup_id!==b.parent_backup_id&&(<>&nbsp;·&nbsp;anchor:&nbsp;<span style={{cursor:"pointer",color:C.i,textDecoration:"underline"}} onClick={()=>jumpToRow(b.parent_full_backup_id)}>{String(b.parent_full_backup_id).slice(0,12)}</span></>)}
