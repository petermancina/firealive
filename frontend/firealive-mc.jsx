    ...
    <div style={{minHeight:"100vh",background:"#050810",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{maxWidth:520,width:"100%",padding:"0 24px",animation:"fadeIn 0.5s ease"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
          {[{r:"analyst",icon:"◉",label:"SOC Analyst",desc:"Personal wellbeing client.\nYour data stays private.\nTier-3 encrypted.",color:C.a,bg:C.ad,bc:C.a+"30"},
            {r:"manager",icon:"◈",label:"Team Lead",desc:"Team capacity dashboard.\nAggregate metrics only.\nNo individual burnout data.",color:"#C8D6E5",bg:"rgba(255,255,255,0.03)",bc:C.b}
          ].map(o=>(
            <button key={o.r} onClick={()=>onSelect(o.r)} style={{padding:"32px 20px",background:o.bg,border:`1px solid ${o.bc}`,borderRadius:16,color:o.color,cursor:"pointer",textAlign:"center",transition:"transform 0.15s ease"}} onMouseEnter={e=>e.currentTarget.style.transform="scale(1.02)"} onMouseLeave={e=>e.currentTarget.style.transform="scale(1)"}>
              <div style={{fontSize:32,marginBottom:14}}>{o.icon}</div>
              <div style={{fontSize:16,fontWeight:500,marginBottom:10}}>{o.label}</div>
              <M style={{color:C.tm,lineHeight:1.7,whiteSpace:"pre-line"}}>{o.desc}</M>
            </button>
          ))}
        </div>
        <Card style={{marginTop:24,padding:12,textAlign:"center"}}>
          <M style={{color:C.td,lineHeight:1.7}}>Production: separate services, separate databases, RBAC at API gateway.<br/>IAM roles: soc_analyst_group and soc_teamlead_group in Active Directory.</M>
        </Card>
      </div>
    </div>
  );
...
function LoginScreen({role, onLogin, onBack}) {
  ...
  const handleCreds = async () => {
    ...
    if(apiMode && window.FireAliveAPI) {
      try {
        const userData = await window.FireAliveAPI.login(user, pass);
        // Check role matches
        if((role==="analyst" && userData.role!=="analyst") || (role==="manager" && !["lead","admin"].includes(userData.role))) {
          setError("Account role does not match selected role.");
          setLoading(false); return;
        }
        setLoading(false); setStage("mfa");
      } catch(e) {
        setLoading(false); setError(e.message || "Authentication failed.");
      }
    } else {
  ...
  const handleMFA = () => {
    if(mfa.length<6){setError("Enter 6-digit code.");return;}
    setLoading(true);setError("");
    setTimeout(()=>{setLoading(false);setStage("verify");
  ...
  return (
    <div style={{minHeight:"100vh",background:"#050810",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans',sans-serif"}}>
      <style>{CSS}</style>
      <div style={{maxWidth:420,width:"100%",padding:"0 24px",animation:"fadeIn 0.4s ease"}}>
        ...
        {stage==="mfa"&&(
          <Card style={{padding:24}}>
            <L>Enter MFA Code</L>
            <M style={{color:C.tm,display:"block",marginBottom:16,lineHeight:1.6}}>Enter the code from your authenticator app.</M>
        ...
        {stage==="verify"&&(
          <Card style={{padding:24,textAlign:"center",borderColor:C.a+"30"}}>
            <div style={{width:24,height:24,border:`2px solid ${C.a}`,borderTopColor:"transparent",borderRadius:"50%",animation:"spin 0.8s linear infinite",margin:"0 auto 16px"}}/>
            <div style={{fontSize:13,color:C.a}}>Verifying...</div>
            <M style={{color:C.td,display:"block",marginTop:8}}>Loading...</M>
          </Card>
        )}
      </div>
    </div>
...
function BreathingExercise() {
  ...
  const [count, setCount] = useState(1); // counts UP 1→4
  const [totalSec, setTotalSec] = useState(120); // 2-min countdown
  const phases = ["Breathe In","Hold","Breathe Out","Hold"];
  const colors = [C.a,C.i,C.p,C.w];
  const circleSize = 120;
  const progress = (count - 1) / 3; // 0→1 within each 4s phase (count goes 1,2,3,4)

  useEffect(()=>{
    if(!active || totalSec <= 0) { if(totalSec <= 0 && active) setActive(false); return; }
    const iv = setInterval(()=>{
      setTotalSec(prev => prev - 1);
      setCount(prev=>{
        if(prev>=4){setPhase(p=>(p+1)%4);return 1;}
        return prev+1;
      });
    },1000);
  ...
  const done = totalSec <= 0 && !active;

  // Circle animation scale: expands on inhale, holds, contracts on exhale, holds
  const getScale = () => {
    if(!active) return 1;
    if(phase === 0) return 1 + progress * 0.3; // inhale: grow
    if(phase === 1) return 1.3; // hold at max
    if(phase === 2) return 1.3 - progress * 0.3; // exhale: shrink
    return 1; // hold at min
  };

  return (
    <Card style={{borderColor:C.a+"30",marginBottom:16}}>
      <div style={{fontSize:13,fontWeight:500,color:C.a,marginBottom:10}}>Box Breathing Exercise</div>
      <M style={{color:C.tm,display:"block",marginBottom:12,lineHeight:1.6}}>A guided breathing technique used by Navy SEALs, first responders, and high-stress professionals. Activates the parasympathetic nervous system to reduce acute stress. 4 seconds in, 4 seconds hold, 4 seconds out, 4 seconds hold. Takes 2 minutes. Safe to do at your desk.</M>
      <M style={{color:C.w,display:"block",marginBottom:16,lineHeight:1.6,fontSize:9}}>If you have any respiratory, cardiovascular, or other medical conditions, consult your physician before practicing breathing exercises. If you feel dizzy or lightheaded at any point, stop immediately and breathe normally.</M>
      
      {!active && !done && (
        <div style={{textAlign:"center",padding:"12px 0"}}>
          <Btn primary onClick={()=>{setActive(true);setPhase(0);setCount(1);setTotalSec(120);}}>Start Box Breathing (2 min)</Btn>
        </div>
      )}

      {done && (
        <div style={{textAlign:"center",padding:"20px 0"}}>
          <div style={{fontSize:18,color:C.a,marginBottom:8,fontFamily:"'Fraunces',serif"}}>Complete</div>
          <M style={{color:C.tm,display:"block",marginBottom:12}}>Well done. Notice how you feel compared to before you started.</M>
          <Btn primary onClick={()=>{setActive(true);setPhase(0);setCount(1);setTotalSec(120);}}>Do Another Round</Btn>
        </div>
      )}
      ...
      {active && (
        <div style={{textAlign:"center",padding:"8px 0"}}>
          ...
          <div style={{position:"relative",width:circleSize,height:circleSize,margin:"0 auto 16px",display:"flex",alignItems:"center",justifyContent:"center"}}>
            <div style={{
              position:"absolute",width:"100%",height:"100%",borderRadius:"50%",
              background:`radial-gradient(circle, ${colors[phase]}15 0%, transparent 70%)`,
              border:`2px solid ${colors[phase]}40`,
              transform:`scale(${getScale()})`,
              transition:"transform 0.9s ease-in-out, border-color 0.3s ease, background 0.3s ease",
            }}/>
            <div style={{position:"relative",zIndex:1}}>
              <div style={{fontSize:36,fontWeight:300,color:colors[phase],fontFamily:"'Fraunces',serif",lineHeight:1}}>{count}</div>
              <div style={{fontSize:13,color:colors[phase],fontWeight:500,marginTop:4}}>{phases[phase]}</div>
            </div>
          </div>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:6,marginBottom:12}}>
            {[0,1,2,3].map(i=><div key={i} style={{
              width:phase===i?24:8,height:8,borderRadius:4,
              background:phase===i?colors[i]:C.b,
          ...
          <div style={{marginBottom:12}}>
            <M style={{color:C.tm,fontSize:12}}>Time remaining: </M>
            <M style={{color:totalSec<=30?C.w:C.a,fontSize:14,fontWeight:500}}>{mins}:{secs.toString().padStart(2,"0")}</M>
          </div>

          {/* Progress bar */}
          <div style={{width:"80%",height:3,background:"rgba(255,255,255,0.04)",borderRadius:2,overflow:"hidden",margin:"0 auto 12px"}}>
            <div style={{width:`${((120-totalSec)/120)*100}%`,height:"100%",background:C.a,borderRadius:2,transition:"width 1s linear"}}/>
          </div>

          <Btn small onClick={()=>{setActive(false);setTotalSec(120);}}>Stop</Btn>
        </div>
      )}
    </Card>
  );
...
function AnalystClient() {
  ...
  const resetLockTimer = () => {
    ...
    lockTimerRef.current = setTimeout(()=>setAppLocked(true), LOCK_TIMEOUT_MS);
  };
  React.useEffect(()=>{
    const events = ["mousedown","keydown","touchstart","scroll"];
    events.forEach(e=>window.addEventListener(e, resetLockTimer));
    resetLockTimer();
    return ()=>{ events.forEach(e=>window.removeEventListener(e, resetLockTimer)); if(lockTimerRef.current) clearTimeout(lockTimerRef.current); };
  },[]);
  // v1.0.0: biometrics config (analyst-side)
  const [biometricEnabled, setBiometricEnabled] = useState(false);
  // v1.0.0: session ID for concurrent prevention
  const [clientSessionId] = useState(()=>Math.random().toString(36).slice(2,14));
  const [tab, setTab] = useState("home");
  const [consentLog, setCL] = useState([]);
  ...
  // v1.0.0 analyst state
  const [peerBoardMsgs, setPeerBoardMsgs] = useState([
    {id:"pb1",content:"Pro tip: If you're getting buried in DNS tunneling alerts, build a baseline of known-good resolver IPs first. Cut my FP rate by 60%.",anonymous:true,category:"technical",createdAt:"2026-04-05T14:00:00Z",reactions:{"👍":3,"🔥":1}},
    {id:"pb2",content:"After a rough ransomware week, I started time-boxing my triage to 45-min blocks with 10-min breaks. Huge difference in how I felt by end of shift.",anonymous:true,category:"burnout_prevention",createdAt:"2026-04-06T09:00:00Z",reactions:{"👍":5,"💡":2}},
    // Board posts populated by analyst submissions.
  ]);
  ...
  // v1.0.0 management state — MOVED to ManagementConsole in v1.0.0 (was causing crash)

  const signals = {investigationTime:{base:20,cur:26,u:"min",label:"Avg time per alert",key:"investigationTime"},dismissRate:{base:15,cur:19,u:"%",label:"Closed without notes",key:"dismissRate"},ticketQuality:{base:82,cur:76,u:"%",label:"Documentation quality",hib:true,key:"ticketQuality"},escalationRate:{base:11,cur:14,u:"%",label:"Escalation rate",key:"escalationRate"}};
  const stage="watch";const daysE=4;
  const sc={watch:{hl:"A few signals are drifting.",sub:"Worth noticing — not worrying about yet.",c:C.w}}[stage];
  ...
  const logC = (a,dt) => setCL(prev=>[...prev,{ts:new Date().toISOString(),a,dt:dt||"Anonymous — no name/signals transmitted"}]);

  const tabs=[{id:"home",label:"Home"},{id:"signals",label:"My Signals"},{id:"delegate",label:"Delegate"},{id:"peers",label:"Peers"},{id:"board",label:"Board"},{id:"ooda",label:"IR Simulator"},{id:"training",label:"Training"},{id:"certs",label:"Certificates"},{id:"skills",label:"Skills & Assessments"},{id:"recovery",label:"Post-Incident Wellness"},{id:"privacy",label:"Privacy"}];

  // v1.0.0: Lock screen
  if(appLocked) return(
    <div style={{minHeight:"100vh",background:"#060A10",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column"}}>
      <style>{CSS}</style>
      <div style={{textAlign:"center",padding:40}}>
        ...
        <M style={{color:C.td,display:"block",marginBottom:24}}>Session locked due to inactivity. Notifications continue in background.</M>
        <div style={{maxWidth:280,margin:"0 auto"}}>
          <input type="password" value={lockPin} onChange={e=>setLockPin(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&lockPin.length>0){setAppLocked(false);setLockPin("");resetLockTimer();}}} placeholder="Enter password to unlock" style={{width:"100%",padding:12,background:"rgba(255,255,255,0.03)",border:`1px solid ${C.b}`,borderRadius:8,color:C.t,fontSize:13,textAlign:"center",marginBottom:12}}/>
          <button onClick={()=>{if(lockPin.length>0){setAppLocked(false);setLockPin("");resetLockTimer();}}} style={{width:"100%",padding:10,background:C.ad,border:`1px solid ${C.a}50`,borderRadius:8,color:C.a,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"'IBM Plex Mono',monospace"}}>Unlock</button>
          {biometricEnabled&&<button onClick={()=>{setAppLocked(false);resetLockTimer();}} style={{width:"100%",padding:10,marginTop:8,background:"transparent",border:`1px solid ${C.b}`,borderRadius:8,color:C.tm,fontSize:11,cursor:"pointer",fontFamily:"'IBM Plex Mono',monospace"}}>Use Biometric (Fingerprint / Face)</button>}
        </div>
      </div>
    </div>
  );
    ...
    <div style={{minHeight:"100vh",background:"#060A10",color:C.t,fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{borderBottom:`1px solid ${C.b}`,background:C.s,padding:"16px 24px"}}>
        <M style={{color:C.td,letterSpacing:2,textTransform:"uppercase",fontSize:9,display:"block",marginBottom:6}}>
          <span style={{display:"inline-block",width:5,height:5,borderRadius:"50%",background:C.a,marginRight:6,boxShadow:`0 0 6px ${C.a}`}}/>
          Analyst Client · Private · Tier-3 Encrypted
        </M>
        <div style={{fontSize:18,fontWeight:300,color:"#E8EDF5",fontFamily:"'Fraunces',serif"}}>FireAlive · Your Wellbeing Dashboard</div>
      </div>
      <Tabs tabs={tabs} active={tab} onTab={setTab}/>
      <div style={{padding:24,maxWidth:700,animation:"fadeIn 0.3s ease"}}>

        {tab==="home"&&(<div>
          <Card style={{marginBottom:20,borderLeft:`3px solid ${sc.c}`,cursor:"pointer"}} onClick={()=>setTab("signals")}>
            <div style={{fontSize:16,fontWeight:500,color:"#E8EDF5",marginBottom:4}}>{sc.hl}</div>
            <div style={{fontSize:13,color:C.tm,lineHeight:1.6}}>{sc.sub}</div>
            <M style={{color:sc.c,marginTop:10}}>{daysE} days · Tap to view signals →</M>
          </Card>
          <L>Your actions made a difference</L>
          {impacts.map((imp,i)=><Card key={i} style={{marginBottom:8,padding:"12px 16px",display:"flex",gap:12}}><M style={{color:C.td,minWidth:48}}>{imp.d}</M><div style={{flex:1}}><div style={{fontSize:12,color:C.t}}>{imp.e}</div><div style={{fontSize:11,color:C.a}}>{imp.o}</div></div>{imp.v&&<Badge color={C.a}>verified</Badge>}</Card>)}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:16}}>
            <Card onClick={()=>{setTab("delegate");setShowDel(true);}}><div style={{fontSize:12,fontWeight:500,color:"#E8EDF5",marginBottom:4}}>Delegate a pattern</div><M style={{color:C.tm}}>Send repeating alerts to automation</M></Card>
            <Card onClick={()=>setShowLQ(true)}><div style={{fontSize:12,fontWeight:500,color:"#E8EDF5",marginBottom:4}}>Request lighter queue</div><M style={{color:C.tm}}>Temp complexity cap — anonymous</M></Card>
            <Card onClick={()=>setTab("peers")}><div style={{fontSize:12,fontWeight:500,color:"#E8EDF5",marginBottom:4}}>Peer skill-share</div><M style={{color:C.tm}}>Ask a peer for advice, put your heads together to solve a problem, learn tricks of the trade, prevent burnout</M></Card>
            <Card onClick={()=>setShowLeadMsg(true)}><div style={{fontSize:12,fontWeight:500,color:"#E8EDF5",marginBottom:4}}>Message my lead</div><M style={{color:C.tm}}>Encrypted, identified — your lead will know who you are</M></Card>
          </div>
        </div>)}

        {tab==="signals"&&(<div>
          <L>Your signals · compared to your own baseline</L>
          <M style={{color:C.tm,display:"block",marginBottom:20,lineHeight:1.5}}>Only you see this. Signals are calculated from your on-shift behavioral data: investigation times, dismiss rates, documentation quality scores, and escalation patterns. These metrics are collected locally and compared to your personal 30-day rolling baseline. Click any signal for pathways.</M>
          {Object.entries(signals).map(([key,sig])=>{
            const delta=sig.hib?sig.base-sig.cur:sig.cur-sig.base;const pct=Math.abs(delta)/sig.base*100;
            const bad=delta>0;const color=bad?(pct>30?C.d:C.w):C.a;
            const intv=SIGNAL_INTERVENTIONS[key];
            return(<div key={key}>
              <Card style={{marginBottom:4,cursor:"pointer",borderColor:showSig===key?C.ba:C.b}} onClick={()=>setShowSig(showSig===key?null:key)}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                  <span style={{fontSize:12}}>{sig.label}</span>
                  <M style={{color}}>{sig.cur}{sig.u} {bad?"↑":"↓"} from {sig.base}{sig.u}</M>
                </div>
                <div style={{height:4,background:"rgba(255,255,255,0.04)",borderRadius:2,overflow:"hidden"}}><div style={{width:`${Math.min(100,(sig.cur/(sig.base*2))*100)}%`,height:"100%",background:color,borderRadius:2,transition:"width 0.5s ease"}}/></div>
              </Card>
              {showSig===key&&intv&&(
                <div style={{padding:"12px 16px",background:"rgba(255,255,255,0.01)",border:`1px solid ${C.b}`,borderTop:"none",borderRadius:"0 0 12px 12px",marginBottom:12}}>
                  <M style={{color:C.td,display:"block",marginBottom:10}}>{intv.label}</M>
                  {intv.paths.map((p,i)=>(
                    <Card key={i} style={{marginBottom:8,padding:"12px 14px",borderLeft:`3px solid ${p.type==="platform"?C.i:p.type==="ai"?C.p:p.type==="peer"?"#F472B6":C.w}`}}>
                      <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                        <div style={{fontSize:12,fontWeight:500,color:"#E8EDF5"}}>{p.title}</div>
                        <Badge color={p.type==="platform"?C.i:p.type==="ai"?C.p:p.type==="peer"?"#F472B6":C.w}>{p.type==="platform"?"training":p.type}</Badge>
                      </div>
                      <div style={{fontSize:11,color:C.tm,lineHeight:1.6,marginBottom:8}}>{p.desc}</div>
                      {p.type==="platform"&&p.pid&&<Btn small primary onClick={e=>{e.stopPropagation();setShowPlatform(p.pid);}}>View platform →</Btn>}
                      {p.type==="ai"&&<Btn small onClick={e=>{e.stopPropagation();setShowPlatform("claude-ai");}}>Start AI session →</Btn>}
                      {p.type==="peer"&&<Btn small onClick={e=>{e.stopPropagation();setShowPeerSched(true);}}>Schedule →</Btn>}
                      {p.type==="structural"&&<Btn small onClick={e=>{e.stopPropagation();setShowLQ(true);}}>Request →</Btn>}
                    </Card>
                  ))}
                </div>
              )}
        ...
        {tab==="delegate"&&(<div>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:16}}><div><L style={{marginBottom:4}}>Delegate to automation</L><M style={{color:C.tm}}>Each accepted delegation permanently reduces team load.</M></div><Btn primary onClick={()=>setShowDel(true)}>+ New</Btn></div>
          {delegations.map(d=><Card key={d.id} style={{marginBottom:8,padding:"12px 16px"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:12}}>{d.pat}</span><Badge color={d.st==="accepted"?C.a:C.w}>{d.st}</Badge></div><M style={{color:C.tm}}>{d.ts} → {d.sys}</M></Card>)}
          {showDel&&<Modal title="Delegate pattern" onClose={()=>setShowDel(false)}>
            <textarea value={newDel} onChange={e=>setNewDel(e.target.value)} placeholder="Describe the repeating pattern..." maxLength={1000} style={{width:"100%",height:80,padding:12,background:"rgba(255,255,255,0.03)",border:`1px solid ${C.b}`,borderRadius:8,color:C.t,fontSize:12,resize:"vertical"}}/>
            ...
            <div style={{display:"flex",gap:8}}><Btn primary style={{flex:1}} onClick={()=>{if(newDel.trim()){setDel(prev=>[...prev,{id:Date.now(),ts:new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'}),pat:newDel,st:"pending",sys:AUTO_SYS_INIT.find(s=>s.id===selSys)?.name||""}]);setNewDel("");setShowDel(false);}}}>Submit</Btn><Btn onClick={()=>setShowDel(false)}>Cancel</Btn></div>
          </Modal>}
        </div>)}
            ...
            <Card key={m.id} style={{marginBottom:8,padding:"14px 16px",borderLeft:`3px solid ${catColors[m.category]||C.tm}`}}>
              ...
              <div style={{display:"flex",gap:6}}>
                {Object.entries(m.reactions||{}).map(([emoji,count])=>(
                  <div key={emoji} style={{padding:"2px 8px",background:"rgba(255,255,255,0.04)",border:`1px solid ${C.b}`,borderRadius:12,fontSize:11,cursor:"pointer"}}>{emoji} {count}</div>
                ))}
                <div onClick={()=>{setPeerBoardMsgs(prev=>prev.map(p=>p.id===m.id?{...p,reactions:{...p.reactions,"👍":(p.reactions["👍"]||0)+1}}:p));}} style={{padding:"2px 8px",background:"rgba(255,255,255,0.02)",border:`1px solid ${C.b}`,borderRadius:12,fontSize:11,cursor:"pointer"}}>+👍</div>
                <div onClick={()=>{setPeerBoardMsgs(prev=>prev.map(p=>p.id===m.id?{...p,reactions:{...p.reactions,"💡":(p.reactions["💡"]||0)+1}}:p));}} style={{padding:"2px 8px",background:"rgba(255,255,255,0.02)",border:`1px solid ${C.b}`,borderRadius:12,fontSize:11,cursor:"pointer"}}>+💡</div>
              </div>
            </Card>);
          })}
        ...
        {tab==="training"&&(<div>
          ...
          <M style={{color:C.tm,display:"block",marginBottom:8,lineHeight:1.6}}>Connected training platforms. Schedule for off-shift hours. Remote VPN access available. Progress syncs to your signal baselines.</M>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {TRAINING_PLATFORMS.map(tp=>(
              <Card key={tp.id} style={{cursor:"pointer",borderColor:tp.id==="claude-ai"?C.p+"30":C.b}} onClick={()=>setShowPlatform(tp.id)}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                  <div style={{fontSize:13,fontWeight:500,color:"#E8EDF5"}}>{tp.name}</div>
                  <Badge color={tp.id==="claude-ai"?C.p:C.i}>{tp.type}</Badge>
                </div>
                <div style={{fontSize:11,color:C.tm,lineHeight:1.5,marginBottom:8}}>{tp.desc}</div>
                <M style={{color:C.td,lineHeight:1.6}}>{tp.integration}</M>
              </Card>
            ))}
          </div>
        </div>)}
        ...
        {tab==="recovery"&&(<div>
          ...
          <Card style={{marginBottom:16}}>
            ...
            {[{t:"Sleep Hygiene",d:"Maintain consistent sleep schedule. Avoid screens 30 min before bed. If you cannot sleep, get up and do something quiet.",ic:"\u{1F319}"},{t:"Physical Movement",d:"Even 10-15 minutes of walking or stretching helps process stress hormones. One of the most evidence-supported recovery tools.",ic:"\u{1F3C3}"},{t:"Box Breathing",d:"Inhale 4 seconds, hold 4, exhale 4, hold 4. Repeat 4 cycles. Activates parasympathetic nervous system.",ic:"\u{1FAC1}"},{t:"Social Connection",d:"Talk to someone you trust. You do not have to talk about the incident. Being with safe people helps.",ic:"\u{1F91D}"},{t:"Limit Re-Exposure",d:"Avoid going through logs or incident details during off-hours. Psychological detachment during recovery is a top predictor of reduced exhaustion (R032).",ic:"\u{1F6AB}"},{t:"Routine Maintenance",d:"Keep eating regular meals, staying hydrated, maintaining routines. Disrupted habits amplify stress.",ic:"\u{1F37D}\u{FE0F}"}].map((s,i)=>(
              <Card key={i} style={{marginBottom:8,padding:"12px 14px",borderLeft:`3px solid ${C.a}`}}>
                <div style={{fontSize:12,fontWeight:500,color:"#E8EDF5",marginBottom:4}}>{s.ic} {s.t}</div>
        ...
        {tab==="skills"&&(<div>
              ...
              <Card key={i} style={{marginBottom:8,padding:"12px 14px",borderLeft:`3px solid ${a.status==="pending"?C.w:C.a}`}}>
                ...
                {a.status==="pending"&&(
                  <div style={{marginTop:8}}>
                    ...
                    {a.platforms.map((p,j)=>{
                      const urls={"Hack The Box":"https://hackthebox.com","LetsDefend":"https://letsdefend.io","CyberDefenders":"https://cyberdefenders.org","TryHackMe":"https://tryhackme.com"};
                      return(
                        <Card key={j} style={{marginBottom:4,padding:"8px 12px",borderLeft:`3px solid ${C.i}`,cursor:"pointer"}} onClick={()=>{logC("assessment_lab_launched",`${a.name} → ${p}`)}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                            <div><M style={{color:"#E8EDF5",fontWeight:500}}>{p}</M><M style={{color:C.td,display:"block"}}>{urls[p]||"#"}</M></div>
                            <Badge color={C.i}>Launch →</Badge>
                          </div>
                        </Card>
                      );
                    })}
                    <M style={{color:C.tm,display:"block",marginTop:6,lineHeight:1.5,fontSize:10}}>Progress tracked via API integration (SAML SSO / OAuth + webhook). Completion data syncs to your skill matrix automatically. Your lead sees aggregate results only after you finish all assigned labs.</M>
                  </div>
                )}
              </Card>
            ))}
          ...
          <Card style={{marginBottom:16,borderColor:C.w+"30"}}>
            <div style={{fontSize:13,fontWeight:500,color:C.w,marginBottom:10}}>Recommended Training (from Assessment Gaps)</div>
            <M style={{color:C.tm,display:"block",marginBottom:10,lineHeight:1.6}}>These recommendations are generated from skills below 70% in your assessments. Complete the training, then re-assess to demonstrate growth. Your lead will see the improvement signal.</M>
            {[{skill:"SIEM Queries (65%)",recs:[{p:"TryHackMe",t:"SOC Level 1 — SIEM Module",d:"Structured Splunk/Elastic query labs"},{p:"LetsDefend",t:"Alert Correlation Practice",d:"Real SIEM query scenarios in simulated environment"}]},
              {skill:"Investigation (62%)",recs:[{p:"Hack The Box",t:"SOC Analyst Path",d:"Hands-on investigation labs building pattern recognition"},{p:"LetsDefend",t:"Incident Investigation Scenarios",d:"Full investigation workflow practice"}]},
              {skill:"Escalation Judgment (55%)",recs:[{p:"SANS GCIH",t:"Incident Handler Certification",d:"Gold-standard training for escalation decision-making (enrolled)"},{p:"Peer Mentoring Program",t:"Calibrate with L2/L3 analyst",d:"Escalation thresholds are partly cultural — peer calibration recalibrates judgment"}]},
              {skill:"Threat Hunting (28%)",recs:[{p:"CyberDefenders",t:"Blue Team Challenges",d:"DFIR and hunting exercises to build baseline skills"},{p:"Hack The Box",t:"Threat Hunter Path",d:"Advanced proactive detection labs"}]},
              {skill:"Malware Analysis (15%)",recs:[{p:"CyberDefenders",t:"Malware Traffic Analysis",d:"PCAP-based malware identification exercises"},{p:"Immersive Labs",t:"Adaptive Malware Course",d:"AI-adjusted difficulty based on current skill level"}]}
            ].map((g,i)=>(
              <Card key={i} style={{marginBottom:10,padding:"10px 14px",borderLeft:`3px solid ${C.w}`}}>
                <div style={{fontSize:12,fontWeight:500,color:C.w,marginBottom:6}}>{g.skill}</div>
                {g.recs.map((r,j)=>(
                  <div key={j} style={{padding:"6px 0",borderBottom:j<g.recs.length-1?`1px solid ${C.b}`:"none"}}>
                    <div style={{display:"flex",justifyContent:"space-between"}}><M style={{color:"#E8EDF5",fontWeight:500}}>{r.t}</M><Badge color={C.i}>{r.p}</Badge></div>
                    <M style={{color:C.tm,lineHeight:1.4}}>{r.d}</M>
                  </div>
                ))}
              </Card>
            ))}
        ...
        {/* ══════════ OODA IR SIMULATOR — ANALYST ══════════ */}
        {tab==="ooda"&&(<div>
          <L>Incident Response Simulator (OODA Loop)</L>
          <M style={{color:C.tm,display:"block",marginBottom:8,lineHeight:1.6}}>Practice incident response using your organization's policies. Choose wisely — wrong paths explain why they fail and do not let you proceed. Only correct choices advance.</M>
          ...
          {(()=>{
            ...
            if(oodaStage>=phases.length) return(
              <Card style={{borderColor:C.a+"40",textAlign:"center",padding:24}}>
                <div style={{fontSize:18,color:C.a,fontFamily:"'Fraunces',serif",marginBottom:8}}>Exercise Complete</div>
                <M style={{color:C.tm,display:"block",marginBottom:16,lineHeight:1.6}}>You navigated all four OODA phases successfully.</M>
                <Btn primary onClick={()=>{setOodaStage(0);setOodaFb(null);}}>Restart Exercise</Btn>
              </Card>
            );
          ...
          <Card style={{padding:"12px 14px",marginTop:12}}><M style={{color:C.td}}>More exercises generated when your lead uploads IR policies from the Management Console.</M></Card>
        </div>)}

        {/* ══════════ TRAINING CERTIFICATES ══════════ */}
        {tab==="certs"&&(<div>
          <L>Training Certificates</L>
          <M style={{color:C.tm,display:"block",marginBottom:16,lineHeight:1.6}}>For security, FireAlive does not link to external training sites. Complete training on the platforms recommended in your Training tab, then submit your completion certificate or verification code here. Your team lead will verify and the completion will count toward your skill progression.</M>
          <Card style={{marginBottom:16}}>
            <div style={{fontSize:13,fontWeight:500,color:"#E8EDF5",marginBottom:10}}>Submit Completion Proof</div>
            <Input label="Skill" placeholder="e.g., Threat Hunting"/>
            <Sel label="Platform">
              <option value="letsdefend">LetsDefend</option><option value="htb">Hack The Box</option><option value="thm">TryHackMe</option><option value="cyberdefenders">CyberDefenders</option><option value="sans">SANS Institute</option><option value="immersive">Immersive Labs</option><option value="internal">Internal Training</option>
            </Sel>
            <Input label="Course Name" placeholder="e.g., SOC Analyst Learning Path"/>
            <Input label="Verification Code" placeholder="Certificate verification code (if available)"/>
            <M style={{color:C.td,display:"block",marginBottom:10,lineHeight:1.5}}>Or upload your certificate file (PDF/image, max 5MB). Files are encrypted at rest and never shared with peers.</M>
            <Btn primary onClick={()=>logC("cert_submitted","Certificate submitted for review")}>Submit for Verification</Btn>
          </Card>
          <Card>
            <div style={{fontSize:13,fontWeight:500,color:"#E8EDF5",marginBottom:10}}>My Submissions</div>
            <M style={{color:C.td}}>No certificates submitted yet. Complete a training course and submit proof above.</M>
          </Card>
        </div>)}

        {tab==="privacy"&&(<div>
          <L>Data Privacy & Consent Log</L>
          <Card style={{marginBottom:16,borderColor:C.a+"30"}}>
          ...
          <L>Consent Events</L>
          {consentLog.length===0?<M style={{color:C.tm,fontStyle:"italic"}}>No data has crossed the privacy boundary.</M>:
          consentLog.map((c,i)=><Card key={i} style={{marginBottom:6,padding:"10px 14px"}}><M style={{color:C.w}}>{new Date(c.ts).toLocaleTimeString()}</M><M style={{color:C.tm}}> · {c.a} · </M><M style={{color:C.td}}>{c.dt}</M></Card>)}
        </div>)}
      </div>
      ...
      {showPlatform&&<Modal title={TRAINING_PLATFORMS.find(t=>t.id===showPlatform)?.name||""} onClose={()=>setShowPlatform(null)} width={520}>
        {(()=>{const tp=TRAINING_PLATFORMS.find(t=>t.id===showPlatform);if(!tp)return null;return(<>
          ...
          <Card style={{marginBottom:16,padding:12,borderColor:C.a+"30"}}>
            <M style={{color:C.a,fontWeight:500,display:"block",marginBottom:4}}>Integration Method</M>
            <M style={{color:C.tm,lineHeight:1.6}}>{tp.integration}</M>
          </Card>
          {tp.url!=="#internal"&&<Card style={{marginBottom:16,padding:12}}>
            <M style={{color:C.td,display:"block",marginBottom:4}}>Platform URL</M>
            <M style={{color:C.i}}>{tp.url}</M>
          </Card>}
          <Input label="Schedule a session" type="datetime-local" value={schedDate} onChange={e=>setSchedDate(e.target.value)}/>
          <Sel label="Reminder" value={schedReminder} onChange={e=>setSchedReminder(e.target.value)}><option value="15min">15 min</option><option value="1hr">1 hour</option><option value="1day">1 day</option></Sel>
          <Btn primary style={{width:"100%"}} disabled={!schedDate} onClick={()=>{setScheduled(prev=>[...prev,{id:Date.now(),title:tp.name,date:schedDate,reminder:schedReminder,platform:tp.name}]);setShowPlatform(null);setSchedDate("");}}>Schedule</Btn>
        </>);})()}
      </Modal>}

      <div style={{padding:"14px 24px",borderTop:`1px solid ${C.b}`,fontSize:10,color:C.td,fontFamily:"'IBM Plex Mono',monospace",display:"flex",justifyContent:"space-between"}}><span>TIER-3 · ENCRYPTED · PRIVATE</span><span>{consentLog.length} consent events</span></div>
    </div>
  );
}
...
function ManagementConsole() {
  ...
  // policies and scenarios from the backend. The mock state from earlier
  // builds is replaced with API data; setOodaPolicies on tab activation
  // overwrites whatever stale entries were there.
  useEffect(()=>{
    ...
    stage('Regression') { steps { sh 'npm run regression-test' } }
        ...
        {tab==="ooda_mgmt"&&(<div>
          ...
          oodaPolicies.map(p=><Card key={p.id} style={{marginBottom:8,padding:"12px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div><M style={{color:C.t}}>{p.title}</M><br/><M style={{color:C.td}}>{p.policy_type||p.type} · {new Date(p.uploaded_at||p.uploadedAt).toLocaleDateString()}</M></div>
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
        ...
        {tab==="restore"&&(<div>
          ...
          <Card style={{marginBottom:16}}>
          <Card style={{marginBottom:16,borderColor:C.d+"30"}}><div style={{fontSize:13,fontWeight:600,color:C.d,marginBottom:10}}>External Restore</div><Sel label="Source"><option value="">Select...</option><option>Network</option><option>NAS</option><option>S3</option><option>Azure</option><option>SFTP</option></Sel><Input label="Path" placeholder="smb://..."/><Input label="Key" type="password"/><div style={{padding:14,background:"rgba(0,0,0,0.2)",borderRadius:8,border:"2px dashed "+C.b,textAlign:"center",marginTop:8,cursor:"pointer"}} onClick={()=>api.post("/api/v054/clients/restore",{action:"browse"}).then(()=>addA("EX","Browsing"))}><M style={{color:C.tm}}>Browse</M></div><Btn danger style={{marginTop:12}} onClick={()=>{if(window.confirm("Restore?")){api.post("/api/v054/clients/restore",{action:"restore"}).then(()=>addA("EX","Restored"));}}}>Restore</Btn></Card>
        ...
        {tab==="mfa"&&(<div>
          <L>Multi-Factor Authentication</L>
            <Card style={{marginBottom:12,borderColor:C.i+"30"}}><div style={{fontSize:13,fontWeight:600,color:C.i,marginBottom:10}}>TOTP Setup</div><div style={{background:"rgba(0,0,0,0.3)",borderRadius:12,padding:20,textAlign:"center",marginBottom:12}}><div style={{width:140,height:140,margin:"0 auto",background:"#fff",borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center"}}><M style={{color:"#000",fontSize:10}}>TOTP QR</M></div></div><Input label="Key" value="JBSWY3DPEHPK3PXP" readOnly/><Input label="Code" placeholder="000000" maxLength={6}/><Btn primary onClick={()=>api.post("/api/auth/mfa/verify",{}).then(()=>addA("MF","MFA verified"))}>Verify</Btn></Card>
          <M style={{color:C.tm,display:"block",marginBottom:16,lineHeight:1.6}}>Configure MFA for FireAlive's built-in authentication. If you use enterprise IAM (SAML/OIDC) with MFA already enforced at the IdP, this is redundant — your IdP handles MFA. This wizard is for deployments without IAM integration.</M>
          <Card style={{marginBottom:16}}>
        ...
        {tab==="legal_hold"&&(<div>
          ...
          <Btn primary onClick={()=>addA("LEGAL_HOLD_EXPORT","Legal hold export initiated — hash: "+legalHoldCfg.hashAlgorithm+", format: "+legalHoldCfg.format)}>Execute Legal Hold Export</Btn>
        </div>)}
