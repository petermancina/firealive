...
2. **Verify Now** re-runs the full check (recompute every row + prev_hash linkage + signed-checkpoint validation) and advances the checkpoint when intact.
3. A background check also runs hourly: on any break it raises a critical alert (the MC routes it through the alert router to SIEM/SOAR/notification; the GD records a critical audit event), and a separate gap check flags missing rows or offline windows.
4. On a break, the lead investigates whether it's a partition (system was offline) or tampering, using the reason, the broken row id, and the SIEM copy as the external cross-check.
