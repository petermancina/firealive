...
6. The Global Dashboard runs its own CISO-gated suite (28 checks) via `POST /api/regression-test`, covering GD schema + FK integrity, AES-256-GCM / SHA-256 / Ed25519 crypto round-trips, auth-flow round-trip (bcrypt + JWT), MC-trust signing-key coverage, cross-region rollup, compliance tables, backups, and the same forward-aware audit-chain check. The AI, model-safety, peer, helper-pay, and IAM checks are intentionally not ported — those subsystems do not run on the GD.
