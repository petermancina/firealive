...
**Model-file integrity & safety gate (before load, fail-closed):** Hash-pinning proves a file is *the* official artifact, but the loader (node-llama-cpp / llama.cpp) parses the GGUF and runs **in-process**, so FireAlive runs a layered gate over every model file before the loader reads it — on both the server and the Analyst Client. The layers run in order and short-circuit on the first failure:
...
**Loader privilege hardening (server):** because the loader runs in-process, FireAlive refuses to load a model as **root** in production (set `FIREALIVE_ALLOW_ROOT_MODEL_LOAD=1` only if a constrained environment genuinely requires it) and recommends running the service as a non-root user, with the model directory mounted read-only, dropped Linux capabilities, a seccomp/AppArmor profile, and no network egress from the inference service. Moving the loader into a sandboxed worker is planned future work.
